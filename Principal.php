...
$cons="Select Nombre,BAColorFon,BAColorLet,BATipoLet,BATamLet,BAEstLet,BBColorFon,BBColorLet,BBTipoLet,BBTamLet,BBEstLet,Nit,Direccion,Telefonos,FirmaPresupuesto,NomFirmPres,Codigo,codsgsss,Departamento,Municipio,NomMunicipio, dependencia
...
$Compania[20]=$filaCompania['dependencia'];
$Compania[7]=$filaCompania[20];
$fechaMigracionCSJDM = "2014-05-05"; // Esta variable se usa para modificar la manera en la que se muestran (Datos.php) y se imprimen(ImpHistoria.php) los formatos migrados
$Estilo=$filaCompania;
...
$consulta="Select primape,segape,primnom,segnom,contrato,numero,fechaini,fechafin,monto,estado,entidad,consumcontra,(fechafin::Date-fechaini::Date),(current_date-fechaini::date)
  ...
  /*$consMP="update contratacionsalud.contratos set mttoejecutado='$ME', porcentajeejecutado='".number_format($PE,3,".",",")."',
	...
	$consMP="update contratacionsalud.contratos set mttoejecutado='$ME', porcentajeejecutado='".$PE."',
           diascontrato='$row[12]', porcentajedias='".$PD."', diastranscurridocontrato='$row[13]' where 
           contrato='$row[4]' and numero='$row[5]' and fechaini='$row[6]' and fechafin='$row[7]' and estado='$row[9]' and entidad='$row[10]' and consumcontra='$row[11]'";
...
if($_GET['volver'] == 1) {?>
		<html>
			...
			<FRAMESET name="Principal" COLS="305,*" FRAMEBORDER=0 FRAMESPACING=0 BORDER=0>
			   <FRAME SRC="Modulos.php?DatNameSID=<? echo $DatNameSID?>" NAME="Encabezados" marginheight=8 marginwidth=8>
				<FRAMESET ROWS="115,*" FRAMEBORDER=0 FRAMESPACING=0 BORDER=0>
					...
					<FRAME SRC="HistoriaClinica/BuscarHistoria.php?DatNameSID=<? echo $DatNameSID?>&UnidadIraLista=<? echo $_GET['UnidadIraLista'];?>" NAME="Derecha" marginheight=8 marginwidth=8>
				</FRAMESET>
...
<FRAMESET name="Principal" COLS="285,*" FRAMEBORDER=0 FRAMESPACING=0 BORDER=0>
   <FRAME SRC="Modulos.php?DatNameSID=<? echo $DatNameSID?>" NAME="Encabezados" marginheight=8 marginwidth=8>
	<FRAMESET ROWS="115,*" FRAMEBORDER=0 FRAMESPACING=0 BORDER=0>
		<FRAME SRC="EncOpciones.php?DatNameSID=<? echo $DatNameSID?>" NAME="Encab" marginheight=8 marginwidth=8>
