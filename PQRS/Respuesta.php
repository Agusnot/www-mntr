<?php
    ...
    if($Guardar){
        ...
        $cons="insert into pqrs.respuesta(comentario_respuesta, fecha_respuesta, id_secuencia, id_pqrs, sigsecuencia_respuesta, id_usuario) values ('".$comentario_respuesta."', '".$fecha_respuesta."', ".$id_secuencia.", ".$pqrs.", ".$sigsecuencia_respuesta.", '".$usuario[1]."')";
        $res=ExQuery($cons);
		
		header("Location: /PQRS/Respuesta.php?DatNameSID=".$DatNameSID."&pqrs=".$pqrs);
    }
    /*if($editar){
...
<html>
    <head>
        ...
        <!--<link rel="stylesheet" href="link/css/ui-lightness/jquery-ui-1.10.4.css">-->
        <link rel="stylesheet" href="link/css/cupertino/jquery-ui.css">
    ...
    <body>
                                    ...
                                    if($fila2['sigsecuencia_respuesta']==='0'){
                                        ...
                                        if(ExNumRows($res2)){
                                            ...
                                            $conssec="SELECT * FROM pqrs.secuencia, pqrs.respuesta 
                                                WHERE secuencia.id_secuencia=respuesta.sigsecuencia_respuesta 
                                            ...
                                            // Consulto la secuencia para comprobar si requiere de visto bueno y no se ha creado ningún visto bueno para esa respuesta
                                            // Y no exista registros de de respuesta
                                            $conssec="SELECT secuencia.* FROM pqrs.pqrs, pqrs.clasetipopqrs, pqrs.tipopqrs, pqrs.secuencia
                                        ...
                                        if($fila2['sigsecuencia_respuesta']){
                                            // Consulto la secuencia para comprobar si se debe crear acciones
                                            $consacc="SELECT * FROM pqrs.secuencia, pqrs.respuesta 
                                                WHERE secuencia.id_secuencia=respuesta.sigsecuencia_respuesta 
                                                    AND secuencia.id_secuencia=".$fila2['sigsecuencia_respuesta']."
                                                    AND secuencia.hijo_secuencia=1";
                                            $resacc=ExQuery($consacc);
                                            $filaacc=ExFetchAssoc($resacc);

                                            if($filaacc['hijo_secuencia']){
                                                // Consulto la secuencia para comprobar si se debe crear acciones
                                                $consacc2="SELECT * FROM pqrs.accion, pqrs.accionxusuario WHERE accion.id_accion=accionxusuario.id_accion AND accion.id_pqrs=$pqrs";
                                                $resacc2=ExQuery($consacc2);
                                                
                                                if(ExNumRows($resacc2)>0){
                                                    $comentario_axu = "";
                                                    while($filaacc2=ExFetchAssoc($resacc2)){
                                                        $comentario_axu .= "Colaborador: ".$filaacc2['id_usuario']."\\nDescripción: ".$filaacc2['descripcion_accion']."\\nFecha: ".$filaacc2['fechalimite_accion']."\\n\\n";
                                                    }
                                                    ?>
                                                        <script>
                                                            $(function() {
                                                                $("#comentario_respuesta").append("<?php echo $comentario_axu; ?>");
                                                            });
                                                        </script>
                                                    <?php
                                                }
                                                ...
                                                <script>
                                                    $(function() {
                                                        $("#comentario_respuesta").attr("readonly","readonly");
                                                        $( "#comentario_respuesta" ).click(function(e) {
                                                            var dlg=$('#dialog').dialog({
                                                                ...
                                                                buttons: {
                                                                    "Guardar": function() {
                                                                        // Permite agregar la descripción recién ingresada
                                                                        var mystr = $("#id_gestoraccion").val();
                                                                        var myarr = mystr.split(",");
                                                                        var myvar = $("#comentario_respuesta").val();
                                                                        for (var i=0; i<myarr.length-1; i++){
                                                                            myvar = myvar+"Colaborador: "+myarr[i]+"\nDescripción: "+$("#descripcion_accion").val()+"\nFecha: "+$("#fechalimite_accion").val()+"\n\n";
                                                                        }
                                                                        //var myvar = myarr[1] + ":" + myarr[2];

                                                                        $("#comentario_respuesta").val(myvar);
                                                                        //$( this ).dialog( "close" );

                                                                        $("#accionForm").submit(function(event) {
                                                                            event.preventDefault();
                                                                            var posting = $.post("PostAccionesXUsuario.php", {descripcion_accion: $("#descripcion_accion").val(), fechalimite_accion: $("#fechalimite_accion").val(), id_pqrs: "<?php echo $pqrs; ?>", id_gestoraccion: $("#id_gestoraccion").val()});
                                                                            //alert("Posstt: "+posting);
                                                                        });
                                                            ...
                                                            e.preventDefault();
                                                            dlg.load('AccionesXUsuario.php?pqrs=<?php echo $pqrs; ?>', function(){
                                                                dlg.dialog('open');
                                                            });
                                                        });
                                                    });
                                        ...
                                        if(ExNumRows($ressec)>0){
                                                ...
                                                while($fila4=ExFetchAssoc($res4)){
                                                    if($usuario[1]==$fila4['usuario']){
                                                        ...
                                                        ?>
                                                            ...
                                                            <td><textarea name="comentario_respuesta" id="comentario_respuesta" rows="7" cols="60"><?php echo $fila['comentario_respuesta']; ?></textarea></td>
                                            ...
                                            if(ExNumRows($res2)){
                                                ...
                                                // Si encuentra un usuario asignado a esa secuencia solo pueden hacer cambios esas personas
                                                $cons33="SELECT secuencia.*, secuenciaxusuario.* FROM pqrs.pqrs, pqrs.clasetipopqrs, pqrs.tipopqrs, pqrs.secuencia, pqrs.secuenciaxusuario 
                                                    WHERE pqrs.id_clasetipopqrs=clasetipopqrs.id_clasetipopqrs 
                                                        AND clasetipopqrs.id_tipopqrs=tipopqrs.id_tipopqrs 
                                                        AND secuencia.id_tipopqrs=tipopqrs.id_tipopqrs 
                                                        AND secuencia.id_secuencia=secuenciaxusuario.id_secuencia 
                                                        AND secuencia.padre_secuencia=0 
                                                        AND pqrs.id_pqrs=$pqrs";
                                                $res33=ExQuery($cons33);
                                            }
        ...
        <table class="imagetable">
            <tr>
                    <th>PASO</th><th>USUARIO</th><th>FECHA</th><th>COMENTARIO</th>
            </tr>
            <?php
		...
		//$fila=ExFetch($res);
                while($fila=ExFetchAssoc($res)){
                    ...
                    <tr>
                        ...
                        <td><?php echo str_replace("\n", "<br>", $fila['comentario_respuesta']); ?></td>
