...
<table BORDER=1  style='font : normal normal small-caps 12px Tahoma;' border="1" bordercolor="#e5e5e5" cellpadding="2" align="center">  
    ...
    <tr>
  	...
  	<?	$cons="Select numero from contratacionsalud.contratos
        ...
        <td>
        	<select name="Tipo">
            	<option value="Sin devolver" <? if($Tipo=="Sin devolver"){?> selected<? }?>>Sin devolver</option>
                <option value="devueltas" <? if($Tipo=="devueltas"){?> selected<? }?>>devueltas</option>
            </select>
        </td>
...
<? if($Ver){?>
<iframe frameborder="0" id="VerLiquidado" src="devolverfacturas.php?DatNameSID=<? echo $DatNameSID?>&FechaIni=<? echo $FechaIni?>&FechaFin=<? echo $FechaFin?>&Entidad=<? echo $Entidad?>&Contrato=<? echo $Contrato?>&NoContrato=<? echo $NoContrato?>&Tipo=<? echo $Tipo?>" width="100%" height="85%">
</iframe>
<? }?>
</body>
</html>
