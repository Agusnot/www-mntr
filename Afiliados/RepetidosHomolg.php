...
*/
	...
	$FilasRep=explode(";",$Repetidos);
	foreach($FilasRep as $RepsFinal){
		$fila=explode(",",$RepsFinal);
		if($fila[0]){
			$NumRec++;
			$Datos[$NumRec]=array($fila[1],$fila[2]." ".$fila[3]." ".$fila[4]." ".$fila[5],$fila[0]);
		}
	}
	/*	?>
			<tr>
...
//Cabecera de página
	...
	//    $this->Image('/Imgs/Logo.jpg',10,8,33);
		...
		$this->Ln(4);
		$this->Cell(0,8,"INFORME USUARIOS REPETIDOS EN LA BASE DE DATOS AFILIADOS",0,0,'C');   	
		$this->Ln(10);
		...
		$this->Cell(30,5,"No. Repeticiones",1,0,'C',1);
		$this->Ln(5);
	}
