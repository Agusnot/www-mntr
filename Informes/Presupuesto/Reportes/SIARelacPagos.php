...
?>
	...
	<tr><td>Fecha de Pago</td><td>Codigo Presupuestal</td><td>Tipo de Pago</td><td>Fte Financiacion</td><td>No Comprobante</td><td>Beneficiario</td><td>Cedula o NIT</td><td>Detalle de Pago</td><td>Valor Comprobante</td><td>Desctos Seg Social</td>
<?
	$cons2="Select Cuenta from Contabilidad.PlanCuentas where Cuenta like '2436%' and Anio=$Anio and Compania='$Compania[0]'";
	$res2=ExQuery($cons2);
...
<td>Otros Descuentos</td><td>Neto Pagado</td><td>Banco</td><td>No de Cuenta</td><td>Cheque</td><td>Cta Contable</td><td>Nombre</td></tr>
...
<?
	...
	$cons3="Select sum(Haber),Comprobante,Numero,Cuenta from Contabilidad.Movimiento where Compania='$Compania[0]' and Estado='AC' and Cuenta like '2436%' and date_part('year',Fecha)=$Anio
	Group By Comprobante,Numero";
	$res3=ExQuery($cons3);
	while($fila3=ExFetch($res3))
	{
		$DatosRetenc[$fila3[1]][$fila3[2]][$fila3[3]]=$fila3[0];
	}
