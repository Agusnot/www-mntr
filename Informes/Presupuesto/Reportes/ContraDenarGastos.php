...
<center>
Ejecución Presupuestal de Gastos<br />
...
<tr style="font-weight:bold" bgcolor="#e5e5e5" align="center"><td>Codigo Presupuestal</td><td>Concepto del Rubro</td><td>Apropiacion Inicial</td><td>Adiciones</td><td>Reducciones</td><td>Creditos</td><td>Contra Creditos</td><td>Apropiación Definitiva</td><td>Ejecucion del Periodo</td></tr>
<?
	}
	Encabezados();
	if($Anio){
	$cons="Select Cuenta,Nombre,length(Cuenta) as Digitos from PlanCuentas where Cuenta like '2%' and PlanCuentas.Compania='$Compania[0]' and Anio=$Anio and Vigencia='Actual' having Digitos<=$NoDigitos Order By Cuenta";
	$res=ExQuery($cons);
	while($fila=ExFetch($res))
	{
		...
		$TotCompromisos=$Valores["Compromiso Presupuestal"][$fila[0]]["Credito"];
		$TotObligaciones=$Valores["Obligacion presupuestal"][$fila[0]]["Credito"];
		$Egresos=$Valores["Egreso presupuestal"][$fila[0]]["Credito"];
		$DispoSinAfecta=$TotDispo-$TotCompromisos;//Falta
		$CompromisosSinAfecta=$TotCompromisos-$TotObligaciones;//Falta
		$ObligacSinAfectar=$TotObligaciones-$Egresos;
		$SaldoDispo=$ApropDef-$TotDispo;

		echo "<td align='right'>".number_format($Egresos,2)."</td>";
		$NumRec++;
	}
	}
?>
