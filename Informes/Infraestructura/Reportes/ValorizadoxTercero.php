...
<?
	function Encabezados()
	{
		...
		<? echo $Compania[1]?><br>VALORIZADO POR TERCEROS <br>Corte a: <? echo "$Fecha"?></td></tr>
...
$cons = "Select CodElementos.AutoId,CodElementos.Codigo,Nombre,Caracteristicas,Modelo,Marca,Serie,CostoInicial,DepAcumulada,Ubicaciones.Responsable,
PrimApe,SegApe,PrimNom,SegNom, AxICostoIni, AxIDepAcumulada
From Infraestructura.Ubicaciones,Central.Terceros,Infraestructura.CodElementos 
Where Ubicaciones.Compania='$Compania[0]' and Terceros.Compania='$Compania[0]' and CodElementos.Compania='$Compania[0]' 
and Terceros.Identificacion = Ubicaciones.Responsable and Ubicaciones.AutoId = CodElementos.AutoId 
...
and (UVE IS NOT NULL) and (Eliminado != 1 or Eliminado IS NULL) order by Ubicaciones.Responsable";
//echo $cons;
$res = ExQuery($cons);
while($fila = ExFetch($res))
{
        ...
        if(!$PDF)
	{
		if($fila[9] != $RespAnt)
		{
			if($RespAnt)
			{
				echo "<tr bgcolor='$Estilo[1]'  style='color:white;font-weight:bold;'><td colspan='2' align='right'>TOTALIZADO($RespAnt)</td>
				<td align='right'>".number_format($TotalCostoIni[$RespAnt],2)."</td>
                                <td align='right'>".number_format($TotalAxICI[$RespAnt],2)."</td>
				<td align='right'>".number_format($TotalDeprecia[$RespAnt],2)."</td>
                                <td align='right'>".number_format($TotalAxIDA[$RespAnt],2)."</td>
				<td align='right'>".number_format($TotalSaldo[$RespAnt],2)."</td></tr>";
			}
			echo "<tr bgcolor='$Estilo[1]'  style='color:white;font-weight:bold;'><td colspan='7' align='center'>$fila[9] - $fila[10] $fila[11] $fila[12] $fila[13]</td></tr>";
		}
	}
	$Depreciacion = $fila[8] + $Dep[$fila[0]];
	$Saldo = $fila[7] - $fila[8];
	$TotalCostoIni[$fila[9]] = $TotalCostoIni[$fila[9]] + $fila[7];
        $TotalAxICI[$fila[9]] = $TotalAxICI[$fila[9]] + $fila[14];
	$TotalDeprecia[$fila[9]] = $TotalDeprecia[$fila[9]] + $Depreciacion;
	$TotalAxIDA[$fila[9]] = $TotalAxIDA[$fila[9]] + $fila[15];
	...
	if(!$PDF)
	{
		echo "<tr><td>$fila[1]</td><td>$fila[2] $fila[3] $fila[4] $fila[5] $fila[6]</td>
                ...
                <td align='right'>".number_format($fila[14],2)."</td>
                <td align='right'>".number_format($Depreciacion,2)."</td>
                <td align='right'>".number_format($fila[15],2)."</td>
	...
	$Datos["$fila[9] - $fila[10] $fila[11] $fila[12] $fila[13]"][$fila[0]]=
        array($fila[1],"$fila[2] $fila[3] $fila[4] $fila[5] $fila[6]",$fila[7],$fila[14],$Depreciacion,$fila[15],$Saldo,$fila[9]);
    }
	...
	class PDF extends FPDF
	{
		function BasicTable($data)
		{
			...
			while(list($cad,$val) = each($data))
			{
				...
				$this->Cell(250,5,"TERCERO: ".utf8_decode(strtoupper($cad)),'1',0,'L',1);
				...
				foreach($val as $Elemento)
				{
					...
					foreach($Elemento as $col)
					{
						...
						if($x==1){$col = substr(utf8_decode($col),0,50);}
