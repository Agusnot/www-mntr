...
<?
	function Encabezados()
	{
        ...
        <td>Codigo</td><td>Elemento</td><td>Ultimo Responsable</td><td>Ultimo CC</td><td>Usuario Elimina</td></tr>
    <?	}
Encabezados();	
}
...
and Eliminado=1 order by FechaFin asc";
$res = ExQuery($cons);
while($fila = ExFetch($res))
{
	$UltUb[$fila[0]] = array("$fila[2] $fila[3] $fila[4] $fila[5]","$fila[6] - $fila[7]");	
}
$cons = "Select AutoId,Codigo,Nombre,Caracteristicas,Modelo,Serie,FechaElimina,UsuarioElimina
From Infraestructura.CodElementos 
Where Compania='$Compania[0]'
And Eliminado = 1 order by FechaElimina";
//echo $cons;
$res = ExQuery($cons);
while($fila = ExFetch($res))
{
	
	$fila[6]=substr($fila[6],0,10);
	$FechaI = explode("-",$Fecha);if($FechaI[1]<10){$FechaI[1]="0".$FechaI[1];}if($FechaI[2]<10){"0".$FechaI[2];}
	$FechaE = explode("-",$fila[6]);if($FechaE[1]<10){$FechaE[1]="0".$FechaE[1];}if($FechaE[2]<10){"0".$FechaE[2];}
	...
	if("$FechaE[0]-$FechaE[1]-$FechaE[2]"<="$FechaI[0]-$FechaI[1]-$FechaI[2]")
	{
		if(!$PDF)
		{
			...
			echo "<tr><td>$fila[1]</td><td>$fila[2] $fila[3] $fila[4] $fila[5]</td><td>".$UltUb[$fila[0]][0]."</td><td>".$UltUb[$fila[0]][1]."</td><td>$fila[7]</td>";
			$NumRec++;
			if($NumRec == $Encabezados)
			{ echo "</table><br />"; Encabezados();$NumRec=0;}		
		}
		$Datos[$fila[0]]=array($fila[1],"$fila[2] $fila[3] $fila[4] $fila[5]",$UltUb[$fila[0]][0],$UltUb[$fila[0]][1],$fila[7]);	
	}
}
if($PDF)
{
		...
		function BasicTable($data)
		{
			...
			foreach($data as $row)
			{
				...
				foreach($row as $col)
				{
					...
					if($x==1){$col = substr(utf8_decode($col),0,40);}
					if($x==2){$col = substr(utf8_decode($col),0,33);}
