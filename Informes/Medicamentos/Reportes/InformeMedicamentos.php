<?
	...
	if($CedPaciente){$OpcPac="and salud.registromedicamentos.cedula='$CedPaciente'";}
	$cons="Select entidad, contrato, nocontrato, Salud.registromedicamentos.cedula, Salud.registromedicamentos.fechacre, hora, Salud.registromedicamentos.autoid,consumo.codproductos.nombreprod1, 
cantidad, Salud.registromedicamentos.usuariocre,consumo.codproductos.almacenppal
from salud.pagadorxservicios
INNER JOIN salud.servicios ON Salud.servicios.numservicio=salud.pagadorxservicios.numservicio
INNER JOIN Salud.registromedicamentos ON Salud.registromedicamentos.numservicio=salud.pagadorxservicios.numservicio 
INNER JOIN consumo.codproductos ON Salud.registromedicamentos.autoid=consumo.codproductos.autoid 

where Salud.registromedicamentos.Fechacre>='$Anio-$MesIni-$DiaIni 00:00:00' 
	and Salud.registromedicamentos.Fechacre<='$Anio-$MesFin-$DiaFin 23:59:59' $OpcionEnt $OpcionCont $OpcionNumCont $OpcPac
	and consumo.codproductos.almacenppal in ('FARMACIA', 'STOCK') and consumo.codproductos.anio='2012'
	and Salud.registromedicamentos.almacenppal=consumo.codproductos.almacenppal and Salud.servicios.estado='AC'
	order by Entidad,Contrato,NoContrato,Salud.registromedicamentos.cedula,autoid,Fechacre,Hora";
	$res=ExQuery($cons);
	while($fila=ExFetch($res))
	{
