	...
	while($fila=ExFetch($res))
	{
		...
		{*/			
			...
			while($filax=ExFetch($resx))
			{
				...
				{*/
					//echo $filax[0]."<br>";
					if($GruposMeds[$filax[0]]){$filax[0]=$MatGrupos[$GruposMeds[$filax[0]]][0];} 
					//echo $filax[0]."<br><br>";
					if($MatGrupos[$filax[0]])
					{
						$MatGrupos[$filax[0]][2]=1;
...
if($MatFacturas)
{
    ...
    <br>
	<table border='1' bordercolor='#e5e5e5' style='font : normal normal small-caps 8px Tahoma;' width='100%'>
    <tr bgcolor='#e5e5e5' style='font-weight:bold' align='center'>
    <td>Entidades/Servicios</td>
    <?
	$rowsp=0;	
    foreach($MatGrupos as $Gr)
	{	
		if($Gr[2])
		{
			$rowsp++;
			?>
			<td align="center"><? echo $Gr[1]?></td>	        
			<?	
		}
    }?>
	...
	{				
		if($Grup[2])
		{
			?><td align="right"><? echo number_format($Grup[3],0);?></td><?			
		}
	}   
	?>
	<td align="right"><? echo number_format($TotCop,0);?></td>
	<td align="right"><? echo number_format($TotDes,0);?></td>
