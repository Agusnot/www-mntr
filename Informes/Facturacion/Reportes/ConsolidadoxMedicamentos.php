...
<!--
function Impresion(){
	window.open("ImprimirMedicamentos.php?DatNameSID=<? echo $DatNameSID?>&FechaIni="+document.FORMA.FechaIni.value+"&FechaFin="+document.FORMA.FechaFin.value+"&pagador="+document.FORMA.pagador.value+"&tipousu="+document.FORMA.tipousu.value+"&ambito="+document.FORMA.ambito.value+"",null,"_blank","location=1,status=1,scrollbars=1, width=100,height=100");
}
function Valida()
{
...
<div align="center">
  <form id="FORMA" name="FORMA" method="post" action="">
    ...
    <p><strong><? if($ambito!="0"){echo"Consolidado por Medicamentos";} ?></strong><br /><br />
Periodo Comprendido entre el 
    ...
    <table border="0" cellpadding="5">
      <tr>
        ...
        <td width="10" nowrap="nowrap" bgcolor="#CCCCCC">&nbsp;</td>
        <td nowrap="nowrap" bgcolor="#CCCCCC">SUBTOTAL</td>
        <td width="10" nowrap="nowrap" bgcolor="#CCCCCC">&nbsp;</td>
        <td nowrap="nowrap" bgcolor="#CCCCCC">VALOR COPAGO </td>
        <td width="10" nowrap="nowrap" bgcolor="#CCCCCC">&nbsp;</td>
        <td nowrap="nowrap" bgcolor="#CCCCCC">VALOR DESCUENTO </td>
        <td width="10" nowrap="nowrap" bgcolor="#CCCCCC">&nbsp;</td>
        <td nowrap="nowrap" bgcolor="#CCCCCC">TOTAL MEDICAMENTOS </td>
        <td width="10" nowrap="nowrap" bgcolor="#CCCCCC">&nbsp;</td>
        <td nowrap="nowrap" bgcolor="#CCCCCC">TOTAL FACTURA </td>
      </tr>
	  <?php 
	   $i=1;
	   $c1=0;
	   ...
	   $c4=0;
	   $c5=0;
	  $cons4="select nofactura,cedula,fechaini,fechafin,ambito,tipousu,pagador,valorcopago,valordescuento from facturacion.liquidacion where fechafin between '$FechaIni' and '$FechaFin' and nofactura is not null $qp $qt $qa
	  group by nofactura,cedula,fechaini,fechafin,ambito,tipousu,pagador,valorcopago,valordescuento
	  order by nofactura";
	  //echo $cons4;
	  $res4=ExQuery($cons4);
	  while($fila4=ExFetch($res4)){
		...
		while($fila5=ExFetch($res5)){
			$cons6="select sum(vrtotal) from facturacion.detallefactura where nofactura='$fila4[0]' and tipo like '%Medicamentos%'";
			//echo $cons6;
			$res6=ExQuery($cons6);
			//echo $pagador;
			while($fila6=ExFetch($res6)){
			$cons9="select subtotal,copago,descuento,total from facturacion.facturascredito where nofactura='$fila4[0]'";
			$res9=ExQuery($cons9);
      ...
      <tr>
        ...
        <td width="10" nowrap="nowrap">&nbsp;</td>
        <td nowrap="nowrap">$<?php echo number_format($fila9[0],2);$c1=$c1+$fila9[0]; ?></td>
        <td width="10" nowrap="nowrap">&nbsp;</td>
        <td nowrap="nowrap">$<?php echo number_format($fila9[1],2);$c2=$c2+$fila9[1];?></td>
        <td width="10" nowrap="nowrap">&nbsp;</td>
        <td nowrap="nowrap">$<?php echo number_format($fila9[2],2);$c3=$c3+$fila9[2] ?></td>
        <td width="10" nowrap="nowrap">&nbsp;</td>
        <td nowrap="nowrap" bgcolor="ffcccc"><strong>$<?php echo number_format($fila6[0],2);$c4=$c4+$fila6[0]; ?></strong></td>
        <td width="10" nowrap="nowrap">&nbsp;</td>
        <td nowrap="nowrap">$<?php echo number_format($fila9[3],2);$c5=$c5+$fila9[3]; ?></td>
      </tr>
	  <?php $i++;}}}}}?>
	  ...
	  <td nowrap="nowrap"><div align="center"><strong>$<?php echo number_format($c5,2) ?></strong></div></td><td>&nbsp;</td>
