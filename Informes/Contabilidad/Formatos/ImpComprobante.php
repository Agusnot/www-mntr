<?
	...
	from Contabilidad.Movimiento where Comprobante='$Comprobante' and Numero='$Numero' and Compania='$Compania[0]' Order By Debe Desc";
	$res=ExQuery($cons);
...
<?
	if($fila["Estado"]=="AN")
	{
		echo "<img src='/Imgs/Anulada.gif' style='position:absolute;top:170px;left:140px'>";
	}
...
<tr bgcolor="#e5e5e5" style="font-weight:bold"><td>Codigo</td><td>Debito</td><td>Credito</td><td>Tercero</td><td>CC</td><td>Doc</td><td>Detalle</td></tr>
<?
	$res=ExQuery($cons);
	while($fila=ExFetch($res))
	{
		echo "<tr><td>$fila[6]</td><td align='right'>".number_format($fila[7],2)."</td><td align='right'>".number_format($fila[8],2)."</td><td>$fila[4]</td><td>$fila[9]</td><td>$fila[10]</td><td>$fila[11]</td></tr>";
