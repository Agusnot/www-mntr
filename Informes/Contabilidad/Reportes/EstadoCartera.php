	...
	<html>
		...
		<body <?php echo $backgroundBodyInfContableMentor;?>>
			...
			?>
						<table class="tablaInformeContable" width="90%" style="margin-top:25px;"  <?php echo $borderTablaInfContable; echo $bordercolorTablaInfContable; echo $cellspacingTablaInfContable; echo $cellpaddingTablaInfContable; ?> >
							<tr>
								<td class='encabezado2HorizontalInfCont' >CONCEPTO</td>
								<?
								if($MostrarDocs){
									echo "<td class='encabezado2HorizontalInfCont'>DOCUMENTO</td>";
									echo "<td class='encabezado2HorizontalInfCont'>FECHA DOCUMENTO</td>";
									echo "<td class='encabezado2HorizontalInfCont'>FECHA DE VENCIMIENTO</td>";
								}
							...
							$cons2="Select sum(Debe) as Suma,DocSoporte,FechaDocumento,Cuenta,Identificacion,Fecha from Contabilidad.Movimiento where 
							Estado='AC' and Movimiento.Compania='$Compania[0]' $condAdc 
							and Movimiento.Cuenta>='$CuentaIni' and Movimiento.Cuenta<='$CuentaFin'
							and Fecha<='$Corte'

							Group By DocSoporte,FechaDocumento,Cuenta,Identificacion,Fecha having sum(Debe)>0 order by Cuenta asc";
							$res2=ExQuery($cons2);
							...
							and Movimiento.Cuenta>='$CuentaIni' and Movimiento.Cuenta<='$CuentaFin'
							and Fecha<='$Corte'
							Group By DocSoporte,Fecha,Cuenta,Identificacion having sum(Haber)>0 order by Cuenta asc";
						...
						if($MostrarDocs=="on"){
							...
							$cons2="Select sum(Debe) as Suma,DocSoporte,FechaDocumento,Cuenta,Identificacion,Fecha,comprobante from Contabilidad.Movimiento where 
							Estado='AC' and Movimiento.Compania='$Compania[0]' $condAdc 
							and Movimiento.Cuenta>='$CuentaIni' and Movimiento.Cuenta<='$CuentaFin'
							and Fecha<='$Corte'
							Group By DocSoporte,FechaDocumento,Cuenta,Identificacion,Fecha,comprobante having sum(Debe)>0 Order By Cuenta asc";
						...
						//		echo "$fila2[3]-->$fila2[4]--->$Periodo--->$fila2[5]--->$fila2[2]--->$fila2[0]<br>";
							...
							Group By Movimiento.Cuenta,Nombre order by Cuenta asc";
							$res=ExQuery($cons);
							while($fila=ExFetch($res))	{

								echo "<tr>";
									echo "<td  class='encabezado2HorizontalInfContInv' colspan=11>$fila[0] $fila[1]</td>";
								echo "</tr>";
							...
							echo "<tr>";
								echo "<td class='filaTotalesInfContable'  colspan='$ColsPan2'>TOTALES</td>";
							for($i=0;$i<=count($Dias)-1;$i++)	{
