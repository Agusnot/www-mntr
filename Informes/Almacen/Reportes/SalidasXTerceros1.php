...
<?	}
	...
	and Estado = 'AC' and Fecha >= '$FechaIni' and Fecha <= '$FechaFin' $conCedula $conAutoId
	...
	while($fila = ExFetch($res))
	{
		...
		<tr style="font-weight:bold" bgcolor="#e5e5e5">
        	<td>Fecha</td><td>Codigo</td><td>Nombre</td>
		...
		Fecha,Numero,Cantidad,VrCosto+Movimiento.VrIVA,TotCosto,Comprobante 
		...
		and Cedula = '$fila[0]' $conAutoId  order by Fecha,TipoComprobante asc";
		//echo $cons0;
		$res0 = ExQuery($cons0);
		...
		while($fila0 = ExFetch($res0))
		{
			...
			$cons9="Select sum(Cantidad) from Consumo.Movimiento where TipoComprobante='Devoluciones' and Fecha>='$FechaIni' and Fecha<='$FechaFin' and AlmacenPpal='$AlmacenPpal'
