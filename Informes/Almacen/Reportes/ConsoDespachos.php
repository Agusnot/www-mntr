        ...
        <tr bgcolor="#e5e5e5">
            ...
            <td><select name="Verx">
                    <option
                        ...
                        ?>
                        value="paciente">Paciente x Servicio</option>
                    <option
                        <?
                        if($Verx=="medicamento"){echo " selected ";}
                        ?>
                        value="medicamento">Medicamento x Servicio</option>
                    <option
                        <?
...
<?
    ...
    if($Verx=="paciente")$Orderby="order by Cedula,Movimiento.AutoId";
    ...
    and TipoComprobante = 'Salidas' and fechadespacho <='$Fecha' and
    ...
    and fechadespacho >= '$FechaI' and fechadespacho <='$FechaF'
    and Movimiento.Estado = 'AC' $Adcons $Orderby";//order by cedula ASC
    $res = ExQuery($cons);
    while($fila = ExFetch($res))
    {
    ...
    if($LTerceros && $LAutoId)
    {
        ...
        and tipo = 'P' and Estado = 'AC' and Paciente in ($LTerceros) and AutoId in ($LAutoId) --order by hora asc";
        $res = ExQuery($cons);
        while($fila = ExFetch($res))
        {
        ...
        <table style='font : normal normal small-caps 12px Tahoma;' border="1" width="100%" bordercolor="#e5e5e5">
            ...
            while(list($Pabellonw,$MxP1) = each($MatrizMxP))
            {
				...
				while(list($Numero,$MxP2) = each($MxP1))
                {
                    ...
                    while(list($Cedula,$MxP3) = each($MxP2))
                    {
                        ...
                        //$OrdenPacientes[$Cedula]
						echo "$C.$NomPaciente[$Cedula]($Cedula)</td></tr>";
		  ...
		  <tr>
			...
			<table>
			  ...
			  <tr>
				<td style="font-weight: bold; font-size: 15" align="center">
		   ...
		   <td>
			 <table>
			  <tr>
				<td style="font-weight: bold; font-size: 15">
				Número de dosis revisadas: ___
				</td>
			  </tr>
			</table>
		   </td>
    ...
    if($MatrizM)
    {
        ...
        while(list($Pabellonw,$MM) = each($MatrizM))
        {
            ?>
                <tr><td align ="center" style="font-weight: bold; font-size: 15" colspan="7">
                    <? echo "$Compania[0]";?><br>
                    Consolidado total de Medicamentos despachados por Servicio<br>
                    Desde <? echo $FechaI ?> Hasta <? echo $FechaF ?><br>
                    <? echo "Pabellon $Pabellonw"?>
                </td></tr>
