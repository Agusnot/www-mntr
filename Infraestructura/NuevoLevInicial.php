<?
	...
	if(!$Codigo){
		...
		//echo $AutoId." -- ".$Grupo." -- ".$Clase." -- ".$Codigo;exit;
	}	
	if($Guardar)
	{
		...
		if(!$DepDesde){$DepDesde = $FechaAd;}
                ...
                if(!$AxICostoIni){$AxICostoIni = 0;}
                if(!$AxIDepAcumulada){$AxIDepAcumulada = 0;}
		if(!$Editar)
		{
			if($Tipo == "Levantamiento Inicial")
			{
				$InsLevIni = " ,Codigo, FechaAdquisicion, Serie, Estado, DepAcumulada,DepDesde,UVE,UUVE,Consumo,AxICostoIni,AxIDepAcumulada ";
				if($Consumo=="on"){$Codigo="";$Consumo=1;}
				else{$Consumo=0;}
				$ValsLevIni = " ,'$Codigo', '$FechaAd', '$Serie', '$Estado', $DepAcumulada, '$DepDesde',
				'$ND[year]-$ND[mon]-$ND[mday] $ND[hours]:$ND[minutes]:$ND[seconds]','$usuario[0]',$Consumo, $AxICostoIni, $AxIDepAcumulada ";
			}
		...
		else
		{
			...
			if($Tipo == "Compras" || $Tipo == "Levantamiento Inicial")
			{
				...
				if($Tipo == "Levantamiento Inicial"){ $UptLevIniCompras = $UptLevIniCompras." ,DepAcumulada=$DepAcumulada, AxICostoIni=$AxICostoIni,
                                    AxIDepAcumulada=$AxIDepAcumulada";}
			}
...
<?
	if($Editar)
	{
		...
		IncluyeIva,DepAcumulada, DepDesde $CadAd, Consumo,AxICostoIni, AxIDepAcumulada
		...
		if($Tipo=="Levantamiento Inicial"){$ChkConsumo = $fila[20]; $AxICostoIni = $fila[21]; $AxIDepAcumulada = $fila[22];}
...
<table style='font : normal normal small-caps 12px Tahoma;' border="1" bordercolor="#e5e5e5">
    ...
    <tr>
        ...
        <? if($Tipo=="Levantamiento Inicial")
        {?>
        <td bgcolor="#e5e5e5" style="font-weight:bold">Ajustes x Inf</td>
        <td><input type="text" name="AxICostoIni" title="Ajustes por Inflacion Costo Inicial" value="<? echo $AxICostoIni;?>" size="8" <? echo $Ocultar;?>
        style="text-align:right;" onKeyUp="xNumero(this);"
        onKeyDown="xNumero(this);" onBlur="campoNumero(this);" />
        </td>
        <?}
    ...
    <?if($Tipo != "Orden Compra")
    {
        ...
        <? if($Tipo == "Levantamiento Inicial")
        {
            ...
            <td align="right">Ajustes x Inf</td>
            <td><input type="text" name="AxIDepAcumulada" title="Ajuestes por Inflacion Dep. Acumulada" value="<? echo $AxIDepAcumulada?>" onKeyUp="xNumero(this)"
                onKeyDown="xNumero(this)" onBlur="campoNumero(this)" size="8" style="text-align:right" <? if($ChkConsumo){echo " disabled ";}?> /></td>
            <?
        }?>
        <td colspan="2" align="right">Depreciar desde</td><td><input type="text" name="DepDesde" size="8" value="<? echo $DepDesde;?>"
