<?
	...
	<?		//exit;
			...
			values ('$Cedula','$AutoId','$Ambito','$FechaIng','$Fecha','$Tipousu','$Nivelusu','$Autorizac1','$Autorizac2','$Autorizac3','AC','$Nocarnet','$Compania[0]','$Medico','$usuario[1]','ActivarCitaAgend.php','$usuario[1]')";
				...
				(compania,usuario,fechacrea,grupo,tipo,codigo,nombre,cantidad,vrunidad,vrtotal,noliquidacion									,fechainterpret,finalidad,causaext,dxppal,dxrel1,dxrel2,dxrel3,dxrel4,tipodxppal,ambito,nofacturable) 
				values ('$Compania[0]','$usuario[1]','$ND[year]-$ND[mon]-$ND[mday] $ND[hours]:$ND[minutes]:$ND[seconds]','$fila3[0]','$fila3[1]',									'$CUP','$fila3[2]',1,$filaVr[0],$filaVr[0],$AutoIdLiq,'$ND[year]-$ND[mon]-$ND[mday] $ND[hours]:$ND[minutes]:$ND[seconds]','','','','','','','','',1,$NoFacturable)";	
				//echo "<br>\n$consF<br>\n";		
				$res = ExQuery($cons);
...
if($TipoFac=="Individual")
{
	$GeneraFactura="on"; ?>
    <tr>      
		<td colspan="4" style=" font:bold" align="center">Generar Factura <input type="checkbox" name="GeneraFactura" <? if($GeneraFactura){ /* <!-- checked modificaciòn cali--> */?>  <? }?>></td>
