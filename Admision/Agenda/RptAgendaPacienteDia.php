...
<tr><td colspan="11"><center><strong><? echo strtoupper($Compania[0])?><br>
		<? echo $Compania[1]?><br>PACIENTES AGENDADOS POR DIA<br></td></tr>
	<tr>
...
<?	
$cons4="Select hrsini,minsini,hrsfin,minsfin,cedula,primape,segape,primnom,segnom,telefono,entidad,estado,medico from central.terceros,salud.agenda where
terceros.identificacion=agenda.cedula  and fecha='$Fecha' and estado='Pendiente' and agenda.compania='$Compania[0]' and terceros.compania='$Compania[0]' order by primape,segape,primnom,segnom";
	$res4=ExQuery($cons4);echo ExError();
if(ExNumRows($res4)>0){?> 			
	<tr bgcolor="#e5e5e5" style="font-weight:bold" align="center">
		<td>Hora</td><td>Cedula</td><td>Nombre<td>Telefono</td><td>Entidad</td><td>Estado</td><td>Especialidad</td><td>Medico</td>
	</tr>
 <? while($fila4 = ExFetchArray($res4)){ 
 		$cons="select nombre from central.usuarios where usuario='$fila4[12]'";
		$res=ExQuery($cons);
		$fila=ExFetch($res);
		$cons2="select especialidad from salud.medicos where usuario='$fila4[12]' and compania='$Compania[0]'";
		$res2=ExQuery($cons2);
		$fila2=ExFetch($res2);
     ...
     <? if($fila4[3]==0){$cero1='0';}else{$cero1='';}
		...
		echo "<td>$fila4[0]:$fila4[1]$cero-$fila4[2]:$fila4[3]$cero1</td><td>$fila4[4]</td><td>$fila4[5] $fila4[6] $fila4[7] $fila4[8]</td><td>$fila4[9]</td><td>$fila3[0]</td><td>$fila4[11]</td><td>$fila2[0]</td><td>$fila[0]</td><tr>";  }		
}
else{
	echo "<tr><td bgcolor='#e5e5e5' style='font-weight:bold' align='center' colspan='11'>No hay citas asignadas para este dia</td></tr>";
}?>
</table>
