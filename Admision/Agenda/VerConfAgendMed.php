...
function comprobarAsignacion(H,M,link,motivo){
        var x;
        var r=confirm("Esta fecha se encuentra restringida por: "+motivo+"\n\n¿Desea asignar una consulta de todas maneras?.");
        if(r==true){
            //x="Presionaste Aceptar!";
            if(ValidarAsignacion(H,M)){
                parent.location.href=link;
            }
            else{
                alert('Hora o Fecha de Asignacion no valida!!!');
            }
        }
        else{
            //x="Presionaste Cancelar!";
        }
        //alert(x);
    }
					...
					else
					{
						$consBlock = "select * from salud.bloqconsexterna where compania_bloqconsexterna='$Compania[0]' and medico_bloqconsexterna='$Profecional' and fechaini_bloqconsexterna<='$Fecha $HI:$cero$MI:00' and fechafin_bloqconsexterna>='$Fecha $HI:$cero$MI:00'";
                                                ...
                                                <?php
                                                    ...
                                                    if($MF<$TiempInt){$cero2='0';}else{$cero2='';}?>	
                                                    <tr title="No se puede asignar" style="cursor:hand" onMouseOver="this.bgColor='#AA3333'" onMouseOut="this.bgColor=''" align="center" onclick="comprobarAsignacion('<? echo $HI?>','<? echo $MI?>','NewConfAgendMed.php?DatNameSID=<? echo $DatNameSID?>&Especialidad=<? echo $Especialidad?>&Horario=<? echo $fila[4]?>&Fecha=<? echo $Fecha?>&Profecional=<? echo $Profecional?>&HrIni=<? echo $HI?>&MinIni=<? echo $MI?>&Tiempo=<? echo $tim?>&HrFin=<? echo $fila[2]?>&MinFin=<? echo $fila[3]?>&CupBloq=<? echo $fila[5]?>','<?php echo $filaBlock['motivo_bloqconsexterna'];?>')">
                                                <?php
