<?
	...
	if($Guardar){
		if(!$Edit){
			$cons2="select codigo from facturacion.codmotivoglosa 
			where compania='$Compania[0]' and codigo='$ClaseGlosa'";
			$res2=ExQuery($cons2);
			//echo $cons2;
			if(ExNumRows($res2)<=0){
				$cons="insert into facturacion.codmotivoglosa (codigo,detalle,compania) 
				values ('$ClaseGlosa','$NombreGlosa','$Compania[0]')";	
				$res=ExQuery($cons); echo ExError();
				?><script language="javascript">location.href="CodigoGlosa.php?DatNameSID=<? echo $DatNameSID?>";</script><?		
			}					
			else{
			...
			//echo $cons2;
			if(ExNumRows($res2)<=0){
			$cons="update facturacion.codmotivoglosa  set codigo='$ClaseGlosa',detalle='$NombreGlosa'
...
<table  style='font : normal normal small-caps 12px Tahoma;' border="2" bordercolor="#e5e5e5" cellpadding="2"> 
	...
	<tr align="center" >
    	<td colspan="2"><input type="submit" value="Guardar" name="Guardar" /><input type="button" value="Cancelar" onClick="location.href='CodigoGlosa.php?DatNameSID=<? echo $DatNameSID?>'"/></td>
  	</tr>
</table>
