...
<?php
	...
	/* Inicia la definicion de funciones */
		...
		function contarRegistrosMySQL2011() {
			...
			$cons = "SELECT COUNT(*) AS conteomysql FROM Salud.SalidaMedicamentos2011";
		...
		function contarRegistrosPostgresql2011() {
			...
			$cons = "SELECT COUNT(*) AS conteo FROM Consumo.movimiento WHERE almacenppal = 'FARMACIA' AND UPPER(comprobante) = 'SALIDAS POR PLANTILLA' AND DATE_PART('year', fecha) = '2011'";
		...
		function eliminarSalidasFarmacia2011() {
			$cnx= conectar_postgres();
			$cons = "DELETE  FROM Consumo.movimiento WHERE almacenppal = 'FARMACIA' AND UPPER(Comprobante) = 'SALIDAS POR PLANTILLA' AND DATE_PART('year', fecha) = '2011'";
			$res =  @pg_query($cnx, $cons);
				if (!$res)  {
		...
		function  cambiarcampoCedula_2011()  {
			$cnx= conectar_postgres();
		...
		function consultarCentroCosto_2011($centrocosto) {
			$cnx= conectar_postgres();
		...
		function llamarMovimientoMySQL_2011() {
			// El limite inicial y final se usan para una inmensa cantidad de registros
			global $res;
			$cnx = conectar_mysql("Salud");
			$cons = "SELECT YEAR(FechaDespacho) AS anio, SalidaMedicamentos2011.*  FROM Salud.SalidaMedicamentos2011 WHERE YEAR(FechaDespacho) = '2011' ORDER BY FechaDespacho ASC ";
		...
		function eliminarTablaMigracion_2011() {
		...
		function crearTablaMigracion_2011() {
  ...
  compcontable character varying(50),  numcompcont character varying(100),  conceptortefte character varying(200),  regmedicamento double precision,  noliquidacion integer,  numservicio integer,  grupo character varying(150),  vobo integer,  usuariovobo character varying(200),  fechavobo timestamp with time zone,  notanovisado character varying(500),  numorden integer,  idescritura integer,  numerocontrolados double precision,  idregistro serial ,  cum character varying(1500),  motivodev character varying(1000),  fechadespacho date,  motivodevolucion text,  lote character(150),  error text )WITH (  OIDS=FALSE)";
		...
		function insertarRegistroMigracion_2011($compania,$almacenppal,$fecha,$comprobante,$tipocomprobante,$numero,$cedula,$detalle,$autoid,$usuariocre,$fechacre,$estado,$cantidad,$vrcosto,$totcosto,$vrventa,$totventa,$centrocosto,$idtraslado,$anio,$incluyeiva,$idregistro,$cum,$fechadespacho,$lote, $grupo,  $error) {
		...
		function insertarRegistroPostgresql_2011($compania,$almacenppal,$fecha,$comprobante,$tipocomprobante,$numero,$cedula,$detalle,$autoid,$usuariocre,$fechacre,$estado,$cantidad,$vrcosto,$totcosto,$vrventa,$totventa,$centrocosto,$idtraslado,$anio,$incluyeiva,$idregistro,$cum,$fechadespacho,$lote, $grupo) {
			...
			$res = @pg_query($cnx, $cons);
				if (!$res) {
					...
					$resUTF8 = @pg_query($cnx, $consUTF8);					
						if (!$resUTF8) {
							$error = pg_last_error();
							insertarRegistroMigracion_2011($compania,$almacenppal,$fecha,$comprobante,$tipocomprobante,$numero,$cedula,$detalle,$autoid,$usuariocre,$fechacre,$estado,$cantidad,$vrcosto,$totcosto,$vrventa,$totventa,$centrocosto,$idtraslado,$anio,$incluyeiva,$idregistro,$cum,$fechadespacho,$lote, $grupo,  $error); 
						}
		...
		function  llenarMatriz_2011(){
			
			unset($matriz); 
			global  $matriz;	
			$res = llamarMovimientoMySQL_2011();
			$posicion=0;
			...
			function insertarTabla_2011()  {
			
				global $res,$matriz;
				$centrocosto = consultarCentroCosto_2011("FARMACIA");
					...
					insertarRegistroPostgresql_2011($compania,$almacenppal,$fecha,$comprobante,$tipocomprobante,$numero,$cedula,$detalle,$autoid,$usuariocre,$fechacre,$estado,$cantidad,$vrcosto,$totcosto,$vrventa,$totventa,$centrocosto,$idtraslado,$anio,$incluyeiva,$idregistro,$cum,$fechadespacho,$lote, $grupo) ;
			...
			function migrarSalidasMedicamentos2011() {
