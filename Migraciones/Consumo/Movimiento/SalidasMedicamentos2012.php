...
<?php
	...
	/* Inicia la definicion de funciones */
		...
		function contarRegistrosMySQL2012() {
			...
			$cons = "SELECT COUNT(*) AS conteomysql FROM Salud.SalidaMedicamentos2012";
		...
		function contarRegistrosPostgresql2012() {
			...
			$cons = "SELECT COUNT(*) AS conteo FROM Consumo.movimiento WHERE almacenppal = 'FARMACIA' AND UPPER(comprobante) = 'SALIDAS POR PLANTILLA' AND DATE_PART('year', fecha) = '2012'";
		...
		function eliminarSalidasFarmacia2012() {
			$cnx= conectar_postgres();
			$cons = "DELETE  FROM Consumo.movimiento WHERE almacenppal = 'FARMACIA' AND UPPER(Comprobante) = 'SALIDAS POR PLANTILLA' AND DATE_PART('year', fecha) = '2012'";
			$res =  @pg_query($cnx, $cons);
				if (!$res)  {
		...
		function  reemplazarTextoTabla4_2012($cadenaBusqueda,$cadenaReemplazo)  {
			$cnx= conectar_postgres();
			$cons = "UPDATE Consumo.movimiento SET compania = replace( compania,'$cadenaBusqueda','$cadenaReemplazo'), almacenppal = replace( almacenppal,'$cadenaBusqueda','$cadenaReemplazo'), comprobante = replace( comprobante,'$cadenaBusqueda','$cadenaReemplazo'), tipocomprobante = replace( tipocomprobante,'$cadenaBusqueda','$cadenaReemplazo'), centrocosto = replace( centrocosto,'$cadenaBusqueda','$cadenaReemplazo') WHERE almacenppal = 'FARMACIA' AND UPPER(Comprobante) = 'SALIDAS POR PLANTILLA' AND DATE_PART('year',fecha) = '2012'";
		...
		function  cambiarcampoCedula_2012()  {
			$cnx= conectar_postgres();
		...
		function consultarCentroCosto_2012($centrocosto) {
			$cnx= conectar_postgres();
		...
		function llamarMovimientoMySQL_2012() {
			// El limite inicial y final se usan para una inmensa cantidad de registros
			global $res;
			$cnx = conectar_mysql("Salud");
			$cons = "SELECT YEAR(FechaDespacho) AS anio, SalidaMedicamentos2012.*  FROM Salud.SalidaMedicamentos2012 WHERE YEAR(FechaDespacho) = '2012' ORDER BY FechaDespacho ASC ";
		...
		function eliminarTablaMigracion_2012() {
		...
		function crearTablaMigracion_2012() {
  ...
  compcontable character varying(50),  numcompcont character varying(100),  conceptortefte character varying(200),  regmedicamento double precision,  noliquidacion integer,  numservicio integer,  grupo character varying(150),  vobo integer,  usuariovobo character varying(200),  fechavobo timestamp with time zone,  notanovisado character varying(500),  numorden integer,  idescritura integer,  numerocontrolados double precision,  idregistro serial ,  cum character varying(1500),  motivodev character varying(1000),  fechadespacho date,  motivodevolucion text,  lote character(150),  error text )WITH (  OIDS=FALSE)";
		...
		function insertarRegistroMigracion_2012($compania,$almacenppal,$fecha,$comprobante,$tipocomprobante,$numero,$cedula,$detalle,$autoid,$usuariocre,$fechacre,$estado,$cantidad,$vrcosto,$totcosto,$vrventa,$totventa,$centrocosto,$idtraslado,$anio,$incluyeiva,$idregistro,$cum,$fechadespacho,$lote, $grupo,  $error) {
		...
		function insertarRegistroPostgresql_2012($compania,$almacenppal,$fecha,$comprobante,$tipocomprobante,$numero,$cedula,$detalle,$autoid,$usuariocre,$fechacre,$estado,$cantidad,$vrcosto,$totcosto,$vrventa,$totventa,$centrocosto,$idtraslado,$anio,$incluyeiva,$idregistro,$cum,$fechadespacho,$lote, $grupo) {
			...
			$res = @pg_query($cnx, $cons);
				if (!$res) {
					...
					$resUTF8 = @pg_query($cnx, $consUTF8);					
						if (!$resUTF8) {
							$error = pg_last_error();
							insertarRegistroMigracion_2012($compania,$almacenppal,$fecha,$comprobante,$tipocomprobante,$numero,$cedula,$detalle,$autoid,$usuariocre,$fechacre,$estado,$cantidad,$vrcosto,$totcosto,$vrventa,$totventa,$centrocosto,$idtraslado,$anio,$incluyeiva,$idregistro,$cum,$fechadespacho,$lote, $grupo,  $error); 
						}
		...
		function  llenarMatriz_2012(){
			
			unset($matriz); 
			global  $matriz;	
			$res = llamarMovimientoMySQL_2012();
			$posicion=0;
			...
			function insertarTabla_2012()  {
			
				global $res,$matriz;
				$centrocosto = consultarCentroCosto_2012("FARMACIA");
					...
					insertarRegistroPostgresql_2012($compania,$almacenppal,$fecha,$comprobante,$tipocomprobante,$numero,$cedula,$detalle,$autoid,$usuariocre,$fechacre,$estado,$cantidad,$vrcosto,$totcosto,$vrventa,$totventa,$centrocosto,$idtraslado,$anio,$incluyeiva,$idregistro,$cum,$fechadespacho,$lote, $grupo) ;
			...
			function migrarSalidasMedicamentos2012() {
