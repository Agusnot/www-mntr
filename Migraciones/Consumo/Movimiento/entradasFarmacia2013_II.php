...
<?php
	...
	/* Inicia la definicion de funciones */
		...
		function contarRegistrosMySQL2013_II() {
			$cnx = conectar_mysql("Salud");
			$cons = "SELECT COUNT(*) FROM  Salud.EntradasFarmacia WHERE YEAR(Fecha) <> '2014' AND OrdenCompra NOT IN (SELECT DISTINCT(OrdenCompra) FROM Salud.EntradasFarmacia2013 ) ";
		...
		function contarRegistrosPostgresql2013_II() {
			$cnx= conectar_postgres();
			$cons = "SELECT COUNT(*) AS conteo FROM Consumo.movimiento WHERE almacenppal = 'FARMACIA' AND UPPER(comprobante) = 'ORDEN DE COMPRA FARMACIA' AND DATE_PART('year', fecha) = '2013_II'";
		...
		function contarRegistrosPostgresql2013_IIErrores() {
			$cnx= conectar_postgres();
		...
		function llamarMovimientoMySQL2013_II() {
			// El limite inicial y final se usan para una inmensa cantidad de registros
			global $res;
			$cnx = conectar_mysql("Salud");
			$cons = "SELECT YEAR(Fecha) AS anio, entradasFarmacia.* FROM  Salud.EntradasFarmacia WHERE YEAR(Fecha) <> '2014' AND OrdenCompra NOT IN (SELECT DISTINCT(OrdenCompra) FROM Salud.EntradasFarmacia2013 )";
			$res =  mysql_query($cons);
			return $res; 
		...
		function  reemplazarTextoTabla1_2013_II($cadenaBusqueda,$cadenaReemplazo)  {
			$cnx= conectar_postgres();
		...
		function  reemplazarTextoTabla2_2013_II($cadenaBusqueda,$cadenaReemplazo)  {
			$cnx= conectar_postgres();
		...
		function  reemplazarTextoTabla3_2013_II($cadenaBusqueda,$cadenaReemplazo)  {
			$cnx= conectar_postgres();
		...
		function  reemplazarTextoTabla4_2013_II($cadenaBusqueda,$cadenaReemplazo)  {
			$cnx= conectar_postgres();
			$cons = "UPDATE Consumo.movimiento SET compania = replace( compania,'$cadenaBusqueda','$cadenaReemplazo'), almacenppal = replace( almacenppal,'$cadenaBusqueda','$cadenaReemplazo'), comprobante = replace( comprobante,'$cadenaBusqueda','$cadenaReemplazo'), tipocomprobante = replace( tipocomprobante,'$cadenaBusqueda','$cadenaReemplazo'), centrocosto = replace( centrocosto,'$cadenaBusqueda','$cadenaReemplazo')  WHERE almacenppal = 'FARMACIA' AND UPPER(Comprobante) = 'ORDEN DE COMPRA FARMACIA' AND DATE_PART('year',fecha) = '2013'";
			...
			function  reemplazarTextoTabla5_2013_II($cadenaBusqueda,$cadenaReemplazo)  {
			$cnx= conectar_postgres();
		...
		function consultarCentroCosto2013_II($centrocosto) {
			$cnx= conectar_postgres();
		...
		function eliminarTablaMigracion2013_II() {
		...
		function crearTablaMigracion2013_II() {
  ...
  compcontable character varying(50),  numcompcont character varying(100),  conceptortefte character varying(200),  regmedicamento double precision,  noliquidacion integer,  numservicio integer,  grupo character varying(150),  vobo integer,  usuariovobo character varying(200),  fechavobo timestamp with time zone,  notanovisado character varying(500),  numorden integer,  idescritura integer,  numerocontrolados double precision,  idregistro serial ,  cum character varying(1500),  motivodev character varying(1000),  fechadespacho date,  motivodevolucion text,  lote character(150),  error text )WITH (  OIDS=FALSE)";
		...
		function insertarRegistroMigracion2013_II($compania, $almacenppal, $fecha, $comprobante, $tipocomprobante, $numero, $autoid, $usuariocre, $estado, $cantidad, $usuariocre, $estado,  $cantidad, $vrcosto, $totcosto, $vrventa, $totventa, $porciva, $vriva,  $porcdescto, $vrdescto, $centrocosto, $nofactura, $vrfactura, $anio, $grupo,$cum, $lote, $error) {
		...
		function insertarRegistroPostgresql2013_II($compania, $almacenppal, $fecha, $comprobante, $tipocomprobante, $numero, $autoid, $usuariocre, $estado, $cantidad,  $vrcosto, $totcosto, $vrventa, $totventa, $porciva, $vriva,  $porcdescto, $vrdescto, $centrocosto, $nofactura, $vrfactura, $anio, $grupo, $cum, $lote ) {
			...
			$res = @pg_query($cnx, $cons);
				if (!$res) {
					...
					$resUTF8 = @pg_query($cnx, $consUTF8);					
						if (!$resUTF8) {
							$error = pg_last_error();
							insertarRegistroMigracion2013_II($compania, $almacenppal, $fecha, $comprobante, $tipocomprobante, $numero, $autoid, $usuariocre, $estado, $cantidad, $usuariocre, $estado,  $cantidad, $vrcosto, $totcosto, $vrventa, $totventa, $porciva, $vriva,  $porcdescto, $vrdescto, $centrocosto, $nofactura, $vrfactura, $anio, $grupo, $cum, $lote,  $error) ;
						}
		...
		function  llenarMatriz2013_II(){
			
			unset($matriz); 
			global  $matriz;	
			$res = llamarMovimientoMySQL2013_II();
			$posicion=0;
			...
			function insertarTabla2013_II()  {
			
				global $res,$matriz;
				$numeroMaximo = consultarMaximoLote();
				$centrocosto = consultarCentroCosto2013_II("FARMACIA");
					...
					$centrocosto= consultarCentroCosto2013_II($centrocosto);
		...
		function migrarEntradasFarmacia2013_II(){
			
			
			
			
			//eliminarTmpMovimiento(1);
			//eliminarTablaMigracion2013_II();
			//crearTablaMigracion2013_II();
			//crearArchivo2013_II();
			//cambiarcampo2013_II();
			
			// Tabla Consumo.Comprobantes 
			reemplazarTextoTabla1_2013_II(utf8_encode("Á"),'&Aacute;');			
			reemplazarTextoTabla1_2013_II(utf8_encode("É"),'&Eacute;');
			reemplazarTextoTabla1_2013_II(utf8_encode("Í"),'&Iacute;');
			reemplazarTextoTabla1_2013_II(utf8_encode("Ó"),'&Oacute;');
			reemplazarTextoTabla1_2013_II(utf8_encode("Ú"),'&Uacute;');
			reemplazarTextoTabla1_2013_II(utf8_encode("Ñ"),'&Ntilde;');
			
			//* Tabla Consumo.TiposComprobante 
			reemplazarTextoTabla2_2013_II(utf8_encode("Á"),'&Aacute;');			
			reemplazarTextoTabla2_2013_II(utf8_encode("É"),'&Eacute;');
			reemplazarTextoTabla2_2013_II(utf8_encode("Í"),'&Iacute;');
			reemplazarTextoTabla2_2013_II(utf8_encode("Ó"),'&Oacute;');
			reemplazarTextoTabla2_2013_II(utf8_encode("Ú"),'&Uacute;');
			reemplazarTextoTabla2_2013_II(utf8_encode("Ñ"),'&Ntilde;');
			
			// Tabla Consumo.Grupos 
			reemplazarTextoTabla3_2013_II(utf8_encode("Á"),'&Aacute;');			
			reemplazarTextoTabla3_2013_II(utf8_encode("É"),'&Eacute;');
			reemplazarTextoTabla3_2013_II(utf8_encode("Í"),'&Iacute;');
			reemplazarTextoTabla3_2013_II(utf8_encode("Ó"),'&Oacute;');
			reemplazarTextoTabla3_2013_II(utf8_encode("Ú"),'&Uacute;');
			reemplazarTextoTabla3_2013_II(utf8_encode("Ñ"),'&Ntilde;');
			
			// Tabla Central.CentrosCosto 
			reemplazarTextoTabla5_2013_II(utf8_encode("Á"),'&Aacute;');			
			reemplazarTextoTabla5_2013_II(utf8_encode("É"),'&Eacute;');
			reemplazarTextoTabla5_2013_II(utf8_encode("Í"),'&Iacute;');
			reemplazarTextoTabla5_2013_II(utf8_encode("Ó"),'&Oacute;');
			reemplazarTextoTabla5_2013_II(utf8_encode("Ú"),'&Uacute;');
			reemplazarTextoTabla5_2013_II(utf8_encode("Ñ"),'&Ntilde;');
			
					
			llenarMatriz2013_II();
			insertarTabla2013_II();
					   
			   
					   
			/* Tabla Consumo.Comprobantes */
			reemplazarTextoTabla1_2013_II('&Aacute;',utf8_encode("Á"));			
			reemplazarTextoTabla1_2013_II('&Eacute;', utf8_encode("É"));
			reemplazarTextoTabla1_2013_II('&Iacute;', utf8_encode("Í"));
			reemplazarTextoTabla1_2013_II('&Oacute;' , utf8_encode("Ó"));
			reemplazarTextoTabla1_2013_II('&Uacute;' , utf8_encode("Ú"));
			reemplazarTextoTabla1_2013_II('&Ntilde;', utf8_encode("Ñ"));
			
			/* Tabla Consumo.TiposComprobante */
			reemplazarTextoTabla2_2013_II('&Aacute;', utf8_encode("Á"));			
			reemplazarTextoTabla2_2013_II('&Eacute;', utf8_encode("É"));
			reemplazarTextoTabla2_2013_II('&Iacute;', utf8_encode("Í"));
			reemplazarTextoTabla2_2013_II('&Oacute;', utf8_encode("Ó"));
			reemplazarTextoTabla2_2013_II('&Uacute;', utf8_encode("Ú"));
			reemplazarTextoTabla2_2013_II('&Ntilde;', utf8_encode("Ñ"));
			
			/* Tabla Consumo.Grupos */
			reemplazarTextoTabla3_2013_II('&Aacute;', utf8_encode("Á"));			
			reemplazarTextoTabla3_2013_II('&Eacute;', utf8_encode("É"));
			reemplazarTextoTabla3_2013_II('&Iacute;', utf8_encode("Í"));
			reemplazarTextoTabla3_2013_II('&Oacute;', utf8_encode("Ó"));
			reemplazarTextoTabla3_2013_II('&Uacute;', utf8_encode("Ú"));
			reemplazarTextoTabla3_2013_II('&Ntilde;', utf8_encode("Ñ"));
			
			/* Tabla Consumo.Movimiento */
			reemplazarTextoTabla4_2013_II('&Aacute;', utf8_encode("Á"));			
			reemplazarTextoTabla4_2013_II('&Eacute;', utf8_encode("É"));
			reemplazarTextoTabla4_2013_II('&Iacute;', utf8_encode("Í"));
			reemplazarTextoTabla4_2013_II('&Oacute;', utf8_encode("Ó"));
			reemplazarTextoTabla4_2013_II('&Uacute;',utf8_encode("Ú"));
			reemplazarTextoTabla4_2013_II('&Ntilde;',utf8_encode("Ñ"));		
			
			/* Tabla Central.CentrosCosto */   
			
			reemplazarTextoTabla5_2013_II('&Aacute;', utf8_encode("Á"));			
			reemplazarTextoTabla5_2013_II('&Eacute;', utf8_encode("É"));
			reemplazarTextoTabla5_2013_II('&Iacute;', utf8_encode("Í"));
			reemplazarTextoTabla5_2013_II('&Oacute;', utf8_encode("Ó"));
			reemplazarTextoTabla5_2013_II('&Uacute;',utf8_encode("Ú"));
			reemplazarTextoTabla5_2013_II('&Ntilde;',utf8_encode("Ñ"));	
				...
				//generarNota2013_II(1,$nota);	
				
				//$nota = "Modificar el tamaño del campo 'CentroCosto' de la tabla Consumo.Movimiento a Character Varying(20) ";
				//generarNota2013_II(2,$nota);	
				
				/*
				$totalMySQL = contarRegistrosMySQL2013_II();
				$totalPostgresql =  contarRegistrosPostgresql2013_II();
				$totalPostgresqlErrores =  contarRegistrosPostgresql2013_IIErrores();
