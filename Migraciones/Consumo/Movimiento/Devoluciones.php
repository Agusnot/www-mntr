...
<?php
	...
	include_once('Devoluciones2010.php');
	include_once('Devoluciones2011.php');
	include_once('Devoluciones2012.php');
	include_once('Devoluciones2013.php');
	...
	/* Inicia la definicion de funciones */
		...
		function contarRegistrosMySQL() {
			...
			$cons = "SELECT COUNT(*) AS conteomysql FROM Salud.SalidaMedicamentos WHERE YEAR(FechaDespacho) = '$anioActual' AND Devuelto <> 0 ";
		...
		function contarRegistrosPostgresql() {
			...
			$cons = "SELECT COUNT(*) AS conteo FROM Consumo.movimiento WHERE almacenppal = 'FARMACIA' AND UPPER(comprobante) = 'DEVOLUCIONES' AND DATE_PART('year', fecha) = '$anioActual' ";
		...
		function llamarMovimientoMySQL() {
			...
			$anioActual = consultarAnio();
			$cnx = conectar_mysql("Salud");
			$cons = "SELECT YEAR(FechaDespacho) AS anio, SalidaMedicamentos.*  FROM Salud.SalidaMedicamentos  WHERE YEAR(FechaDespacho) = '$anioActual' AND Devuelto <> 0 ORDER BY FechaDespacho ASC ";
		...
		function eliminarDevoluciones() {
			$anioActual = consultarAnio();
			$cnx= conectar_postgres();
			$cons = "DELETE  FROM Consumo.movimiento WHERE almacenppal = 'FARMACIA' AND UPPER(Comprobante) = 'DEVOLUCIONES' AND DATE_PART('year', fecha) = '$anioActual' ";
			$res =  @pg_query($cnx, $cons);
				if (!$res)  {
		...
		function eliminarTablaMigracion() {
			// Elimina la tabla Consumo.movimientoMigracion4
			$cnx= conectar_postgres();
			$cons = "DROP TABLE  IF EXISTS Consumo.movimientoMigracion4";
			$res =  pg_query($cons);
  ...
  compcontable character varying(50),  numcompcont character varying(100),  conceptortefte character varying(200),  regmedicamento double precision,  noliquidacion integer,  numservicio integer,  grupo character varying(150),  vobo integer,  usuariovobo character varying(200),  fechavobo timestamp with time zone,  notanovisado character varying(500),  numorden integer,  idescritura integer,  numerocontrolados double precision,  idregistro serial ,  cum character varying(1500),  motivodev character varying(1000),  fechadespacho date,  motivodevolucion text,  lote character(150),  error text )WITH (  OIDS=FALSE)";
		...
		function crearArchivo() {
			$fp = fopen("ReporteDevoluciones.html", "w+");
		...
		if($_GET['tabla']="Devoluciones") {
		
			echo "<fieldset>";			
			echo "<legend> Migracion tabla MySQL </legend>";
			echo "<span align='left'> <a href='../../index.php?migracion=MIG010' class = 'link1'> Panel de Administracion </a> </span>";
				...
				crearTablaMigracion();
				crearArchivo();
				migrarDevoluciones2010();
				migrarDevoluciones2011();
				migrarDevoluciones2012();
				migrarDevoluciones2013();
				migrarDevoluciones();
				...
				echo "<p class= 'error1'> Total errores generados(Tabla Consumo.movimientoMigracion4):</p>";
				echo  $totalPostgresqlErrores."<br/>";
				
				echo "<p> <a href='ReporteDevoluciones.html' class = 'link1' target='_blank'> Ver Reporte de errores de la migracion </a> </p>";
