...
<?php
	...
	/* Inicia la definicion de funciones */
		...
		function contarRegistrosMySQL2010() {
			...
			$cons = "SELECT COUNT(*) AS conteomysql FROM Salud.EntradasFarmacia2010";
		...
		function contarRegistrosPostgresql2010() {
			...
			$cons = "SELECT COUNT(*) AS conteo FROM Consumo.movimiento WHERE almacenppal = 'FARMACIA' AND UPPER(comprobante) = 'ORDEN DE COMPRA FARMACIA' AND DATE_PART('year', fecha) = '2010'";
		...
		function eliminarEntradasFarmacia2010() {
			$cnx= conectar_postgres();
			$cons = "DELETE  FROM Consumo.movimiento WHERE almacenppal = 'FARMACIA' AND UPPER(Comprobante) = 'ORDEN DE COMPRA FARMACIA' AND DATE_PART('year', fecha) = '2010'";
			$res =  @pg_query($cnx, $cons);
				if (!$res)  {
		...
		function  reemplazarTextoTabla4_2010($cadenaBusqueda,$cadenaReemplazo)  {
			$cnx= conectar_postgres();
			$cons = "UPDATE Consumo.movimiento SET compania = replace( compania,'$cadenaBusqueda','$cadenaReemplazo'), almacenppal = replace( almacenppal,'$cadenaBusqueda','$cadenaReemplazo'), comprobante = replace( comprobante,'$cadenaBusqueda','$cadenaReemplazo'), tipocomprobante = replace( tipocomprobante,'$cadenaBusqueda','$cadenaReemplazo'), centrocosto = replace( centrocosto,'$cadenaBusqueda','$cadenaReemplazo')  WHERE almacenppal = 'FARMACIA' AND UPPER(Comprobante) = 'ORDEN DE COMPRA FARMACIA' AND DATE_PART('year',fecha) = '2010'";
		...
		function consultarCentroCosto2010($centrocosto) {
			$cnx= conectar_postgres();
		...
		function llamarMovimientoMySQL2010() {
			// El limite inicial y final se usan para una inmensa cantidad de registros
			global $res;
			$cnx = conectar_mysql("Salud");
			$cons = "SELECT YEAR(Fecha) AS anio, EntradasFarmacia2010.*  FROM Salud.EntradasFarmacia2010 ORDER BY Fecha ASC, OrdenCompra ASC";
			$res =  mysql_query($cons);
			return $res; 
		
		}
		
		function eliminarTablaMigracion2010() {
		...
		function crearTablaMigracion2010() {
  ...
  compcontable character varying(50),  numcompcont character varying(100),  conceptortefte character varying(200),  regmedicamento double precision,  noliquidacion integer,  numservicio integer,  grupo character varying(150),  vobo integer,  usuariovobo character varying(200),  fechavobo timestamp with time zone,  notanovisado character varying(500),  numorden integer,  idescritura integer,  numerocontrolados double precision,  idregistro serial ,  cum character varying(1500),  motivodev character varying(1000),  fechadespacho date,  motivodevolucion text,  lote character(150),  error text )WITH (  OIDS=FALSE)";
		...
		function insertarRegistroMigracion2010($compania, $almacenppal, $fecha, $comprobante, $tipocomprobante, $numero, $autoid, $usuariocre, $estado, $cantidad, $usuariocre, $estado,  $cantidad, $vrcosto, $totcosto, $vrventa, $totventa, $porciva, $vriva,  $porcdescto, $vrdescto, $centrocosto, $nofactura, $vrfactura, $anio, $grupo,$cum, $lote, $error) {
		...
		function insertarRegistroPostgresql2010($compania, $almacenppal, $fecha, $comprobante, $tipocomprobante, $numero, $autoid, $usuariocre, $estado, $cantidad,  $vrcosto, $totcosto, $vrventa, $totventa, $porciva, $vriva,  $porcdescto, $vrdescto, $centrocosto, $nofactura, $vrfactura, $anio, $grupo, $cum, $lote ) {
			...
			$res = @pg_query($cnx, $cons);
				if (!$res) {
					...
					$resUTF8 = @pg_query($cnx, $consUTF8);					
						if (!$resUTF8) {
							$error = pg_last_error();
							insertarRegistroMigracion2010($compania, $almacenppal, $fecha, $comprobante, $tipocomprobante, $numero, $autoid, $usuariocre, $estado, $cantidad, $usuariocre, $estado,  $cantidad, $vrcosto, $totcosto, $vrventa, $totventa, $porciva, $vriva,  $porcdescto, $vrdescto, $centrocosto, $nofactura, $vrfactura, $anio, $grupo, $cum, $lote,  $error) ;
						}
		...
		function  llenarMatriz2010(){
			
			unset($matriz); 
			global  $matriz;	
			$res = llamarMovimientoMySQL2010();
			$posicion=0;
			...
			function insertarTabla2010()  {
			
				global $res,$matriz;
				$centrocosto = consultarCentroCosto2010("FARMACIA");
				$numeroMaximo = (int)consultarMaximoLote();
					for($pos=0;$pos <= mysql_num_rows($res); $pos++)  {
						...
						$centrocosto= consultarCentroCosto2010($centrocosto);
		...
		function migrarEntradasFarmacia2010(){
			
			
			
			eliminarEntradasFarmacia2010();
			//eliminarTmpMovimiento(1);
			//eliminarTablaMigracion2010();
			//crearTablaMigracion2010();
			//crearArchivo2010();
			//cambiarcampo2010();
			...
			reemplazarTextoTabla5_2010('&Ntilde;',utf8_encode("Ñ"));	
				...
				//generarNota2010(1,$nota);	
				
				//$nota = "Modificar el tamaño del campo 'CentroCosto' de la tabla Consumo.Movimiento a Character Varying(20) ";
				//generarNota2010(2,$nota);	
				
				/*
				$totalMySQL = contarRegistrosMySQL2010();
				$totalPostgresql =  contarRegistrosPostgresql2010();
				$totalPostgresqlErrores =  contarRegistrosPostgresql2010Errores();
