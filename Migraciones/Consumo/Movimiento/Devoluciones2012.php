...
<?php
	...
	/* Inicia la definicion de funciones */
		...
		function contarRegistrosMySQL2012() {
			...
			$cons = "SELECT COUNT(*) AS conteomysql FROM Salud.SalidaMedicamentos2012 WHERE YEAR(FechaDespacho) = '2012' AND Devuelto <> 0 ";
		...
		function contarRegistrosPostgresql2012() {
			...
			$cons = "SELECT COUNT(*) AS conteo FROM Consumo.movimiento WHERE almacenppal = 'FARMACIA' AND UPPER(comprobante) = 'DEVOLUCIONES' AND DATE_PART('year', fecha) = '2012' ";
		...
		function llamarMovimientoMySQL2012() {
			...
			$cnx = conectar_mysql("Salud");
			$cons = "SELECT YEAR(FechaDespacho) AS anio, SalidaMedicamentos2012.*  FROM Salud.SalidaMedicamentos2012  WHERE YEAR(FechaDespacho) = '2012' AND Devuelto <> 0 ORDER BY FechaDespacho ASC ";
		...
		function eliminarDevoluciones2012() {
			
			$cnx= conectar_postgres();
			$cons = "DELETE  FROM Consumo.movimiento WHERE almacenppal = 'FARMACIA' AND UPPER(Comprobante) = 'DEVOLUCIONES' AND DATE_PART('year', fecha) = '2012' ";
			$res =  @pg_query($cnx, $cons);
				if (!$res)  {
		...
		function insertarRegistroMigracion2012($compania,$almacenppal,$fecha,$comprobante,$tipocomprobante,$numero,$cedula,$detalle,$autoid,$usuariocre,$fechacre,$estado,$cantidad,$vrcosto,$totcosto,$vrventa,$totventa,$centrocosto,$idtraslado,$anio,$incluyeiva,$idregistro,$cum,$fechadespacho,$lote, $grupo,  $error) {
		...
		function insertarRegistroPostgresql2012($compania,$almacenppal,$fecha,$comprobante,$tipocomprobante,$numero,$cedula,$detalle,$autoid,$usuariocre,$fechacre,$estado,$cantidad,$vrcosto,$totcosto,$vrventa,$totventa,$centrocosto,$idtraslado,$anio,$incluyeiva,$idregistro,$cum,$fechadespacho,$lote, $grupo) {
			...
			$res = @pg_query($cnx, $cons);
				if (!$res) {
					...
					$resUTF8 = @pg_query($cnx, $consUTF8);					
						if (!$resUTF8) {
							$error = pg_last_error();
							insertarRegistroMigracion2012($compania,$almacenppal,$fecha,$comprobante,$tipocomprobante,$numero,$cedula,$detalle,$autoid,$usuariocre,$fechacre,$estado,$cantidad,$vrcosto,$totcosto,$vrventa,$totventa,$centrocosto,$idtraslado,$anio,$incluyeiva,$idregistro,$cum,$fechadespacho,$lote, $grupo,  $error); 
						}
		...
		function  llenarMatriz2012(){
			...
			function insertarTabla2012()  {
				...
				$centrocosto = consultarCentroCosto2012("FARMACIA");
					...
					insertarRegistroPostgresql2012($compania,$almacenppal,$fecha,$comprobante,$tipocomprobante,$numero,$cedula,$detalle,$autoid,$usuariocre,$fechacre,$estado,$cantidad,$vrcosto,$totcosto,$vrventa,$totventa,$centrocosto,$idtraslado,$anio,$incluyeiva,$idregistro,$cum,$fechadespacho,$lote, $grupo) ;
			...
			function migrarDevoluciones2012(){
				eliminarDevoluciones2012();
