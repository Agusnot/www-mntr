...
<?php
	...
	/* Inicia la definicion de funciones */
		...
		function contarRegistrosMySQL2010() {
			...
			$cons = "SELECT COUNT(*) AS conteomysql FROM Salud.SalidaMedicamentos2010 WHERE YEAR(FechaDespacho) = '2010' AND Devuelto <> 0 ";
		...
		function contarRegistrosPostgresql2010() {
			...
			$cons = "SELECT COUNT(*) AS conteo FROM Consumo.movimiento WHERE almacenppal = 'FARMACIA' AND UPPER(comprobante) = 'DEVOLUCIONES' AND DATE_PART('year', fecha) = '2010' ";
		...
		function llamarMovimientoMySQL2010() {
			...
			$cnx = conectar_mysql("Salud");
			$cons = "SELECT YEAR(FechaDespacho) AS anio, SalidaMedicamentos2010.*  FROM Salud.SalidaMedicamentos2010  WHERE YEAR(FechaDespacho) = '2010' AND Devuelto <> 0 ORDER BY FechaDespacho ASC ";
		...
		function eliminarDevoluciones2010() {
			
			$cnx= conectar_postgres();
			$cons = "DELETE  FROM Consumo.movimiento WHERE almacenppal = 'FARMACIA' AND UPPER(Comprobante) = 'DEVOLUCIONES' AND DATE_PART('year', fecha) = '2010' ";
			$res =  @pg_query($cnx, $cons);
				if (!$res)  {
		...
		function insertarRegistroMigracion2010($compania,$almacenppal,$fecha,$comprobante,$tipocomprobante,$numero,$cedula,$detalle,$autoid,$usuariocre,$fechacre,$estado,$cantidad,$vrcosto,$totcosto,$vrventa,$totventa,$centrocosto,$idtraslado,$anio,$incluyeiva,$idregistro,$cum,$fechadespacho,$lote, $grupo,  $error) {
		...
		function insertarRegistroPostgresql2010($compania,$almacenppal,$fecha,$comprobante,$tipocomprobante,$numero,$cedula,$detalle,$autoid,$usuariocre,$fechacre,$estado,$cantidad,$vrcosto,$totcosto,$vrventa,$totventa,$centrocosto,$idtraslado,$anio,$incluyeiva,$idregistro,$cum,$fechadespacho,$lote, $grupo) {
			...
			$res = @pg_query($cnx, $cons);
				if (!$res) {
					...
					$resUTF8 = @pg_query($cnx, $consUTF8);					
						if (!$resUTF8) {
							$error = pg_last_error();
							insertarRegistroMigracion2010($compania,$almacenppal,$fecha,$comprobante,$tipocomprobante,$numero,$cedula,$detalle,$autoid,$usuariocre,$fechacre,$estado,$cantidad,$vrcosto,$totcosto,$vrventa,$totventa,$centrocosto,$idtraslado,$anio,$incluyeiva,$idregistro,$cum,$fechadespacho,$lote, $grupo,  $error); 
						}
		...
		function  llenarMatriz2010(){
			...
			function insertarTabla2010()  {
				...
				$centrocosto = consultarCentroCosto2010("FARMACIA");
					...
					insertarRegistroPostgresql2010($compania,$almacenppal,$fecha,$comprobante,$tipocomprobante,$numero,$cedula,$detalle,$autoid,$usuariocre,$fechacre,$estado,$cantidad,$vrcosto,$totcosto,$vrventa,$totventa,$centrocosto,$idtraslado,$anio,$incluyeiva,$idregistro,$cum,$fechadespacho,$lote, $grupo) ;
			...
			function migrarDevoluciones2010(){
				eliminarDevoluciones2010();
