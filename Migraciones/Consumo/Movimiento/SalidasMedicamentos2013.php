...
<?php
	...
	/* Inicia la definicion de funciones */
		...
		function contarRegistrosMySQL2013() {
			$cnx = conectar_mysql("Salud");
			$cons = "SELECT COUNT(*) AS conteomysql FROM Salud.SalidaMedicamentos2013";
		...
		function contarRegistrosPostgresql2013() {
			$cnx= conectar_postgres();
			$cons = "SELECT COUNT(*) AS conteo FROM Consumo.movimiento WHERE almacenppal = 'FARMACIA' AND UPPER(comprobante) = 'SALIDAS POR PLANTILLA' AND DATE_PART('year', fecha) = '2013'";
		...
		function contarRegistrosPostgresql2013Errores() {
			$cnx= conectar_postgres();
		...
		function eliminarSalidasFarmacia2013() {
			$cnx= conectar_postgres();
			$cons = "DELETE  FROM Consumo.movimiento WHERE almacenppal = 'FARMACIA' AND UPPER(Comprobante) = 'SALIDAS POR PLANTILLA' AND DATE_PART('year', fecha) = '2013'";
			$res =  @pg_query($cnx, $cons);
				if (!$res)  {
		...
		function  reemplazarTextoTabla1_2013($cadenaBusqueda,$cadenaReemplazo)  {
			$cnx= conectar_postgres();
		...
		function  reemplazarTextoTabla2_2013($cadenaBusqueda,$cadenaReemplazo)  {
			$cnx= conectar_postgres();
		...
		function  reemplazarTextoTabla3_2013($cadenaBusqueda,$cadenaReemplazo)  {
			$cnx= conectar_postgres();
		...
		function  reemplazarTextoTabla4_2013($cadenaBusqueda,$cadenaReemplazo)  {
			$cnx= conectar_postgres();
			$cons = "UPDATE Consumo.movimiento SET compania = replace( compania,'$cadenaBusqueda','$cadenaReemplazo'), almacenppal = replace( almacenppal,'$cadenaBusqueda','$cadenaReemplazo'), comprobante = replace( comprobante,'$cadenaBusqueda','$cadenaReemplazo'), tipocomprobante = replace( tipocomprobante,'$cadenaBusqueda','$cadenaReemplazo'), centrocosto = replace( centrocosto,'$cadenaBusqueda','$cadenaReemplazo') WHERE almacenppal = 'FARMACIA' AND UPPER(Comprobante) = 'SALIDAS POR PLANTILLA' AND DATE_PART('year',fecha) = '2013'";
			...
			function  reemplazarTextoTabla5_2013($cadenaBusqueda,$cadenaReemplazo)  {
			$cnx= conectar_postgres();
		...
		function  cambiarcampoCedula_2013()  {
			$cnx= conectar_postgres();
		...
		function consultarCentroCosto_2013($centrocosto) {
			$cnx= conectar_postgres();
		...
		function llamarMovimientoMySQL_2013() {
			// El limite inicial y final se usan para una inmensa cantidad de registros
			global $res;
			$cnx = conectar_mysql("Salud");
			$cons = "SELECT YEAR(FechaDespacho) AS anio, SalidaMedicamentos2013.*  FROM Salud.SalidaMedicamentos2013 WHERE YEAR(FechaDespacho) = '2013' ORDER BY FechaDespacho ASC ";
		...
		function eliminarTablaMigracion_2013() {
		...
		function crearTablaMigracion_2013() {
  ...
  compcontable character varying(50),  numcompcont character varying(100),  conceptortefte character varying(200),  regmedicamento double precision,  noliquidacion integer,  numservicio integer,  grupo character varying(150),  vobo integer,  usuariovobo character varying(200),  fechavobo timestamp with time zone,  notanovisado character varying(500),  numorden integer,  idescritura integer,  numerocontrolados double precision,  idregistro serial ,  cum character varying(1500),  motivodev character varying(1000),  fechadespacho date,  motivodevolucion text,  lote character(150),  error text )WITH (  OIDS=FALSE)";
		...
		function insertarRegistroMigracion_2013($compania,$almacenppal,$fecha,$comprobante,$tipocomprobante,$numero,$cedula,$detalle,$autoid,$usuariocre,$fechacre,$estado,$cantidad,$vrcosto,$totcosto,$vrventa,$totventa,$centrocosto,$idtraslado,$anio,$incluyeiva,$idregistro,$cum,$fechadespacho,$lote, $grupo,  $error) {
		...
		function insertarRegistroPostgresql_2013($compania,$almacenppal,$fecha,$comprobante,$tipocomprobante,$numero,$cedula,$detalle,$autoid,$usuariocre,$fechacre,$estado,$cantidad,$vrcosto,$totcosto,$vrventa,$totventa,$centrocosto,$idtraslado,$anio,$incluyeiva,$idregistro,$cum,$fechadespacho,$lote, $grupo) {
			...
			$res = @pg_query($cnx, $cons);
				if (!$res) {
					...
					$resUTF8 = @pg_query($cnx, $consUTF8);					
						if (!$resUTF8) {
							$error = pg_last_error();
							insertarRegistroMigracion_2013($compania,$almacenppal,$fecha,$comprobante,$tipocomprobante,$numero,$cedula,$detalle,$autoid,$usuariocre,$fechacre,$estado,$cantidad,$vrcosto,$totcosto,$vrventa,$totventa,$centrocosto,$idtraslado,$anio,$incluyeiva,$idregistro,$cum,$fechadespacho,$lote, $grupo,  $error); 
						}
		...
		function  llenarMatriz_2013(){
			
			unset($matriz); 
			global  $matriz;	
			$res = llamarMovimientoMySQL_2013();
			$posicion=0;
			...
			function insertarTabla_2013()  {
			
				global $res,$matriz;
				$centrocosto = consultarCentroCosto_2013("FARMACIA");
					...
					insertarRegistroPostgresql_2013($compania,$almacenppal,$fecha,$comprobante,$tipocomprobante,$numero,$cedula,$detalle,$autoid,$usuariocre,$fechacre,$estado,$cantidad,$vrcosto,$totcosto,$vrventa,$totventa,$centrocosto,$idtraslado,$anio,$incluyeiva,$idregistro,$cum,$fechadespacho,$lote, $grupo) ;
			...
			function migrarSalidasMedicamentos2013() {
				
				
				/* Tabla Consumo.Comprobantes */
				reemplazarTextoTabla1_2013(utf8_encode("Á"),'&Aacute;');			
				reemplazarTextoTabla1_2013(utf8_encode("É"),'&Eacute;');
				reemplazarTextoTabla1_2013(utf8_encode("Í"),'&Iacute;');
				reemplazarTextoTabla1_2013(utf8_encode("Ó"),'&Oacute;');
				reemplazarTextoTabla1_2013(utf8_encode("Ú"),'&Uacute;');
				reemplazarTextoTabla1_2013(utf8_encode("Ñ"),'&Ntilde;');
				
				/* Tabla Consumo.TiposComprobante */
				reemplazarTextoTabla2_2013(utf8_encode("Á"),'&Aacute;');			
				reemplazarTextoTabla2_2013(utf8_encode("É"),'&Eacute;');
				reemplazarTextoTabla2_2013(utf8_encode("Í"),'&Iacute;');
				reemplazarTextoTabla2_2013(utf8_encode("Ó"),'&Oacute;');
				reemplazarTextoTabla2_2013(utf8_encode("Ú"),'&Uacute;');
				reemplazarTextoTabla2_2013(utf8_encode("Ñ"),'&Ntilde;');
				
				/* Tabla Consumo.Grupos */
				reemplazarTextoTabla3_2013(utf8_encode("Á"),'&Aacute;');			
				reemplazarTextoTabla3_2013(utf8_encode("É"),'&Eacute;');
				reemplazarTextoTabla3_2013(utf8_encode("Í"),'&Iacute;');
				reemplazarTextoTabla3_2013(utf8_encode("Ó"),'&Oacute;');
				reemplazarTextoTabla3_2013(utf8_encode("Ú"),'&Uacute;');
				reemplazarTextoTabla3_2013(utf8_encode("Ñ"),'&Ntilde;');
				
				/* Tabla Central.CentrosCosto */
				reemplazarTextoTabla5_2013(utf8_encode("Á"),'&Aacute;');			
				reemplazarTextoTabla5_2013(utf8_encode("É"),'&Eacute;');
				reemplazarTextoTabla5_2013(utf8_encode("Í"),'&Iacute;');
				reemplazarTextoTabla5_2013(utf8_encode("Ó"),'&Oacute;');
				reemplazarTextoTabla5_2013(utf8_encode("Ú"),'&Uacute;');
				reemplazarTextoTabla5_2013(utf8_encode("Ñ"),'&Ntilde;');
				...
				/* Tabla Consumo.Comprobantes */
				reemplazarTextoTabla1_2013('&Aacute;',utf8_encode("Á"));			
				reemplazarTextoTabla1_2013('&Eacute;', utf8_encode("É"));
				reemplazarTextoTabla1_2013('&Iacute;', utf8_encode("Í"));
				reemplazarTextoTabla1_2013('&Oacute;' , utf8_encode("Ó"));
				reemplazarTextoTabla1_2013('&Uacute;' , utf8_encode("Ú"));
				reemplazarTextoTabla1_2013('&Ntilde;', utf8_encode("Ñ"));
				
				/* Tabla Consumo.TiposComprobante */
				reemplazarTextoTabla2_2013('&Aacute;', utf8_encode("Á"));			
				reemplazarTextoTabla2_2013('&Eacute;', utf8_encode("É"));
				reemplazarTextoTabla2_2013('&Iacute;', utf8_encode("Í"));
				reemplazarTextoTabla2_2013('&Oacute;', utf8_encode("Ó"));
				reemplazarTextoTabla2_2013('&Uacute;', utf8_encode("Ú"));
				reemplazarTextoTabla2_2013('&Ntilde;', utf8_encode("Ñ"));
				
				/* Tabla Consumo.Grupos */
				reemplazarTextoTabla3_2013('&Aacute;', utf8_encode("Á"));			
				reemplazarTextoTabla3_2013('&Eacute;', utf8_encode("É"));
				reemplazarTextoTabla3_2013('&Iacute;', utf8_encode("Í"));
				reemplazarTextoTabla3_2013('&Oacute;', utf8_encode("Ó"));
				reemplazarTextoTabla3_2013('&Uacute;', utf8_encode("Ú"));
				reemplazarTextoTabla3_2013('&Ntilde;', utf8_encode("Ñ"));
				
				/* Tabla Consumo.Movimiento */
				reemplazarTextoTabla4_2013('&Aacute;', utf8_encode("Á"));			
				reemplazarTextoTabla4_2013('&Eacute;', utf8_encode("É"));
				reemplazarTextoTabla4_2013('&Iacute;', utf8_encode("Í"));
				reemplazarTextoTabla4_2013('&Oacute;', utf8_encode("Ó"));
				reemplazarTextoTabla4_2013('&Uacute;',utf8_encode("Ú"));
				reemplazarTextoTabla4_2013('&Ntilde;',utf8_encode("Ñ"));		
				
				/* Tabla Central.CentrosCosto */   
				
				reemplazarTextoTabla5_2013('&Aacute;', utf8_encode("Á"));			
				reemplazarTextoTabla5_2013('&Eacute;', utf8_encode("É"));
				reemplazarTextoTabla5_2013('&Iacute;', utf8_encode("Í"));
				reemplazarTextoTabla5_2013('&Oacute;', utf8_encode("Ó"));
				reemplazarTextoTabla5_2013('&Uacute;',utf8_encode("Ú"));
				reemplazarTextoTabla5_2013('&Ntilde;',utf8_encode("Ñ"));	
