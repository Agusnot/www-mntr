...
<?php
	...
	/* Inicia la definicion de funciones */
		...
		function contarRegistrosMySQL2011() {
			...
			$cons = "SELECT COUNT(*) AS conteomysql FROM Salud.EntradasFarmacia2011";
		...
		function contarRegistrosPostgresql2011() {
			...
			$cons = "SELECT COUNT(*) AS conteo FROM Consumo.movimiento WHERE almacenppal = 'FARMACIA' AND UPPER(comprobante) = 'ORDEN DE COMPRA FARMACIA' AND DATE_PART('year', fecha) = '2011'";
		...
		function eliminarEntradasFarmacia2011() {
			$cnx= conectar_postgres();
			$cons = "DELETE  FROM Consumo.movimiento WHERE almacenppal = 'FARMACIA' AND UPPER(Comprobante) = 'ORDEN DE COMPRA FARMACIA' AND DATE_PART('year', fecha) = '2011'";
			$res =  @pg_query($cnx, $cons);
				if (!$res)  {
		...
		function  reemplazarTextoTabla4_2011($cadenaBusqueda,$cadenaReemplazo)  {
			$cnx= conectar_postgres();
			$cons = "UPDATE Consumo.movimiento SET compania = replace( compania,'$cadenaBusqueda','$cadenaReemplazo'), almacenppal = replace( almacenppal,'$cadenaBusqueda','$cadenaReemplazo'), comprobante = replace( comprobante,'$cadenaBusqueda','$cadenaReemplazo'), tipocomprobante = replace( tipocomprobante,'$cadenaBusqueda','$cadenaReemplazo'), centrocosto = replace( centrocosto,'$cadenaBusqueda','$cadenaReemplazo')  WHERE almacenppal = 'FARMACIA' AND UPPER(Comprobante) = 'ORDEN DE COMPRA FARMACIA' AND DATE_PART('year',fecha) = '2011'";
		...
		function llamarMovimientoMySQL2011() {
			...
			global $res;
			$cnx = conectar_mysql("Salud");
			$cons = "SELECT YEAR(Fecha) AS anio, EntradasFarmacia2011.*  FROM Salud.EntradasFarmacia2011 ORDER BY Fecha ASC, OrdenCompra ASC ";
			$res =  mysql_query($cons);
			return $res; 
		
		}
		
		function eliminarTablaMigracion2011() {
		...
		function crearTablaMigracion2011() {
  ...
  compcontable character varying(50),  numcompcont character varying(100),  conceptortefte character varying(200),  regmedicamento double precision,  noliquidacion integer,  numservicio integer,  grupo character varying(150),  vobo integer,  usuariovobo character varying(200),  fechavobo timestamp with time zone,  notanovisado character varying(500),  numorden integer,  idescritura integer,  numerocontrolados double precision,  idregistro serial ,  cum character varying(1500),  motivodev character varying(1000),  fechadespacho date,  motivodevolucion text,  lote character(150),  error text )WITH (  OIDS=FALSE)";
		...
		function insertarRegistroMigracion2011($compania, $almacenppal, $fecha, $comprobante, $tipocomprobante, $numero, $autoid, $usuariocre, $estado, $cantidad, $usuariocre, $estado,  $cantidad, $vrcosto, $totcosto, $vrventa, $totventa, $porciva, $vriva,  $porcdescto, $vrdescto, $centrocosto, $nofactura, $vrfactura, $anio, $grupo,$cum, $lote, $error) {
		...
		function insertarRegistroPostgresql2011($compania, $almacenppal, $fecha, $comprobante, $tipocomprobante, $numero, $autoid, $usuariocre, $estado, $cantidad,  $vrcosto, $totcosto, $vrventa, $totventa, $porciva, $vriva,  $porcdescto, $vrdescto, $centrocosto, $nofactura, $vrfactura, $anio, $grupo, $cum, $lote ) {
			...
			$res = @pg_query($cnx, $cons);
				if (!$res) {
					...
					$resUTF8 = @pg_query($cnx, $consUTF8);					
						if (!$resUTF8) {
							$error = pg_last_error();
							insertarRegistroMigracion2011($compania, $almacenppal, $fecha, $comprobante, $tipocomprobante, $numero, $autoid, $usuariocre, $estado, $cantidad, $usuariocre, $estado,  $cantidad, $vrcosto, $totcosto, $vrventa, $totventa, $porciva, $vriva,  $porcdescto, $vrdescto, $centrocosto, $nofactura, $vrfactura, $anio, $grupo, $cum, $lote,  $error) ;
						}
		...
		function  llenarMatriz2011(){
			...
			function insertarTabla2011()  {
				...
				$centrocosto = consultarCentroCosto2011("FARMACIA");
					...
					$centrocosto= consultarCentroCosto2011($centrocosto);
		...
		function migrarEntradasFarmacia2011(){
			
			
			
			eliminarEntradasFarmacia2011();
			//eliminarTmpMovimiento(1);
			//eliminarTablaMigracion2011();
			//crearTablaMigracion2011();
			//crearArchivo2011();
			//cambiarcampo2011();
			...
			reemplazarTextoTabla5_2011('&Ntilde;',utf8_encode("Ñ"));	
				...
				//generarNota2011(1,$nota);	
				
				//$nota = "Modificar el tamaño del campo 'CentroCosto' de la tabla Consumo.Movimiento a Character Varying(20) ";
				//generarNota2011(2,$nota);	
				
				/*
				$totalMySQL = contarRegistrosMySQL2011();
				$totalPostgresql =  contarRegistrosPostgresql2011();
				$totalPostgresqlErrores =  contarRegistrosPostgresql2011Errores();
