...
<?php
	...
	/* Inicia la definicion de funciones */
		...
		function contarRegistrosMySQL2013() {
			...
			$cons = "SELECT COUNT(*) AS conteomysql FROM Salud.EntradasFarmacia2013";
		...
		function contarRegistrosPostgresql2013() {
			...
			$cons = "SELECT COUNT(*) AS conteo FROM Consumo.movimiento WHERE almacenppal = 'FARMACIA' AND UPPER(comprobante) = 'ORDEN DE COMPRA FARMACIA' AND DATE_PART('year', fecha) = '2013'";
		...
		function eliminarEntradasFarmacia2013() {
			$cnx= conectar_postgres();
			$cons = "DELETE  FROM Consumo.movimiento WHERE almacenppal = 'FARMACIA' AND UPPER(Comprobante) = 'ORDEN DE COMPRA FARMACIA' AND DATE_PART('year', fecha) = '2013'";
			$res =  @pg_query($cnx, $cons);
				if (!$res)  {
		...
		function consultarCentroCosto2013($centrocosto) {
			$cnx= conectar_postgres();
		...
		function llamarMovimientoMySQL2013() {
			// El limite inicial y final se usan para una inmensa cantidad de registros
			global $res;
			$cnx = conectar_mysql("Salud");
			$cons = "SELECT YEAR(Fecha) AS anio, EntradasFarmacia2013.*  FROM Salud.EntradasFarmacia2013 ORDER BY Fecha ASC, OrdenCompra ASC ";
			$res =  mysql_query($cons);
			return $res; 
		
		}
		
		function eliminarTablaMigracion2013() {
		...
		function crearTablaMigracion2013() {
  ...
  compcontable character varying(50),  numcompcont character varying(100),  conceptortefte character varying(200),  regmedicamento double precision,  noliquidacion integer,  numservicio integer,  grupo character varying(150),  vobo integer,  usuariovobo character varying(200),  fechavobo timestamp with time zone,  notanovisado character varying(500),  numorden integer,  idescritura integer,  numerocontrolados double precision,  idregistro serial ,  cum character varying(1500),  motivodev character varying(1000),  fechadespacho date,  motivodevolucion text,  lote character(150),  error text )WITH (  OIDS=FALSE)";
		...
		function insertarRegistroMigracion2013($compania, $almacenppal, $fecha, $comprobante, $tipocomprobante, $numero, $autoid, $usuariocre, $estado, $cantidad, $usuariocre, $estado,  $cantidad, $vrcosto, $totcosto, $vrventa, $totventa, $porciva, $vriva,  $porcdescto, $vrdescto, $centrocosto, $nofactura, $vrfactura, $anio, $grupo,$cum, $lote, $error) {
		...
		function insertarRegistroPostgresql2013($compania, $almacenppal, $fecha, $comprobante, $tipocomprobante, $numero, $autoid, $usuariocre, $estado, $cantidad,  $vrcosto, $totcosto, $vrventa, $totventa, $porciva, $vriva,  $porcdescto, $vrdescto, $centrocosto, $nofactura, $vrfactura, $anio, $grupo, $cum, $lote ) {
			...
			$res = @pg_query($cnx, $cons);
				if (!$res) {
					...
					$resUTF8 = @pg_query($cnx, $consUTF8);					
						if (!$resUTF8) {
							$error = pg_last_error();
							insertarRegistroMigracion2013($compania, $almacenppal, $fecha, $comprobante, $tipocomprobante, $numero, $autoid, $usuariocre, $estado, $cantidad, $usuariocre, $estado,  $cantidad, $vrcosto, $totcosto, $vrventa, $totventa, $porciva, $vriva,  $porcdescto, $vrdescto, $centrocosto, $nofactura, $vrfactura, $anio, $grupo, $cum, $lote,  $error) ;
						}
		...
		function  llenarMatriz2013(){
			
			unset($matriz); 
			global  $matriz;	
			$res = llamarMovimientoMySQL2013();
			$posicion=0;
			...
			function insertarTabla2013()  {
			
				global $res,$matriz;
				$numeroMaximo = consultarMaximoLote();
				$centrocosto = consultarCentroCosto2013("FARMACIA");
					...
					$centrocosto= consultarCentroCosto2013($centrocosto);
		...
		function migrarEntradasFarmacia2013(){
			
			
			
			eliminarEntradasFarmacia2013();
			//eliminarTmpMovimiento(1);
			//eliminarTablaMigracion2013();
			//crearTablaMigracion2013();
			//crearArchivo2013();
			//cambiarcampo2013();
			...
			reemplazarTextoTabla5_2013('&Ntilde;',utf8_encode("Ñ"));	
				...
				//generarNota2013(1,$nota);	
				
				//$nota = "Modificar el tamaño del campo 'CentroCosto' de la tabla Consumo.Movimiento a Character Varying(20) ";
				//generarNota2013(2,$nota);	
				
				/*
				$totalMySQL = contarRegistrosMySQL2013();
				$totalPostgresql =  contarRegistrosPostgresql2013();
				$totalPostgresqlErrores =  contarRegistrosPostgresql2013Errores();
