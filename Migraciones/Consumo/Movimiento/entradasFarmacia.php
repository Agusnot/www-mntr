...
<?php
	...
	include_once('entradasFarmacia2010.php');
	include_once('entradasFarmacia2011.php');
	include_once('entradasFarmacia2012.php');
	include_once('entradasFarmacia2013.php');
	include_once('entradasFarmacia2013_II.php');
	...
	// Inicia la definicion de funciones 
		...
		function contarRegistrosMySQL() {
			...
			$cons = "SELECT COUNT(*) AS conteomysql FROM Salud.EntradasFarmacia WHERE YEAR(Fecha) = '$anioActual'";
		...
		function contarRegistrosPostgresql() {
			...
			$anioActual = consultarAnio();
			$cons = "SELECT COUNT(*) AS conteo FROM Consumo.movimiento WHERE almacenppal = 'FARMACIA' AND UPPER(comprobante) = 'ORDEN DE COMPRA FARMACIA' AND DATE_PART('year', fecha) = '$anioActual'";
		...
		function llamarMovimientoMySQL() {
			...
			global $res;
			$cnx = conectar_mysql("Salud");
				// Se incluye una clausula para el año porque se pueden duplicar los registros con los de la tabla EntradasFarmacia2013
			$cons = "SELECT YEAR(Fecha) AS anio, EntradasFarmacia.*  FROM Salud.EntradasFarmacia WHERE YEAR(Fecha) = '$anioActual'  ORDER BY Fecha ASC, OrdenCompra ASC ";
			$res =  mysql_query($cons);
			return $res; 
		...
		function eliminarEntradasFarmacia() {
			$cnx= conectar_postgres();
			$anioActual = consultarAnio();
			$cons = "DELETE  FROM Consumo.movimiento WHERE almacenppal = 'FARMACIA' AND UPPER(Comprobante) = 'ORDEN DE COMPRA FARMACIA' AND DATE_PART('year', fecha) = '$anioActual' ";
			$res =  @pg_query($cnx, $cons);
				if (!$res)  {
		...
		function  reemplazarTextoTabla4($cadenaBusqueda,$cadenaReemplazo)  {
			$anioActual = consultarAnio();
			$cnx= conectar_postgres();
			$cons = "UPDATE Consumo.movimiento SET compania = replace( compania,'$cadenaBusqueda','$cadenaReemplazo'), almacenppal = replace( almacenppal,'$cadenaBusqueda','$cadenaReemplazo'), comprobante = replace( comprobante,'$cadenaBusqueda','$cadenaReemplazo'), tipocomprobante = replace( tipocomprobante,'$cadenaBusqueda','$cadenaReemplazo'), centrocosto = replace( centrocosto,'$cadenaBusqueda','$cadenaReemplazo')  WHERE almacenppal = 'FARMACIA' AND UPPER(Comprobante) = 'ORDEN DE COMPRA FARMACIA' AND DATE_PART('year',fecha) = '$anioActual'";
  ...
  compcontable character varying(50),  numcompcont character varying(100),  conceptortefte character varying(200),  regmedicamento double precision,  noliquidacion integer,  numservicio integer,  grupo character varying(150),  vobo integer,  usuariovobo character varying(200),  fechavobo timestamp with time zone,  notanovisado character varying(500),  numorden integer,  idescritura integer,  numerocontrolados double precision,  idregistro serial ,  cum character varying(1500),  motivodev character varying(1000),  fechadespacho date,  motivodevolucion text,  lote character(150),  error text )WITH (  OIDS=FALSE)";
		...
		function insertarRegistroMigracion($compania, $almacenppal, $fecha, $comprobante, $tipocomprobante, $numero, $autoid, $usuariocre, $estado, $cantidad, $usuariocre, $estado,  $cantidad, $vrcosto, $totcosto, $vrventa, $totventa, $porciva, $vriva,  $porcdescto, $vrdescto, $centrocosto, $nofactura, $vrfactura, $anio, $grupo,$cum, $lote, $error) {
		...
		function insertarRegistroPostgresql($compania, $almacenppal, $fecha, $comprobante, $tipocomprobante, $numero, $autoid, $usuariocre, $estado, $cantidad,  $vrcosto, $totcosto, $vrventa, $totventa, $porciva, $vriva,  $porcdescto, $vrdescto, $centrocosto, $nofactura, $vrfactura, $anio, $grupo, $cum, $lote ) {
			...
			$res = @pg_query($cnx, $cons);
				if (!$res) {
					...
					$resUTF8 = @pg_query($cnx, $consUTF8);					
						if (!$resUTF8) {
							$error = pg_last_error();
							insertarRegistroMigracion($compania, $almacenppal, $fecha, $comprobante, $tipocomprobante, $numero, $autoid, $usuariocre, $estado, $cantidad, $usuariocre, $estado,  $cantidad, $vrcosto, $totcosto, $vrventa, $totventa, $porciva, $vriva,  $porcdescto, $vrdescto, $centrocosto, $nofactura, $vrfactura, $anio, $grupo, $cum, $lote,  $error) ;
						}
		...
		function  llenarMatriz(){
			...
			function migrarEntradasFarmacia(){
				
				
				
				eliminarEntradasFarmacia();
		...
		if($_GET['tabla']="Entradas_Farmacia") {
		
			echo "<fieldset>";			
			echo "<legend> Migracion tabla MySQL </legend>";
			echo "<span align='left'> <a href='../../index.php?migracion=MIG008' class = 'link1'> Panel de Administracion </a> </span>";
			
				eliminarLotes();
				crearArchivoErroresLotes();
				cambiarCampoNumeroLotes();
				...
				migrarEntradasFarmacia2010();
				migrarEntradasFarmacia2011();
				migrarEntradasFarmacia2012();
				migrarEntradasFarmacia2013();				
				migrarEntradasFarmacia2013_II();
				migrarEntradasFarmacia();
				...
				$nota = "Modificar el tama&ntilde;o del campo 'CentroCosto' de la tabla Consumo.Movimiento a Character Varying(20) ";
				generarNota(2,$nota);	
