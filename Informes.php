<?
 ...
 //////////////////////////
	...
	function ExQuery($ExCons)
	{
		...
		//$status = pg_result_status($VarDev);if($status==PGSQL_NONFATAL_ERROR||$status==PGSQL_FATAL_ERROR){echo "$ExCons<br>";}	
		//if(ExError()){echo "$ExCons<br>";}
		return $VarDev;
	}
	...
	{
		global $Compania;
		if($BaseDatos=="Presupuesto"){$condAdc=" and Vigencia='Actual' ";}
		$cons="Select Numero from $BaseDatos.Movimiento where Comprobante='$NomComprobante' and date_part('year',Fecha)=$Anio and Compania='$Compania[0]' $condAdc  Order By Numero Desc;";
		
		$res=ExQuery($cons);
		if(ExNumRows($res)==0)
		{
			$cons2="Select NumeroInicial from $BaseDatos.Comprobantes where Comprobante='$NomComprobante' and Compania='$Compania[0]'";
			$res2=ExQuery($cons2);
			$fila2=ExFetch($res2);
			$Consec=$Anio.$fila2[0];
		}
	...
	function NumerosxLet($Num)
	{
		...
		if($Mill && !$Mi && !$Ce && !$De && !$Un && !$Adc){$Letras = $Mill . $Mi . $Ce . $De . $Un . $Adc . " de pesos ";}
		else{$Letras = $Mill . $Mi . $Ce . $De . $Un . $Adc . " pesos ";}
