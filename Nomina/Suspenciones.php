<?php
	...
	if($Guardar)
	{
		...
		$cons="insert into nomina.suspenciones (compania,identificacion,concepto,fecinicio,fecfinal,dias,detalle,resolucion,autorizacion,usuario,fecha,estado,numero) values ('$Compania[0]','$Identificacion','$RegNomina','$FecInicio','$FecFinal',$Dias,'$Detalle','$Resolucion','$Autorizacion','$usuario[1]','$Fec','$Estado','$Numero')";
		$res=ExQuery($cons);
		$cons="select movimiento,claseconcepto,detconcepto from nomina.conceptosliquidacion where concepto='$RegNomina'";
//		$res=ExQuery($cons);
		?>
        <script>alert("La Suspencion ha sido Guardada !!!");
...
<tr>
	<td colspan="6" colspan=4 bgcolor="#666699" style="color:white" align="center">SUSPENCIONES</td>
</tr>
<tr>
...
<tr>
    ...
    <td>Estado</td>
    <td><select name="Estado" onChange="FORMA.submit()">
		<option></option>
