<?php
	...
	if($Guardar)
	{
		$consn="select numero from nomina.suspenciones order by numero desc";
		$resn=ExQuery($consn);$fila=ExFetch($resn);
		if($fila){$Numero=$fila[0]+1;}else{$Numero=1;}
		$cons="insert into nomina.suspenciones (compania,identificacion,concepto,fecinicio,fecfinal,dias,detalle,resolucion,autorizacion,estado,usuario,fecha,numero) values ('$Compania[0]','$Identificacion','$RegNomina','$FecInicio','$FecFinal',$Dias,'$Detalle','$Resolucion','$Autorizacion','$Estado','$usuario[1]','$Fec','$Numero')";
		$res=ExQuery($cons);
		$cons="select movimiento,claseconcepto,detconcepto from nomina.conceptosliquidacion where concepto='$RegNomina'";
		$res=ExQuery($cons);
		?>
        <script>alert("La Suspencion ha sido Guardada !!!");</script>
<?        
	}
?>
<html>
...
<tr><td colspan="2" bgcolor="#666699" style="color:white" align="center">EMPLEADO SUSPENCIONES</td></tr>
<tr>
...
?>	
	...
	<tr>
		<td colspan="6" bgcolor="#666699" style="color:white" align="center">SUSPENCIONES</td>
	</tr>
	<tr>
		...
		<select name="RegNomina" onChange="FORMA.submit()" style="width:100%" >
			<option></option>
			<?
			$cons="select concepto,detconcepto from nomina.conceptosliquidacion where compania='$Compania[0]' and claseconcepto='Dias' and novedad='Suspencion'";
			//echo $cons;
			$res=ExQuery($cons);
			while($fila=Exfetch($res))
			{
