<?php
	...
	if($Guardar)
	{
		...
		$cons="insert into nomina.incapacidades (compania,identificacion,concepto,fecinicio,fecfinal,dias,detalle,resolucion,autorizacion,estado,usuario,fecha,numero) values ('$Compania[0]','$Identificacion','$RegNomina','$FecInicio','$FecFinal',$Dias,'$Detalle','$Resolucion','$Autorizacion','$Estado','$usuario[1]','$Fec','$Numero')";
		$res=ExQuery($cons);
		$cons="select movimiento,claseconcepto,detconcepto from nomina.conceptosliquidacion where concepto='$RegNomina'";
		$res=ExQuery($cons);
		?>
        <script>alert("La Incapacidad ha sido Guardada !!!");
		</script>
<?       // target="_top"
	}
?>
...
<tr><td colspan="2" bgcolor="#666699" style="color:white" align="center">EMPLEADO INCAPACIDADES</td></tr>
<tr>
