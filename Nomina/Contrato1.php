...
//	else{$FecFin="'$FecFin'";}
		if($con==0)
		{
			...
			if($cont==0)
			{
				$cons="insert into nomina.contratos(compania,identificacion,tipocontrato,estado,tipovinculacion,fecinicio,fecfin,cargo,seccion,grupo,hrslab,jornflexible,pactocolectivo,alimentos,cuenta,banco,numero,usuario) values ('$Compania[0]','$Identificacion','$TipoContrato','$Estado','$TipoVinculacion','$FecInicio',$FecFin,'$Cargo','$Seccion','$Grupo','$HorLab','$Jornada','$PaColectivo','$Alimentacion','$Cuenta','$Banco','$Numero','$usuario[1]')";
			//	echo $cons;
				$res=ExQuery($cons);
				$dia=substr($FecInicio,8,2);
				//echo $dia;
//				echo $dias;
				if($dia!=01)
				{
//					if($FecFin){$FecFin="'$FecFin'";}
					$anio=substr($FecInicio,0,4);
					$mes=substr($FecInicio,5,2);
					$dias=31-$dia;
					echo $anio." - ".$mes." - ".$dias;
					$cons2="select concepto from nomina.conceptosliquidacion,nomina.tiposvinculacion where conceptosliquidacion.compania='$Compania[0]' and tiposvinculacion.compania=conceptosliquidacion.compania and diastr='1' and tiposvinculacion.codigo='$TipoVinculacion' and conceptosliquidacion.tipovinculacion=tiposvinculacion.tipovinculacion";
					//echo $cons2;
					$res2=ExQuery($cons2);
					$fila2=ExFetch($res2);
					//echo $fila2[0]."<--";
					$cons="insert into nomina.diastrab(compania,identificacion,diastr,anio,mestr,concepto,vinculacion,numero) values('$Compania[0]','$Identificacion','$dias','$anio','$mes','$fila2[0]','$TipoVinculacion','$Numero')";
//					echo $cons." 1<br>";
					$res=ExQuery($cons);
				}
	...
	<?		}
		...
		{
			$cons="select numero from nomina.contratos where compania='$Compania[0]' and identificacion='$Identificacion' and numero='$NContrato'";
			$res=ExQuery($cons);
			$cont=ExNumRows($res);
	//		echo $cons."  -->  ".$cont;
			if($NContrato!=$NContAnt)
			{
				if($cont==0)
				{
					$cons="update nomina.contratos set tipocontrato='$TipoContrato',estado='$Estado',tipovinculacion='$TipoVinculacion',fecinicio='$FecInicio',fecfin=$FecFin,cargo='$Cargo',seccion='$Seccion',grupo='$Grupo',hrslab='$HorLab',jornflexible='$Jornada',pactocolectivo='$PaColectivo',alimentos='$Alimentacion',cuenta='$Cuenta',banco='$Banco',numero='$NContrato' where numero='$NContAnt'";
					$res=ExQuery($cons);
					//echo $cons;
				}
				else
		...
		<?		}
			...
			{
				$cons="update nomina.contratos set tipocontrato='$TipoContrato',estado='$Estado',tipovinculacion='$TipoVinculacion',fecinicio='$FecInicio',fecfin=$FecFin,cargo='$Cargo',seccion='$Seccion',grupo='$Grupo',hrslab='$HorLab',jornflexible='$Jornada',pactocolectivo='$PaColectivo',alimentos='$Alimentacion',cuenta='$Cuenta',banco='$Banco' where numero='$NContrato'";
					$res=ExQuery($cons);
			}
		}
	$Act=0;
	//echo $NContAnt;
	//--------------- inserta la fecha final si la hay -----------------------				
//			echo $FecFin." --> ".$FecFinA."j";
				if($FecFin!="NULL"&&$FecFin!="'$FecFinA'")
				{
					$aniof=substr($FecFin,1,4);
					$mesf=substr($FecFin,6,2);
					$diaf=substr($FecFin,9,2);
//					echo $aniof." --> ".$mesf." --> ".$diaf."<br>";
					$cons2="select concepto from nomina.conceptosliquidacion,nomina.tiposvinculacion where conceptosliquidacion.compania='$Compania[0]' and tiposvinculacion.compania=conceptosliquidacion.compania and diastr='1' and tiposvinculacion.codigo='$TipoVinculacion' and conceptosliquidacion.tipovinculacion=tiposvinculacion.tipovinculacion";
//					echo $cons2;
					$res2=ExQuery($cons2);
					$fila2=ExFetch($res2);
					$cons="insert into nomina.diastrab(compania,identificacion,diastr,anio,mestr,concepto,vinculacion,numero) values('$Compania[0]','$Identificacion','$diaf','$aniof','$mesf','$fila2[0]','$TipoVinculacion','$Numero')";
//					echo $cons." 2<br>";
					$res=ExQuery($cons);
				}
//------------------------------------------------------------------------
...
if($Editar==1)
{
	$cons="select identificacion,tipocontrato,estado,tipovinculacion,fecinicio,fecfin,cargo,seccion,grupo,hrslab,jornflexible,pactocolectivo,alimentos,cuenta,banco,numero from nomina.contratos where compania='$Compania[0]' and identificacion='$Identificacion' and numero='$Numero'";
	$res=ExQuery($cons);
	$con=ExNumRows($res);
	//echo $con;
	$fila=ExFetch($res);
...
//	if(!$FecInicio){$FecInicio=$fila[4];}
	if(!$AnioIni){$AnioIni=substr($fila[4],0,4);$MesIni=substr($fila[4],5,2);$DiaIni=substr($fila[4],8,2);}
...
//					echo $Mes;
        ...
        <tr>
            ...
            <td colspan="3"><select name="Cargo" onChange="FORMA.submit()" style="width:auto" <? if($Nuevo==1){echo "disabled";}?> />
            	<option></option>
                <?
				$cons="select cargos.codigo,cargos.cargo from nomina.cargos,nomina.tiposvinculacion where cargos.compania='$Compania[0]' and tiposvinculacion.compania=cargos.compania and cargos.vinculacion='$TipoVinculacion' and tiposvinculacion.codigo=cargos.vinculacion order by cargos.codigo";
				$res=ExQuery($cons);
				while($fila=ExFetch($res))
				{
					if($fila[0]==$Cargo)
        	...
        	<td><select name="Seccion"  style="width:100%" <? if($Nuevo==1){echo "disabled";}?> />
            	<option></option>
                <?
                $cons="select codigo,seccion from nomina.secciones where compania='$Compania[0]'";
				$res=ExQuery($cons);
				while($fila=Exfetch($res))
				{
					if($fila[1]==$Seccion)
        ...
        <tr><td>Fecha Inicio<br><font color="#000099" size="-4">AAAA-MM-DD</font></td>
        ...
        <tr>
            ...
            <td colspan="3"><select name="Jornada"  <? if($Nuevo==1){echo "disabled";}?> />
            	<option></option>
        ...
        <tr><td>Cuenta No.</td>
        	<td colspan="4"><input type="text" name="Cuenta" value="<? echo $Cuenta?>" onKeyDown="xNumero(this)" onKeyUp="xNumero(this)" maxlength="80" style="width:100%" <? if($Nuevo==1){echo "disabled";}?> /></td>
            <td>Banco</td>
        	<td colspan="2"><select name="Banco"  style="width:100%" <? if($Nuevo==1){echo "disabled";}?> />
            	<option></option>
                <?
...
<!--        <tr>
        	<td colspan="6" bgcolor="#666699"style="color:white" align="center">Notas</td>
        </tr>
        <tr>
        	<td colspan="6" ><textarea name="Nota" style=" width:100%" rows="3"><?// echo $Nota;?></textarea></td>
        </tr>-->
</table>
<center><input type="submit" value="Guardar" name="Guardar" <? if($Nuevo==1){echo "disabled";}?> ><input type="button" name="Pdf" value="PDF" /></center>
</form>
