...
include("Funciones.php");
	...
	if($Iniciar)
	{
		...
		ObtieneValoresxDocxCuenta("$Anio-01-01","$Anio-12-31",'Compromiso presupuestal');
		$PerIni="$Anio-01-01";
		$PerFin="$Anio-12-31";

		$AnioSig=$AnioSel+1;
		$cons="Select Cuenta,Nombre,Naturaleza,Tipo from Presupuesto.PlanCuentas where Cuenta ilike '2%' and Anio=$AnioSel and Vigencia='Actual' and Compania='$Compania[0]' and Tipo='Detalle' Order By Cuenta";
		$res=ExQuery($cons);
		while($fila=ExFetch($res))
		{
			...
			if($CompromisosSinAfecta>0)
			{
				...
				while($fila2=ExFetch($res2))
				{
					...
					$cons3="Select Cuenta from Presupuesto.PlanCuentas where Cuenta = '$ParteCta' and Anio=$AnioSig and Vigencia='Anteriores'  and ClaseVigencia='Reservas'
					...
					if(ExNumRows($res3)==0)
					{
						...
						if(ExNumRows($res4)>0)
						{
							...
							values('$Compania[0]',$AnioSig,$ParteCta,'$fila4[1]','$fila4[2]','Titulo','Anteriores','Reservas')";
							$res5=ExQuery($cons5);
						}
						else
						{
							$cons5="Insert into Presupuesto.PlanCuentas(Compania,Anio,Cuenta,Nombre,Naturaleza,Tipo,Vigencia,ClaseVigencia)
							values('$Compania[0]',$AnioSig,$ParteCta,'$ParteCta','Contra Credito','Titulo','Anteriores','Reservas')";
							$res5=ExQuery($cons5);
						}
					}
				}
				...
				values('$Compania[0]',$AnioSig,'$fila[0]','$fila[1]','$fila[2]','$fila[3]',$CompromisosSinAfecta,'Anteriores','Reservas')";
				$res2=ExQuery($cons2);
				...
				Cuenta = '$fila[0]' and Fecha>='$Anio-01-01' and Fecha<='$Anio-12-31' and (TipoComprobant='Compromiso presupuestal')
				and Vigencia='Actual'
...
////////////CREAMOS EL COMPROMISO
						...
						values($AutoId,'$AnioSig-01-01','$fila9[2]',$fila9[3],'$fila9[9]','$fila9[4] (Reserva Vigencia Anterior)','$fila9[0]','$Valor','0','$fila9[3]','$Compania[0]','Cierre Fiscal $Anio','$ND[year]-$ND[mon]-$ND[mday]','AC','','$fila101[0]','Anteriores','Reservas',$AnioSig)";
						$res10=ExQuery($cons10);

////////////CREAMOS EL C.D.P.
						
						$cons10="Insert Into Presupuesto.Movimiento(AutoId,Fecha,Comprobante,Numero,Identificacion,Detalle,Cuenta,Credito,ContraCredito,DocSoporte,Compania,UsuarioCre,FechaCre,Estado,DocOrigen,CompAfectado,Vigencia,ClaseVigencia,Anio)
						values($AutoId,'$AnioSig-01-01','$fila101[0]',$fila9[3],'$fila9[9]','$fila9[4] (Reserva Vigencia Anterior)','$fila9[0]','$Valor','0','0','$Compania[0]','Cierre Fiscal $Anio','$ND[year]-$ND[mon]-$ND[mday]','AC','','','Anteriores','Reservas',$AnioSig)";
						$res10=ExQuery($cons10);

					}
				}
			}
			if($ObligacSinAfectar>0)
			{
			ObtieneValoresxDocxCuenta("$Anio-01-01","$Anio-12-31",'Obligacion presupuestal');
				...
				$cons2="Insert into Presupuesto.PlanCuentas(Compania,Anio,Cuenta,Nombre,Naturaleza,Tipo,Apropiacion,Vigencia,ClaseVigencia)
				values('$Compania[0]',$AnioSig,'$fila[0]','$fila[1]','$fila[2]','$fila[3]',$ObligacSinAfectar,'Anteriores','CxP')";
				$res2=ExQuery($cons2);


				$cons9="Select Cuenta,Fecha,Movimiento.Comprobante,Numero,Detalle,'','',sum(Credito),sum(ContraCredito),Identificacion,'' 
				from Presupuesto.Movimiento,Presupuesto.Comprobantes where Movimiento.Comprobante=Comprobantes.Comprobante and
				Cuenta = '$fila[0]' and Fecha>='$Anio-01-01' and Fecha<='$Anio-12-31' and (TipoComprobant='Obligacion presupuestal')
				and Vigencia='Actual'
				and Estado='AC' and Movimiento.Compania='$Compania[0]' and Comprobantes.Compania='$Compania[0]'
				Group By Cuenta,Movimiento.Comprobante,Numero,Fecha,Detalle,Identificacion,Vigencia,ClaseVigencia 
				Order By Numero";
//				echo $cons9."<br>";
				$res9=ExQuery($cons9);
			//	echo "Valor-->".$CompromisosSinAfecta."<br>";
				while($fila9=ExFetch($res9))
				{
					$Valor=CalcularSaldoxDocxCuenta($fila9[0],$fila9[3],$fila9[2],"$Anio-$MesIni-01","$Anio-$MesFin-31","Actual","");
					if($Valor>0)
					{
//						echo "<font color='#ff0000'>$Valor</font><br>";;
	//					echo "$fila9[0]-$fila9[1]-$fila9[2]-$fila9[3]-$fila9[4]-$fila9[6]-$fila9[7]-$fila9[8]<br>";
...
////////////CREAMOS LA OBLIGACION
						$cons100="Select CompDestino from Presupuesto.CruceComprobantes where CompOrigen='Obligacion presupuestal'";
						$res100=ExQuery($cons100);
						$fila100=ExFetch($res100);
						...
						$cons10="Insert Into Presupuesto.Movimiento(AutoId,Fecha,Comprobante,Numero,Identificacion,Detalle,Cuenta,Credito,ContraCredito,DocSoporte,Compania,UsuarioCre,FechaCre,Estado,DocOrigen,CompAfectado,Vigencia,ClaseVigencia,Anio)
						values($AutoId,'$AnioSig-01-01','$fila9[2]',$fila9[3],'$fila9[9]','$fila9[4] (CxP Vigencia Anterior)','$fila9[0]','$Valor','0','$fila9[3]','$Compania[0]','Cierre Fiscal $Anio','$ND[year]-$ND[mon]-$ND[mday]','AC','','$fila101[0]','Anteriores','CxP',$AnioSig)";
						$res10=ExQuery($cons10);
////////////CREAMOS EL REGISTRO

						$consA100="Select CompDestino from Presupuesto.CruceComprobantes where CompOrigen='Compromiso presupuestal'";
						$resA100=ExQuery($consA100);
						$filaA100=ExFetch($resA100);

						$consA101="Select Comprobante from Presupuesto.Comprobantes where TipoComprobant='$filaA100[0]'";
						$resA101=ExQuery($consA101);
						$filaA101=ExFetch($resA101);

						
						$cons10="Insert Into Presupuesto.Movimiento(AutoId,Fecha,Comprobante,Numero,Identificacion,Detalle,Cuenta,Credito,ContraCredito,DocSoporte,Compania,UsuarioCre,FechaCre,Estado,DocOrigen,CompAfectado,Vigencia,ClaseVigencia,Anio)
						values($AutoId,'$AnioSig-01-01','$fila101[0]',$fila9[3],'$fila9[9]','$fila9[4] (CxP Vigencia Anterior)','$fila9[0]','$Valor','0','$fila9[3]','$Compania[0]','Cierre Fiscal $Anio','$ND[year]-$ND[mon]-$ND[mday]','AC','','$filaA101[0]','Anteriores','CxP',$AnioSig)";
						$res10=ExQuery($cons10);

////////////CREAMOS EL C.D.P.
						
						$cons10="Insert Into Presupuesto.Movimiento(AutoId,Fecha,Comprobante,Numero,Identificacion,Detalle,Cuenta,Credito,ContraCredito,DocSoporte,Compania,UsuarioCre,FechaCre,Estado,DocOrigen,CompAfectado,Vigencia,ClaseVigencia,Anio)
						values($AutoId,'$AnioSig-01-01','$filaA101[0]',$fila9[3],'$fila9[9]','$fila9[4] (CxP Vigencia Anterior)','$fila9[0]','$Valor','0','0','$Compania[0]','Cierre Fiscal $Anio','$ND[year]-$ND[mon]-$ND[mday]','AC','','','Anteriores','CxP',$AnioSig)";
						$res10=ExQuery($cons10);
