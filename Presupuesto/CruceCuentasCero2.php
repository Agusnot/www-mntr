...
<?	}
	...
	if($Mes==1)
	{
		...
		while($fila=ExFetch($res))
		{
			...
			values($AutoId,'$FechaContable','$fila[2]',$Numero,'".$Tercero."','Apropiacion Inicial','".$fila[4]."','".$Valor."','0','0','0','0','$Compania[0]','$usuario[0]','$ND[year]-$ND[mon]-$ND[mday] $ND[hours]:$ND[minutes]','Apropiacion inicial')";
			$res10=ExQuery($cons10);
			$AutoId++;
			$cons10="Insert into Contabilidad.Movimiento(AutoId,Fecha,Comprobante,Numero,Identificacion,Detalle,Cuenta,Debe,Haber,CC,DocSoporte,BaseGravable,Compania,UsuarioCre,FechaCre,DocDestino)
			values($AutoId,'$FechaContable','$fila[2]',$Numero,'".$Tercero."','Apropiacion Inicial','".$fila[5]."','0','".$Valor."','0','0','0','$Compania[0]','$usuario[0]','$ND[year]-$ND[mon]-$ND[mday] $ND[hours]:$ND[minutes]','Apropiacion inicial')";
			$res10=ExQuery($cons10);}
		}
	$Numero++;
	}
	...
	while($fila=ExFetch($res))
	{
		...
		while($fila1=ExFetchArray($res1))
		{
			...
			$AutoId++;
			$cons10="Insert into Contabilidad.Movimiento(AutoId,Fecha,Comprobante,Numero,Identificacion,Detalle,Cuenta,Debe,Haber,CC,DocSoporte,BaseGravable,Compania,UsuarioCre,FechaCre,DocDestino)
			values($AutoId,'$FechaContable','$fila[2]',$Numero,'$Tercero','Interface Cuenta Cero Mes $MesLet','".$fila[4]."','".$Valor."','0','0','0','0','$Compania[0]','$usuario[0]','$ND[year]-$ND[mon]-$ND[mday] $ND[hours]:$ND[minutes]','".$fila1['comprobante']."-".$fila1['numero']."')";
			$res10=ExQuery($cons10);

			$AutoId++;
			$cons10="Insert into Contabilidad.Movimiento(AutoId,Fecha,Comprobante,Numero,Identificacion,Detalle,Cuenta,Debe,Haber,CC,DocSoporte,BaseGravable,Compania,UsuarioCre,FechaCre,DocDestino)
			values($AutoId,'$FechaContable','$fila[2]',$Numero,'$Tercero','Interface Cuenta Cero Mes $MesLet','".$fila[5]."','0','".$Valor."','0','0','0','$Compania[0]','$usuario[0]','$ND[year]-$ND[mon]-$ND[mday] $ND[hours]:$ND[minutes]','".$fila1['comprobante']."-".$fila1['numero']."')";
			$res10=ExQuery($cons10);
	...
	$cons="Select * from Presupuesto.CruceCuentasCero where Anio=$Anio and Vigencia='$Vigencia' and Compania='$Compania[0]'";
	$res=ExQuery($cons);
	while($fila=ExFetch($res))
	{
...
/*
	...
	while($fila=ExFetch($res))
	{
		...
		while($fila1=ExFetchArray($res1))
		{
			$Valor=$fila1['credito']+$fila1['contracredito'];
			$AutoId++;
			$cons10="Insert into Contabilidad.Movimiento(AutoId,Fecha,Comprobante,Numero,Identificacion,Detalle,Cuenta,Debe,Haber,CC,DocSoporte,BaseGravable,Compania,UsuarioCre,FechaCre,DetConcepto,DocDestino)
			values($AutoId,'$FechaContable','$fila[2]',$Numero,'$Tercero','Interface Cuenta Cero Mes $MesLet','".$fila[4]."','".$Valor."','0','0','0','0','$Compania[0]','$usuario[0]','$ND[year]-$ND[mon]-$ND[mday] $ND[hours]:$ND[minutes]','Interface presupuesto-contabilidad','".$fila1['comprobante']."-".$fila1['numero']."')";
			$res10=ExQuery($cons10);

			$AutoId++;
			$cons10="Insert into Contabilidad.Movimiento(AutoId,Fecha,Comprobante,Numero,Identificacion,Detalle,Cuenta,Debe,Haber,CC,DocSoporte,BaseGravable,Compania,UsuarioCre,FechaCre,DetConcepto,DocDestino)
			values($AutoId,'$FechaContable','$fila[2]',$Numero,'$Tercero','Interface Cuenta Cero Mes $MesLet','".$fila[5]."','0','".$Valor."','0','0','0','$Compania[0]','$usuario[0]','$ND[year]-$ND[mon]-$ND[mday] $ND[hours]:$ND[minutes]','Interface presupuesto-contabilidad','".$fila1['comprobante']."-".$fila1['numero']."')";
			$res10=ExQuery($cons10);
