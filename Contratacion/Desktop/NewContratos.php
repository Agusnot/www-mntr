	...
	if($Cancelar){
		...
		if($Guardar)
		{
			...
			{				
				...
				,'$NomCuenta3',$RestrPago,'$AmbitoContrato' $FecF2)";	
				$Edit=1;
			}
			else{ 				
				...
				where Entidad='$EntidadAnt' and Contrato='$ContratoAnt' and Numero='$NumeroAnt' and Compania='$Compania[0]'";		
			}
...
<script language="javascript">
	...
	{		
			...
			if(document.FORMA.Objeto.value==""){alert("");return false;}
    ...
    <tr>
  	...
  	<? 	if(!$CuentaContable){$CuentaContable=$fila['cuentacont'];}
        ...
        <?	if(!$CompFac){$CompFac=$fila['compfacturacion'];}
			$consCFac="select comprobante from contabilidad.comprobantes where compania='$Compania[0]' and tipocomprobant='Facturas'";
