...
<?	
	if($Guardar){
		...
		interpretacion='$Interpretacion',usuariointerpretacion='$usuario[1]',fechainterpretacion='$ND[year]-$ND[mon]-$ND[mday] $ND[hours]:$ND[minutes]:$ND[seconds]'
		...
		if($Tbl){
			...
			if($row['numorden']){
				...
				if($BanUpdate){
					...
					$res=ExQuery($cons); $fila=ExFetch($res); $Id_h=$fila[0]+1;								
