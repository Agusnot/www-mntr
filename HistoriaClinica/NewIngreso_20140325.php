<?php
	...
	if($Anadir){
		...
		$cons="insert into salud.tmpalertasingreso (compania,usuario,cedula,tmpcod,alerta,numservicio,fechaini,fechafin,numalert) 
		values ('$Compania[0]','$usuario[1]','$Ced','$TMPCOD','$Alerta',$NumServ,'$FechaIni','$FechaFin',$NumAlert)";
		$res=ExQuery($cons);
	}
	if($Ingresar){				
		...
		while($fila=ExFetch($res)){
			...
			values ('$Compania[0]','$usuario[1]','$ND[year]-$ND[mon]-$ND[mday] $ND[hours]:$ND[minutes]:$ND[seconds]',$NumServ,'$fila[1]','$fila[2]','$fila[0]','$Ced')";
			$res2=ExQuery($cons2);
		}
		$cons="update salud.servicios set usuarioingreso='$usuario[1]',ingreso=1,fecharegingreso='$ND[year]-$ND[mon]-$ND[mday] $ND[hours]:$ND[minutes]:$ND[seconds]',notaingreso='$Notas', autorizac1='$Autorizac'
	...
	if(document.FORMA.Alerta.value==""){
		...
		if(document.FORMA.FechaIni.value==""||document.FORMA.FechaFin.value==""){
			...
			if(document.FORMA.FechaIni.value<document.FORMA.FechaComp.value){
				...
				if(document.FORMA.FechaIni.value>document.FORMA.FechaFin.value){
					alert("La fecha de inicio es mayor a la fehca final!!!");
				}
				else{
...
<table bordercolor="#e5e5e5" border="1" style='font : normal normal small-caps 12px Tahoma;' align="center" cellpadding="4">
	 <tr align="center"  bgcolor="#e5e5e5" style="font-weight:bold"><td>Cedula</td><td>Nombre</td><td>Aseguradora</td><td>Contrato</td><td>No. Contrato</td>
     <td>No Autorizacion</td></tr>
<? 	$cons="select primnom,segnom,primape,segape,identificacion,numservicio,autorizac1 from salud.servicios,central.terceros where 
	...
	<tr><td  align="center"  bgcolor="#e5e5e5" style="font-weight:bold" colspan="6">Lista de Verificacion de Ingreso</td></tr>
<?	$cons3="select pregunta,obligatorio from salud.preguntasingreso where compania='$Compania[0]'";
	$res3=ExQuery($cons3);
	if(ExNumRows($res3)>0){?>
...
<?			$cont++;
	    ...
	    <tr><td colspan="6">No sen han ingresado preguntas de verificacion de Ingreso</td></tr>
<?	}?>    
	<tr><td  align="center"  bgcolor="#e5e5e5" style="font-weight:bold" colspan="6">Notificaciones</td></tr>
<?	$cons4="select alerta,fechaini,fechafin,numalert from salud.tmpalertasingreso 
    ...
    Desde: <input type="text" readonly name="FechaIni" style="width:80" 
    onClick="popUpCalendar(this, FORMA.FechaIni, 'yyyy-mm-dd')"> Hasta: <input type="text" readonly name="FechaFin" style="width:80" 
    ...
    <tr><td align="center" colspan="6"><input type="submit" value="Ingresar" name="Ingresar"><input type="button" value="Cancelar" onClick="Salir()"></td></tr>
</table>
