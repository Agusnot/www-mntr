...
if($Formato=="FORMATO NO POS"){?>
      ...
      <select name="menu1" onChange="MM_jumpMenu('self',this,0)">
        <?php
	$consNOPOS="select usuario,fecha,cmp00002,cmp00006,hora from histoclinicafrms.tbl00049 where cedula='$Paciente[1]' order by fecha desc";
...
///////////////// INSERCION DE DATOS A LA HISTORIA CLINICA /////////////////////////////
		...
		and Pantalla=$IdPantalla and Compania='$Compania[0]' and estado='AC'  Order By Id_Item";		
	...
	<?			}
		...
		{			
			...
			if($MsjError){echo "<br><em>Favor copie este mensaje y envielo al correo interno de sistemas para su revision</em><br>";exit;}
				...
				$resAgendInt=ExQuery($consAgendInt); $filaAgendaInt=ExFetch($resAgendInt); $AmbitoAgendInt=$filaAgendaInt[0];
				...
				$resAgendInt=ExQuery($consAgendInt); $filaAgendaInt=ExFetch($resAgendInt);
				...
				$resAgendInt=ExQuery($consAgendInt); $filaAgendaInt=ExFetch($resAgendInt); $FrecAgendaInt=$filaAgendaInt[0];
				if($FrecAgendaInt)
				{
					...
					$resAgendInt=ExQuery($consAgendInt); $filaAgendaInt=ExFetch($resAgendInt); $FecProxAgendaInt=$filaAgendaInt[0];
					//echo $consAgendInt;
					if($FecProxAgendaInt)
					{						
...
////////////////////////////////////////ASIGNACION DE CUPS////////////////////////////////
		...
		from HistoriaClinica.ItemsxFormatos where Formato='$Formato' and TipoFormato='$TipoFormato' 
		and Pantalla=$IdPantalla and (Titulo IS NULL or titulo = 0)  and Compania='$Compania[0]' and estado='AC' Order By orden";
		...
		?>	<script language="javascript">
			...
			if(!$BanCUP){
				if($PacienteSeguro=="Si")
				{
					...
					$cons="insert into pacienteseguro.sucesos (compania,usuario,fechacrea,formato,tipoformato,idhistoria,cedula,pabellon,ambito) values
...
if($IncluirSignosVitales)
{
	...
	<div align="center">
	<table border="1" bordercolor="#e5e5e5" width="800px" style="font : 12px Tahoma;"> 
	...
	<? 
		...
		if($IdHistoria)
		{
			...
			//---
			$cons9="Select Id_Item,Item,cargoxitem,subformato from HistoriaClinica.ItemsxFormatos where Formato='$Formato' and TipoFormato='$TipoFormato' and Compania='$Compania[0]' and (Titulo IS NULL or titulo = 0)
		...
		{	
			...
			{			
				...
				if($Alineacion=="Vertical")
				{
					...
					if($fila['titulo']==1){
						if($Mensaje=="<b>INFORMACIÓN MEDICO </b>" || $Mensaje=="<b> INFORMACIÓN MEDICO </b>"){
		...
		<?		}
			...
			<?	}
				if($fila['item']=="Diagnostico")
				{
					...
					if(!$ValDx[1]){$ValDx[1]=$filaServ1[1];}
					$cons8="Select Detalle,CIE10,Id from historiaclinica.dxformatos where Compania='$Compania[0]' and Estado='AC' and Formato='$Formato' and TipoFormato='$TipoFormato' Order By Id";
					$res8=ExQuery($cons8);
					//echo $cons8;
					$banDx=0;
					...
					{			
						...
						{ 	
							...
							if($banDx==0){
								...
								?>
								<input readonly type="text" name="Dx<? echo $fila8[2] ?>" id="Dx<? echo $fila8[2] ?>" style="width:40px;" value="<? if($Formato=='NOTAS EVOLUCION') echo $ValDx[$fila8[2]];?>"  onClick="BuscarDx('Dx<? echo $fila8[2]?>','DetDx<? echo $fila8[2]?>', 'Dx')">
								<input readonly type="text" name="DetDx<? echo $fila8[2] ?>"  id="DetDx<? echo $fila8[2] ?>" style="width:<? echo $Width?>;" value="<? if($Formato=='NOTAS EVOLUCION') echo $DetValDx;?>"  onClick="BuscarDx('Dx<? echo $fila8[2]?>','DetDx<? echo $fila8[2]?>','DetDx')">
						<?		$banDx=1;
							}
							else{
								...
								<input readonly type="text" name="DetDx<? echo $fila8[2] ?>" id="DetDx<? echo $fila8[2] ?>" value="<? if($DetValDx=='SIN DIAGNOSTICO'){$DetValDx='';}echo $DetValDx?>" style="width:<? echo $Width?>;" onClick="BuscarDx('Dx<? echo $fila8[2]?>','DetDx<? echo $fila8[2]?>', 'DetDx')">
						<?	}?>       
