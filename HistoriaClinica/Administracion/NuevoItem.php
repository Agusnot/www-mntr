				...
				if((ExNumRows($res)>0)||($Pantalla==1))
				{
					...
					{				
						...
						{					
							...
							if($TipoControl=="Area de Texto"||$TipoControl=="Medicamentos"||$TipoControl=="CUPS"){$ClasDat=" TEXT "; $agregarCampo = 1;}
							if($TipoControl=="Cuadro de Texto"){$ClasDat=" character varying($Longitud) "; $agregarCampo = 1;}
							if($TipoControl=="Lista Opciones"||$TipoControl=="PDF"){$ClasDat=" character varying(255) "; $agregarCampo = 1;}
							if($TipoControl=="Cuadro de Chequeo"){$ClasDat=" character varying(5) "; $agregarCampo = 1;}
							if($TipoControl=="Imagen"){$ClasDat=" TEXT "; $agregarCampo = 1;}
							if($TipoControl=="Fecha"){$ClasDat=" date "; $agregarCampo = 1;}
							if(strtoupper(TipoControl)=="MEDICAMENTOS MULTILINEA"){ $agregarCampo = 0;}
							if(strtoupper(TipoControl)=="MEDICAMENTOS UNILINEA"){ $agregarCampo = 0;}
							if(strtoupper(TipoControl)=="MEDICAMENTOS FORMULA"){ $agregarCampo = 0;}
					...
					else
					{
						...
						{					
							$NumCampo=substr("00000",0,5-strlen($IdItem)).$IdItem;
							if($TipoControl=="Area de Texto"||$TipoControl=="Medicamentos"||$TipoControl=="CUPS"){$ClasDat=" TEXT "; $cambiarCampo = 1;}
							if($TipoControl=="Cuadro de Texto"){$ClasDat=" character varying($Longitud) "; $cambiarCampo = 1;}
							if($TipoControl=="Lista Opciones"||$TipoControl=="PDF"){$ClasDat=" character varying(255) "; $cambiarCampo = 1;}
							if($TipoControl=="Cuadro de Chequeo"){$ClasDat=" character varying(100) "; $cambiarCampo = 1;}
							if($TipoControl=="Imagen"){$ClasDat=" TEXT "; $cambiarCampo = 1;}
							if($TipoControl=="Fecha"){$ClasDat=" date "; $cambiarCampo = 1;}
							if(strtoupper(TipoControl)=="MEDICAMENTOS MULTILINEA"){ $cambiarCampo = 0;}
							if(strtoupper(TipoControl)=="MEDICAMENTOS UNILINEA"){ $cambiarCampo = 0;}
							if(strtoupper(TipoControl)=="MEDICAMENTOS FORMULA"){ $cambiarCampo = 0;}
							//echo "Tipo control: $TipoControl --> Longitud: $Longitud<br>";
							if ($cambiarCampo == 1){
				...
				<div align="center">
					<form name="FORMA"  onSubmit="return validar()" method="post">	
						  <table class="tabla3"    <?php echo $borderTabla3Mentor; echo $bordercolorTabla3Mentor; echo $cellspacingTabla3Mentor; echo $cellpaddingTabla3Mentor; ?> >    
							<tr>
								<td class="encabezado2VerticalInvertido">PANTALLA</td>
								<td><input type="text" maxlength="3" name="Pantalla" style="width:30px;" value="<?echo $Pantalla?>" onKeyUp="xNumero(this)" onKeyDown="xNumero(this)" onBlur="campoNumero(this)"></td>
								<td class="encabezado2VerticalInvertido">ITEM</td>
								<td colspan="7"><input type="text" name="Item" style="width:100%;" value="<?echo $Item?>" onKeyUp="xLetra(this)" onKeyDown="xLetra(this)"></td>      
							</tr>
							<tr>
								...
								<td>
									<table width="100%" border="0" cellpadding="2" cellspacing="0">
										...
										<tr>
											<td width="40%" style="font-size:12px;font-weight:bold;text-align:left;padding-left:10px;"> Valor por defecto</td> 
											<td style="text-align:left;padding-left:10px;"> <input type="Button" class="boton2Envio" value="..." onClick="VrDefecto()"> </td>
										</tr>
									</table>	
									
								</td>
								<td class="encabezado2VerticalInvertido">TIPO DE DATO</td>
								<td>
									<select name="TipoDato" onChange="AnchoAlto()"><option></option>
										<?
								...
								<td class="encabezado2VerticalInvertido">TIPO DE CONTROL</td>
								<td colspan="5">
									<select name="TipoControl" onChange="AnchoAlto()">
										<option></option>
											<?
							...
							<tr> 	
								<td colspan="8">
									<table cellpadding="2" border="0"  width="100%" cellspacing="0"> 
										<tr>
											<td class="encabezado2VerticalInvertido" width="14%">LIMITE INFERIOR</td>
											<td style="text-align:left;padding-left:10px;" width="11%">
												<input type="text" name="LimInf" id="LimInf" style="width:30px" onKeyDown="xNumero(this)" onKeyUp="xNumero(this)"
												<? if($TipoDato=='N'){?> value="<? echo $LimInf?>"<? }else{?> disabled value="" <? }?> >
											</td>           
											
											<td class="encabezado2VerticalInvertido" width="13%">LIMITE SUPERIOR</td>
											<td style="text-align:left;padding-left:10px;" width="12%">
												<input type="text" name="LimSup" id="LimSup" style="width:30" onKeyDown="xNumero(this)" onKeyUp="xNumero(this)"
													<? if($TipoDato=='N'){?> value="<? echo $LimSup?>"<? }else{?> disabled value=""<? }?> >
											</td>   		
											<td class="encabezado2VerticalInvertido" width="5%" >ANCHO</td>
											<td style="text-align:left;padding-left:10px;" width="7%" >	
												<input type="text" name="Ancho" style="width:30" id="Ancho" onKeyDown="xNumero(this)" onKeyUp="xNumero(this)"
											...
											<td class="encabezado2VerticalInvertido" width="5%" >ALTO</td>
											<td style="text-align:left;padding-left:10px;"width="7%" >
												<input type="text" name="Alto" style="width:30" id="Alto" onKeyDown="xNumero(this)" onKeyUp="xNumero(this)"
												<?	if($TipoControl=='Area de Texto'||$TipoControl=='Medicamentos'||$TipoControl=='CUPS'||$TipoControl=='Cuadro de Texto'||$TipoControl=='Imagen')
												{?>value="<? echo $Alto?>"<? }else{?> disabled value=""<? }?>>
											</td>
											<td class="encabezado2VerticalInvertido" width="5%" >LONGITUD</td>
											<td style="text-align:left;padding-left:10px;" width="7%" ><input type="text" name="Longitud" id="Longitud" style="width:30" onKeyDown="xNumero(this)" onKeyUp="xNumero(this)"
							...
							<tr>
							  ...
							  <?		if($Modificar){?><script language="javascript">AnchoAlto();</script><? }?>
								...
								<td class="encabezado2VerticalInvertido">IMAGEN</td>
								<td><input name="Imagen" type="text" style="width:100%" value="<? echo $Imagen?>"/></td>
								<td class="encabezado2VerticalInvertido">PAR&Aacute;METROS</td>
								<td colspan="5"><input type="text" name="Parametro" style="width:100%" value="<?echo $Parametro?>" ></td>
							<tr>      
								<td class="encabezado2VerticalInvertido">TRAER DE</td>      
								<td colspan="7" style="text-align:left;padding-left:10px;">
									<table  border="0" cellpadding="2" cellspacing="0" style="text-align:justify;">
										<tr>
											<td>
												...
												<select name="TFTraerDe" onChange="frames.FrameOpener.location.href='BuscadorHC.php?DatNameSID=<? echo $DatNameSID?>&BuscaFormatos=1&TipoFormato='+this.value;">
													<option value="">&nbsp;</option>
														<?	while($fila11=ExFetch($res11)) {
															if($TFTraerDe==$fila11[0]){
																echo "<option selected value='$fila11[0]'>$fila11[0]</option>";
															}
															else{
												...
												<select name="CampoTraerDe">
													<option value="">&nbsp; </option>
														...
														$cons="Select Item from HistoriaClinica.ItemsxFormatos where TipoFormato='$TFTraerDe' and Formato='$Traerde' and Compania='$Compania[0]' 
																and estado='AC' AND (titulo <> 1 OR titulo IS NULL) Order By Pantalla,Id_Item";
														$res=ExQuery($cons,$conex);
														while($fila=ExFetch($res)){								
															if($fila[0]==$CampoTraerDe){
																echo "<option selected value='$fila[0]'>$fila[0]</option>";
															}
															else{
																echo "<option value='$fila[0]'>$fila[0]</option>";
							...
							<tr>
								<td class="encabezado2VerticalInvertido">TIP</td>
								<td colspan="4"> <input name="TIP" type="text" style="width:100%" value="<? echo $TIP?>" onKeyUp="xLetra(this)" onKeyDown="xLetra(this)"/> </td> 
								<td colspan="3"  style="text-align:justify">
									<table border="0"  cellpadding="2" cellspacing="0">
										<tr>
											<td class="encabezado2VerticalInvertido"> OBLIGAR </td>
											<td> <? if ($Obligatorio==1){?><input name="Obligatorio" type="checkbox" value="1" checked><? }else{?><input name="Obligatorio" type="checkbox" value="1"><? }?> </td>
											<td class="encabezado2VerticalInvertido">LINEA SOLA </td>
											<td><? if($LineaSola==1){?><input name="LineaSola" type="checkbox"  value="1" checked><? }else{?><input name="LineaSola" type="checkbox" value="1"><? }?> </td>
											<td class="encabezado2VerticalInvertido"> CIERRA </td>
											<td><? if($CierraFila==1){?><input name="CierraFila" type="checkbox" value="1" checked><? }else{?><input name="CierraFila" type="checkbox" value="1"><? }?> </td>
										</tr>
									</table>
								</td>		
							</tr>
							<tr>
								...
								<td colspan="7">
									<table cellpadding="2" cellspacing="0">
										<tr>
											<td class="encabezado2VerticalInvertido"> TAG </td>
											<td>
											...
											<td class="encabezado2VerticalInvertido">ETIQUETA </td>
											<td>
							...
							<tr>
								<td colspan="8" scope="row" style="text-align:center;">
									...
									{?>
									<input type="button" name="Dependencia" class="boton2Envio" value="Dependencia" onClick="Dependencias();" >
									<?
									}?>
