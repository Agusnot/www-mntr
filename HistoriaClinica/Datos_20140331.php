...
<?	}
		...
		while($fila=ExFetchArray($res))
		{
			...
			if($Alineacion=="Vertical")
			{
				...
				echo "<tr bgcolor='#e5e5e5'><td colspan='11'><strong>" . $MatMedicos[$fila['usuario']][3] . " - " . $fila['fecha'] . " - " . $fila['hora'] . "<br>";
				//echo "" .  $MatMedicos[$fila['usuario']][1]. " R.M. " .  $MatMedicos[$fila['usuario']][2]  . "<br>";
		...
		<?	}
			...
			foreach($MatItems as $IndItems)
			{
				...
				if(empty($DatCampos[$IndItems[0]])||($DatCampos[$IndItems[0]][3]==$DatCampos[$IndItems[0]][4]))	{										
					...
					{
						if($Alineacion=="Vertical")	{
							if($Mensaje=="<b> INFORMACIÓN MÉDICO </b>" || $Mensaje=="<b> INFORMACI&Oacute;N M&Eacute;DICO </b>" || $Mensaje=="INFORMACIÓN MÉDICO"){
								$extra="display:none;";
							}
							else{
								$extra="";
							...
							echo "<tr><td bgcolor='#e5e5e5' colspan='99' style='$extra $oculto'><strong><center>".$Mensaje."</td></tr>";
