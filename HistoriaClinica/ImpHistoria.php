				...
				<div align="center">
					...
					<div align="center">
							...
							while($fila=ExFetchArray($res))	{
									...
									$fechaFormato = $fila['fecha'];
									$cargoMedico = $fila['cargo'];
									$cargoMedico  = strtoupper($cargoMedico);
									...
									foreach($MatItems as $IndItems)	{
										...
										if(empty($DatCampos[$IndItems[0]])||($DatCampos[$IndItems[0]][3]==$DatCampos[$IndItems[0]][4]))	{
											...
											if (strtoupper($TipoCont) == "MEDICAMENTOS FORMULA"  ){									
																
												mostrarMedxFormula($Formato,$TipoFormato,$fila['id_historia'], $Paciente[1], $Compania[0], $IdItem);
									...
									if($UsuMed&&!$UsuCreaIgual){
										...
										<table  width="<?php echo $anchoTabla;?>" cellpadding="0" cellspacing="0"   style="border-width:0px; font : 12px Tahoma;text-align:center; vertical-align:middle;"  >
											<tr align="center">
												...
												<?php
													$RAIZ=$_SERVER['DOCUMENT_ROOT'];
														...
														if (strtoupper($Compania[0]) == "CLINICA SAN JUAN DE DIOS" and strtotime($fechaFormato) <= strtotime($fechaMigracionCSJDM)){
															
															$regMed = definirRMMedico($Compania[0],$UsuMed); 
																// Esta funcion se usa para mostrar informacion de formatos migrados
														} 
														else {
															$regMed = $fila[0];
														}
														
													$dimensiones = calcularDimensiones("$RAIZ/Firmas/$regMed.GIF",200);
											...
											<tr align="center">
												<td><center><img src='/Firmas/<? echo "$regMed.GIF"?>' <?php echo $styleImagen; ?> ></center></td>
											</tr>
											...
											<tr>
												<td style="text-align:center;">
													...
													<?php 
														if ($Compania[0] == "CLINICA SAN JUAN DE DIOS"){
															if ($cargoMedico == "PSIQUIATRA" or $cargoMedico == "MEDICO GENERAL" or  $cargoMedico == "MEDICO RESIDENTE" or $cargoMedico == "RESIDENTE" or $cargoMedico == "MEDICO RURAL"){
																$cadena = "REGISTRO MEDICO ";
															}
															else {
																$cadena = "C&Oacute;DIGO ";
															}
														} 
														else {
															$cadena = "REGISTRO MEDICO ";
