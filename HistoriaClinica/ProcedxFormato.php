	...
	// Inicia definicion de funciones
		...
		function insertarRegistro($formato,$tipoformato,$idhistoria, $cedula, $compania,$usuario, $iditem, $orden, $codprocedimiento, $nombre_procedimiento, $presentacion, $posologia, $tiempo_tratamiento){
			$cons = "INSERT INTO HistoriaClinica.Procedxformato (formato, tipo_formato, id_historia, cedula, compania, usuario, id_item, orden, cod_procedimiento, nombre_procedimiento, presentacion, posologia, tiempo_tratamiento) VALUES
			('$formato','$tipoformato','$idhistoria', '$cedula', '$compania', '$usuario', $iditem, $orden, '$codprocedimiento', '$nombre_procedimiento', '$presentacion', '$posologia', '$tiempo_tratamiento')";
			ExQuery($cons);
		}
		...
		function consultarOrden($formato,$tipoformato,$idhistoria, $cedula, $compania, $iditem){
			$cons = "SELECT MAX(orden) AS maximo FROM HistoriaClinica.Procedxformato WHERE formato = '$formato' AND tipo_formato = '$tipoformato' AND id_historia = '$idhistoria' AND cedula = '$cedula' AND compania = '$compania' AND id_item= $iditem  ";
			$res = ExQuery($cons);
		...
		function eliminarRegistro($formato,$tipoformato,$idhistoria, $cedula, $compania, $iditem, $orden){
			$cons = "DELETE FROM HistoriaClinica.Procedxformato  WHERE formato = '$formato' AND tipo_formato = '$tipoformato' AND id_historia = '$idhistoria' AND cedula = '$cedula' AND compania = '$compania' AND id_item = '$iditem' AND orden = '$orden'";
			ExQuery($cons);
		}
		...
		function contarRegistros($formato,$tipoformato,$idhistoria, $cedula, $compania, $iditem){
			$cons = "SELECT COUNT(*) AS conteo FROM HistoriaClinica.Procedxformato WHERE formato = '$formato' AND tipo_formato = '$tipoformato' AND id_historia = '$idhistoria' AND cedula = '$cedula' AND compania = '$compania' AND id_item = '$iditem'";
			$res = ExQuery($cons);
			$fila = ExFetchArray($res);
			$conteo = $fila["conteo"];
				if(!isset($conteo)){
		...
		function mostrarRegistros($formato,$tipoformato,$idhistoria, $cedula, $compania, $iditem){
			$listadoRegistros = seleccionarRegistros($formato,$tipoformato,$idhistoria, $cedula, $compania, $iditem);
				...
				while ($registro = ExFetchArray($listadoRegistros)){
					...
					echo "<tr>";
					echo "<td> <input  type='text'  name='nombre_procedimiento$registro[orden]' id='nombre_procedimiento$registro[orden]' value='$registro[nombre_procedimiento]' size='30' style='border:1px;border-style:outset;border-color:#333;' disabled > </td>";
	...
	//Termina definicion de funciones
			...
			if (isset($_GET["accion"])){
				if($_GET["accion"]=="guardar"){
					...
					$nombreprocedimiento = $_GET["nombreprocedimiento"];
					$nombreprocedimiento = normalizarCampoRegistro($nombreprocedimiento);
					...
					insertarRegistro($_GET["formato"],$_GET["tipoformato"],$_GET["idhistoria"],$_GET["cedula"],$_GET["compania"],$_GET["usuario"],$_GET["iditem"], $orden, 'NULL',$nombreprocedimiento, $presentacion,$posologia , $tiempotratamiento);
	...
	<html>
		<head>
			...
			<script language="javascript">
				function guardarMed(){
					if(document.getElementById('nombre_procedimiento').value == ''){
						document.getElementById('msjNombreProced').innerHTML= 'Campo Requerido'; 
						return false;
					} else {
						document.getElementById('msjNombreProced').innerHTML= ''
					} 
					...
					var nombreprocedimiento = document.getElementById('nombre_procedimiento').value;
					...
					window.location.href='Procedxformato.php?tipoformato=<?php echo $_GET["tipoformato"];?>&formato=<?php echo $_GET["formato"];?>&idhistoria=<?php echo $_GET["idhistoria"];?>&cedula=<?php echo $_GET["cedula"];?>&compania=<?php echo $_GET["compania"];?>&usuario=<?php echo $_GET["usuario"];?>&iditem=<?php echo $_GET["iditem"];?>&insercion=<?php echo $_GET["insercion"];?>&accion=guardar&nombreprocedimiento='+nombreprocedimiento+'&presentacion='+presentacion+'&posologia='+posologia+'&tiempotratamiento='+tiempotratamiento;
				}
			</script>
		</head>
			<?php
				if (strtoupper($_GET["insercion"])=="MULTILINEA"){
					$eventoOnLoad = 'onLoad="document.getElementById'."('nombre_procedimiento')".'.focus();"';
				} else {
					$eventoOnLoad = "";
				}
			?>
				...
				<table  width="100%"  cellspacing="0" cellpadding="5"  style="border:0px;margin-top:10px;  text-align:center;" >
					...
					<?php 
						if ($agregarLinea == 1) {
							?>
							<tr>
								<td> <input  type="text"  name="nombre_procedimiento" id="nombre_procedimiento" size="30" style="border:1px;border-style:outset;border-color:#333;" tabindex="1"> </td>
							...
							<tr> 
								<td id="msjNombreProced" style="color:#FF0000;font-size:12px;font-weight:bold;"> &nbsp; </td>
