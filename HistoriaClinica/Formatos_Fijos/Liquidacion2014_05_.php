...
<?	
	...
	else
	{
		if($NoLiquidacion&&!$Paga){
			...
			$cons="select grupo,tipo,codigo,nombre,sum(cantidad),vrunidad,vrtotal,generico,presentacion,forma,almacenppal from facturacion.detalleliquidacion where compania='$Compania[0]'
			and noliquidacion=$NoLiquidacion group by grupo,tipo,codigo,nombre,cantidad,vrunidad,vrtotal,generico,presentacion,forma,almacenppal";
			$res=ExQuery($cons);
			//echo $cons;
			if(ExNumRows($res)>0){
				...
				while($fila=ExFetch($res)){
					...
					$cons2="insert into facturacion.tmpcupsomeds 
						(compania,tmpcod,cedula,grupo,tipo,codigo,nombre,cantidad,vrund,vrtotal,generico,presentacion,forma,almacenPpal) values 
					('$Compania[0]','$TMPCOD2','$Paciente[1]','$fila[0]','$fila[1]','$fila[2]','$fila[3]',$fila[4],$fila[5],$fila[6],'$fila[7]','$fila[8]','$fila[9]','$fila[10]')";										
					//echo $cons2;
					$res2=ExQuery($cons2); 
				}
			}
...
/*echo "<br><br>".$Compania[0].",".$usuario[1].",".$ND['year']."-".$ND['mon']."-".$ND['mday']." ".$ND['hours'].":".$ND['minutes'].":".$ND['seconds'].",".$fila[0].",".$fila[1].",
					...
					if($fila[1]!="Medicamentos"){
						...
						'$CUM_CODE','$fila[3]','$fila[4]','$fila[5]','$fila[6]','$NoLiquidacion',
						'$fila[11]','$fila[12]','$fila[13]','$fila[14]','$fila[15]','$fila[16]','$fila[17]',
						'$fila[18]','$fila[19]','$fila[20]',$fila[21],'$CUM_CODE','$code')";//$fila[23]						
					}
					else{
						$cons2="insert into facturacion.detalleliquidacion 
						(compania,usuario,fechacrea,grupo,tipo,
...
/*if(!$stn){
		...
		$cons4="select codigo,nombre,sum(cantidad),vrund,sum(vrtotal),sum(nofacturable),sum(labnointerp),cum,atc from facturacion.tmpcupsomeds 
		where compania='$Compania[0]' and cedula='$Paciente[1]' and tmpcod='$TMPCOD2' and grupo='$fila5[1]'
		group by codigo,nombre,vrund,cum,atc order by codigo";			
		$res4=ExQuery($cons4);
		if(ExNumRows($res4)>0){
			$Sub=0;
			$banFac=0;
					...
					if($fila5[0]<=0)//Si tiene grupo
					{
						if($RestricCobro&&$Rescric)
						{
							...
							<?		//echo "<td align='center'>$fila4[0]</td><td align='center'>$fila4[1]";
									...
									echo "</td><td align='center'>$fila4[1]";
							...
							//echo "<td align='center'>$fila4[0]</td><td align='center'>$fila4[1]</td>";
								...
								?>
                                <td><img src="/Imgs/b_edit.png" style="cursor:hand" 
                                        onClick="EditarCoM('<? echo $fila4[0]?>','<? echo $fila4[1]?>','Cup')" title="Editar">
                                    </td>
                                    <td><img style="cursor:hand"  title="Eliminar" 
