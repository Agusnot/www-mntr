<?php
	...
	if($Trasladar){
		...
		$AutoId = $fila5[0];
                ...
                $consan="select ambito,pabellon,idcama,numservicio,ambito,pabellon from salud.pacientesxpabellones where cedula='$Paciente[1]' and estado='AC' and compania='$Compania[0]'";
                $resan=ExQuery($consan);echo ExError();
                $filaan=ExFetchAssoc($resan);
                
                // Si sale o entra a UCA queda en SIN DEFINIR MÉDICO
                $quitarmedtte='';
                if($filaan['pabellon']=='UCA' || $UndTraslado=='UCA'){
                    $quitarmedtte = ", medicotte='SIN DEFINIR MÉDICO'";
                }
		$cons="update salud.servicios set tiposervicio='$AmbitoTraslado' $quitarmedtte where compania='$Compania[0]' and cedula='$Paciente[1]' and numservicio=$AutoId";
		$res=ExQuery($cons);
...
<?php 	$cons20="select pabellon from salud.pabellones where ambito='$AmbitoTraslado' and compania='$Compania[0]' and pabellon !='$fila[1]'";
