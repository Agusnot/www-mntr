...
<?	if($Guardar)
	{
		if(!$Editar)
		{
            ...
            if(!$Paquete)
            {
                if($DosisUnica == "on"){$Du = 0;}else{$Du=1;}
                ...
                $cons = "Insert into Salud.OrdenesMedicas (Compania,Fecha,Cedula,NumServicio,Detalle,IdEscritura,NumOrden,Usuario,TipoOrden,Acarreo,posologia,DosisUnica,viasumin) values
                ('$Compania[0]','$ND[year]-$ND[mon]-$ND[mday] $ND[hours]:$ND[minutes]:$ND[seconds]','$Paciente[1]',$NoServicio,
                '$TextoOrden',$IdEscritura,$NoOrden,'$usuario[1]','Sedaciones',1,'$Nota',$Du,'$ViadeSum')";
                $res = ExQuery($cons);
                ...
                CedPaciente,FechaIni,FechaFin,CantDiaria,ViaSuministro,Justificacion,Notas,NumServicio,Detalle,TipoMedicamento,
                            posologia,numOrden,idescritura,dosisunica)
                ...
                '$ND[year]-$ND[mon]-$ND[mday]',NULL,$CantDiaria,'$ViadeSum','$Justificacion','$Notas',$NoServicio,'$TextoOrden',
                            'Medicamento Urgente','$Nota',$NoOrden,$IdEscritura,$Du)"; //Falta la Nota
                ExQuery($cons);
                if(!$POS){
            ...
            <?	}
                ...
                else
                {
                    ...
                    {
                        ?><script language="javascript">
                            location.href="OMSedaciones.php?DatNameSID=<? echo $DatNameSID?>&IdEscritura=<? echo $IdEscritura++?>";
                        </script><?
                    }
                }
            }
            else
            {
                ...
                {
                    ?><script language="javascript">
                        location.href="OMSedaciones.php?DatNameSID=<? echo $DatNameSID?>&Origen=Paquetes&IdPaq=<?echo $IdPaq?>&Paquete=<?echo $Paquete?>&IdPaquete=<?echo $IdPaq?>&Entidad=<?echo $Entidad?>&Contrato=<?echo $Contrato?>&NoContrato=<? echo $NoContrato?>";
                    </script><?
                }
            }
		}
...
<script language="javascript">
	...
	{
		frames.FrameOpener.location.href="Ampollas.php?DatNameSID=<? echo $DatNameSID?>&Formulacion=Urgentes&Paquete=<?echo $Paquete?>&Entidad=<?echo $Entidad?>&Contrato=<?echo $Contrato?>&NoContrato=<?echo $NoContrato?>";
...
<input type="hidden" name="IdPaq" value="<?echo $IdPaq?>" />
	<table rules="groups" width="60%" align="center" cellpadding="2" border="1" bordercolor="#e5e5e5" style='font : normal normal small-caps 13px Tahoma;'>
    	<tbody><tr>
        	<td colspan="4" bgcolor="<? echo $Estilo[1]?>" style="color:white" align="center"><strong>MEDICAMENTO <?echo strtoupper($Paquete)?></strong></td>
        </tr></tbody>
        <tbody><tr>
        	<td colspan="4" align="center"><input type="Text" name="Medicamento" value="<? echo $Medicamento?>" readonly size="90" style="text-align:center"/>
        ...
        <? if($Medicamento)
		{
			...
			?><tbody><tr bgcolor="#e5e5e5" align="center">
            	<td width="40%">Cantidad</td><td width="60%">Posologia</td>
			</tr>            
            <tr align="center">
            <? 	/*
