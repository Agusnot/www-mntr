								...
								$fila2=ExFetch($res2);
									...
									if($fila2[0]==1) {
										...
										if(($fila3[0])){
											...
											<td><input type="radio" name="Opc" onClick="location.href='OMProcedimientos.php?DatNameSID=<? echo $DatNameSID?>&IdEscritura=<? echo $IdEscritura?>'"/> 
...
<?php	if($fila[1]){
		...
		$cons3="select detalle,tipoorden,idescritura,numorden,posologia,viasumin,fechareprog,consistenciaDieta,observacionDieta,numservicio,fecha,tipodosis from salud.ordenesmedicas where cedula='$Paciente[1]' and estado='AC' 
		order by idescritura desc,numorden desc";
		...
		if(ExNumRows($res3)>0){   	?>
			...
			while($fila3=ExFetch($res3)){
				$consNota="select notas,justificacion from salud.plantillamedicamentos where numservicio='$fila3[9]' and idescritura='$fila3[2]' and fechaformula='$fila3[10]'";
				$resNota=ExQuery($consNota);
				$filaNota=ExFetch($resNota);
                                $consJu="select justificacion from salud.plantillanotas where numservicio='$fila3[9]' and idescritura='$fila3[2]' and fechaini='$fila3[10]'";
				...
				if($filaNota2[0]){
					echo "$fila3[0]. $fila3[7] $fila3[8] - $filaNota[0] $filaNota[1] $filaJu[0] - $fila3[11]";
				} ELSE{
					echo "$fila3[0]. $fila3[7] $fila3[8] $fila3[4] - $filaNota[0] $filaNota[1] $filaJu[0] - $fila3[11]"; 
				}
				if($fila3[5]){
					echo "Via $fila3[5] ";
				}
