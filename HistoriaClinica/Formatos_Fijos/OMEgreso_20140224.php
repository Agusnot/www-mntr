...
//-----------------------------------------------------------------------Insercion en la tabla ordenes medicas----------------------------------------------------------------------------
		$Detalle="Egreso paciente por: ".$MotivoSalida." - ".$Observaciones;
		...
		('$Compania[0]','$ND[year]-$ND[mon]-$ND[mday] $ND[hours]:$ND[minutes]:$ND[seconds]','$Paciente[1]',$NumServ,'$Detalle',$IdEscritura,$AutoId,'$usuario[1]','Orden Egreso','AC',1)";				
		$res = ExQuery($cons);echo ExError($res);
		//echo $cons;
