<?php
	...
	function ActualizaMpo($Dpto){
        ...
        function ActualizaMpoNac($departamentonac){
		$respuesta=new xajaxResponse();	
		$respuesta->addScript("alert('Respondo!');");
		return $respuesta->getXML();
	}
	...
	$obj->registerFunction("ActualizaMpo");
        $obj->registerFunction("ActualizaMpoNac");
	...
	{		
		...
		if(document.FORMA.Sexo_.value==1){
			if(document.FORMA.Sexo.value=="")
			{
                ...
                if(document.FORMA.departamentonac_.value==1){
			if(document.FORMA.departamentonac.value=="")
			{
				alert("Seleccione el departamento de nacimiento del paciente");
				document.FORMA.departamentonac.focus();			
				return false;
			}
		}
		if(document.FORMA.municipionac_.value==1){
			if(document.FORMA.municipionac.value=="")
			{
				alert("Seleccione el municipio de nacimiento del paciente");
				document.FORMA.municipionac.focus();			
				return false;
			}
		}
...
$ND=getdate();
	if($BotonG)
	{
		...
		if(ExNumRows($resultado)==0)
		{
			...
			docacompanante,expcedacomp,diracompanante,telacompanante,parentescoacomp,instresponsable,raza,tipodocac,departamentonac,municipionac)
					...
					'$telacompanante', '$parentescoacomp', '$InstResponsable', '$Raza', '$tipodocac', '$departamentonac', '$municipionac')";
				$resultado = ExQuery($cons,$conex);
		}
...
//			$cons=$cons . "LugarExp='$LugarExp',";
            ...
            $cons=$cons . "departamentonac='$departamentonac',";
            $cons=$cons . "municipionac='$municipionac'";
...
?>
    ...
    <tr>
    <td>Departamento Nacimiento <? if($Aux['departamentonac_']==1){?><font color="red">*</font><? }?></td><td>

<select name="departamentonac" style="width:180px;" onChange="xajax_ActualizaMpoNac(this.value);BuscaMpoNac.location.href='BuscarMunicipios.php?departamentonac='+this.value+'&DatNameSID=<? echo $DatNameSID?>'">
<option value=""></option>
<?php
...
<td>Municipio Nacimiento<? if($Aux['municipionac_']==1){?><font color="red">*</font><? }?></td><td>

<select name="municipionac" style="width:180px;" onChange="BuscaVereda.location.href='BuscarVereda.php?Depto='+departamentonac.value+'&Mpo='+this.value+'&DatNameSID=<? echo $DatNameSID?>'">
<option value=""></option>
<?php echo $Paciente[73];
	$cons="Select municipio from Central.Municipios,Central.Departamentos where Departamentos.Departamento='$Paciente[73]'
...
<? 
if(!$departamentonac){$departamentonac=$Paciente[75];}
if(!$municipionac){$municipionac=$Paciente[76];}?>
</td></tr>
...
<td>Departamento residencia <? if($Aux['Departamento_']==1){?><font color="red">*</font><? }?></td><td>
...
<td>Municipio Residencia <? if($Aux['Municipio_']==1){?><font color="red">*</font><? }?></td><td>
	...
	<td><input style="width:180px;" type="Text" class="Texto" name="docacompanante" size="500" value=<?php echo "'$Paciente[65]'" ?> onKeyUp="xLetra(this)" onKeyDown="xLetra(this)"></td>
        <td>Tipo Documento <? if($Aux['Tipo_Doc_Ac_']==1){?><font color="red">*</font><? }?></td><td>
            ...
            <?php
                            ...
                            {
                                    if(!$Paciente[72]){
                                        $Paciente[72]="Cedula de ciudadania";
                                    }
                                    if($Paciente[72]==$fila[0])
                                    {
                                            echo "<option value='$fila[0]' selected>$fila[0]</option>";
                                    }
                                    else
...
<iframe name="BuscaMpoNac" id="BuscaMpoNac" src="BuscarMunicipios.php?DatNameSID=<? echo $DatNameSID?>" style="visibility:hidden;position:absolute;top:1px"></iframe>
