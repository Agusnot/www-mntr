	...
	if($Guardar)
	{
		if($Cup){
			...
			while( list($cad,$val) = each($Cup))
			{
				if($cad && $val)
				{    
					$cons="insert into salud.tmpcupsordenesmeds (compania,usuario,cedula,tmpcod,cup,finalidadcup,tipofinalidad,formaquirurgica,cantidad,nota,justificacion) values 
				('$Compania[0]','$usuario[1]','$Paciente[1]','$TMPCOD2','$cad','$FinalidadProc[$cad]','$TipFinalidad[$cad]','$FormaActQuir[$cad]','$CantCup[$cad]','$nota_proc[$cad]','$just_proc[$cad]')";
					$res=ExQuery($cons);
					//echo $cons;
				}
			}
...
<body background="/Imgs/Fondo.jpg">
    <script lang="JavaScript">
            function validarnoju(codi){
                var nota = document.getElementById("nota_proc["+codi+"]");
                var justificacion = document.getElementById("just_proc["+codi+"]");
                
                if(justificacion.value==""){
                    alert("Debe ingresar justificación!!!");
                    document.getElementById("Cup_"+codi).checked=false;
                }
                else{
...
<tr align="center" bgcolor="#e5e5e5" style="font-weight:bold">
    ...
    <td style="display:none;">Forma Acto Quirurgico</td>
    <td>Notas</td>
    <td>Justificación</td>
    <td>Seleccionar</td></tr><?
	...
	while($fila3=ExFetch($res3))
	{
    	...
    	<tr onMouseOver="this.bgColor='#AAD4FF'" onMouseOut="this.bgColor=''" style="cursor:hand" >
    		<td><? echo "$fila3[0]";?></td>
                <td><? echo $fila3[1]?></td>
                <td style="display:none;"><? echo $NP?></td>
            ...
            <?	if(!$fila3[10]){
                    ...
                    <select name="FormaActQuir[<? echo $fila3[0]?>]" id="FormaActQuir_<? echo $fila3[0]?>">
                        <option value="6">Sin definir</option>
                    <?	while($fila=ExFetch($res))
                                            {
            ...
            <td><textarea name="nota_proc[<? echo $fila3[0]?>]" id="nota_proc[<? echo $fila3[0]?>]" cols="25" rows="3"></textarea></td>
            <td><textarea name="just_proc[<? echo $fila3[0]?>]" id="just_proc[<? echo $fila3[0]?>]" cols="25" rows="3"></textarea></td>
            <td>
            <input type="checkbox" name="Cup[<? echo $fila3[0]?>]" id="Cup_<? echo $fila3[0]?>"    
    <?
	$bandera="";	
				//Se comenta para poder ordenar cualquier procedimiento
		...
		<?		}*/
				...
				{?>
					onclick="alert('Este item no ha sido aun configurado adecuadamente para ser seleccionado!!!');
					if(getElementById('Cup_<? echo $fila3[0]?>').checked==true){getElementById('Cup_<? echo $fila3[0]?>').checked=false;}"
			<?	}*/
				if($fila3[6]){
					if(!$Sexo){$bandera="bandera";?>
                    	onclick="alert('alert('Este CUP requiere que se registre el genero del paciente!!!');
							if(getElementById('Cup_<? echo $fila3[0]?>').checked==true){getElementById('Cup_<? echo $fila3[0]?>').checked=false;}"	
				<?	}
					elseif($Sexo!=$fila3[6]){$bandera="bandera";
						if($Sexo=='M'){?>
				...
				elseif($fila3[8]){	
					if(!$Edad&&$Edad!="0"){$bandera="bandera";?>
						...
						if(getElementById('Cup_<? echo $fila3[0]?>').checked==true){getElementById('Cup_<? echo $fila3[0]?>').checked=false;}"
				<?	}
					elseif($Edad>$fila3[8]){$bandera="bandera";?>
				...
				if($bandera==""){
					?>
                    onclick="validarnoju('<? echo $fila3[0]?>');"
                    <?
				}
				/*else{?>
				      	onclick="
