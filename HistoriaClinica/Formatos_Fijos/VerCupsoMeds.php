...
<script language="javascript">	
	...
	function Asignar(C,N,G,T,V,Gen,P)
	{
		...
		if(document.FORMA.TipoNuevo.value=="Cup"){
			...
			parent.document.FORMA.CodCoM.value=C;
			parent.document.FORMA.Nombre.value=N;
			parent.document.FORMA.VrUnidad.value=V;
...
if($CodCoM!=''||$Nombre!='')
{
	if($TipoNuevo=="Cup")
	{
		...
		if($Nombre==''){					
			...
			where codigo=cupsxplanservic.cup and cupsxplanservic.cup=cupsxplanes.cup and codigo ilike '$CodCoM%' and cupsxplanes.compania='$Compania[0]' and cups.compania='$Compania[0]'
			...
			if($CodCoM==''){		
				...
				$cons3="select cups.codigo,cups.nombre,cups.grupo,cups.tipo,cupsxplanes.valor,reqvobo,facturable,minimos,maximos
				from contratacionsalud.cupsxplanservic,contratacionsalud.cups,contratacionsalud.cupsxplanes 			
				where codigo=cupsxplanservic.cup and cupsxplanservic.cup=cupsxplanes.cup and codigo ilike '$CodCoM%' and nombre ilike '$Nombre%'
...
<?			}?>
	...
	{		
		...
		if($Nombre==''){
			...
			and CodProductos.Compania='$Compania[0]' and TiposdeProdxFormulacion.AlmacenPpal='$AlmacenPpal' and CodProductos.Anio=$ND[year] and Codigo1 like '$CodCoM%'
