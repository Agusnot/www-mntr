<?php
	...
	and usuario='$usuario[1]' and vistobuenoaux=1 and medicos.cargo=cargos.cargos";	
	$res=ExQuery($cons);
	$fila=ExFetch($res);
	$PermiteNNota=$fila[0];		
	if(!$PermiteNNota){$Disa="disabled";}	
...
<center>
<font  style="text-transform:uppercase"><strong>NOTAS DE CAMBIO DE TURNO <? echo $SelUnidad?></strong></font>
</center>
...
<?	$cons="Select numero,mes from central.meses";	
	...
	<td><input type="Button" value="Agregar Registro" onClick="if(document.FORMA.SelUnidad.value!=''){location.href='NuevaNotaCambio.php?&DatNameSID=<? echo $DatNameSID?>&SelUnidad=<? echo $SelUnidad?>'}else{alert('Seleccione la unidad');}" title="Crear Nota con Fecha de Hoy" <? echo $Disa;?>>				</td>
	</tr>
</table>
<table  border="1" bordercolor="#e5e5e5" style='font : normal normal small-caps 11px Tahoma;' width="100%">
<?
$cons = "Select fecha,usuario,nota,unidad From HistoriaClinica.NotasCambioTurno where Compania='$Compania[0]' and unidad='$SelUnidad' and date(Fecha)='$Anio-$Mes-$Dia' Order By Fecha Desc;";
//echo $cons;
$res = ExQuery($cons);
while($fila=ExFetch($res))
{
