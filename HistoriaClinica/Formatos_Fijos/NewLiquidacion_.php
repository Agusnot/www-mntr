...
<?
	if($Guardar){
		...
		if($Edit){
			$cons2="delete from facturacion.tmpcupsomeds where compania='$Compania[0]' and cedula='$Paciente[1]' and codproducto='$CodCoM' and tmpcod='$TMPCOD'";
			$res=ExQuery($cons2);
		}
			$cons="select codigo,cantidad,vrund from facturacion.tmpcupsomeds where compania='$Compania[0]' and cedula='$Paciente[1]' and codproducto='$CodCoM' and tmpcod='$TMPCOD'";
			$res2=ExQuery($cons2);
			if(ExNumRows($res2)>0){				
				...
				where compania='$Compania[0]' and cedula='$Paciente[1]' and codproducto='$CodCoM' and tmpcod='$TMPCOD'";
				//$res=ExQuery($cons);
			}
			else{
			//echo "<td>$TipoNuevo</td></br>";
			//echo "<td>$Grupo</td></br>";
				if($TipoNuevo=="Cup"){
					$cons="insert into facturacion.tmpcupsomeds (compania,tmpcod,cedula,grupo,tipo,codigo,nombre,cantidad,vrund,vrtotal,ambito,finalidad,causaext,dxppal,tipodxppal,fecha,codproducto) 
					values ('$Compania[0]','$TMPCOD','$Paciente[1]','$Grupo','$Tipo','$CodCoM','$Nombre',$Cantidad,$VrUnidad,$VrTotal,$Ambito,'$FinalidadProc','$CausaExterna'
					,'$CodDiagnostico1','$TipoDx','$Fecha 00:00:00','$CodCoM')";
					...
					$consMED="select codproductos.codigo2,codproductos.nombreprod1,codproductos.unidadmedida,codproductos.presentacion,movimiento.cum 
					from consumo.codproductos, consumo.movimiento 
					where codproductos.autoid='$CodCoM' and movimiento.autoid='$CodCoM' and movimiento.cedula='$Paciente[1]' and movimiento.almacenppal='FARMACIA'
					and codproductos.almacenppal='FARMACIA'
					group by movimiento.autoid,movimiento.cum,codproductos.codigo2,codproductos.nombreprod1,codproductos.unidadmedida,codproductos.presentacion";
					$resMED=ExQuery($consMED);
					if(ExNumRows($resMED)>0){	
						if($filaMED=ExFetch($resMED)){
							$Codigo1=$filaMED[0];
							$ATC=$filaMED[4];
						}
					}else{
						//$Grupo=$GruposMeds[$Grupo];
						$consMED="select codigo2,nombreprod1,unidadmedida,presentacion from consumo.codproductos where autoid='$CodCoM' and almacenppal='FARMACIA'";
						$resMED=ExQuery($consMED);
						if($filaMED=ExFetch($resMED)){$Codigo1=$filaMED[0];$ATC=$Codigo1;}
					}
					$cons="insert into facturacion.tmpcupsomeds 
					(compania,tmpcod,cedula,grupo,tipo,codigo,nombre,cantidad,vrund,vrtotal,generico,presentacion,forma,almacenPpal,ambito,fecha,atc,cum,codproducto) values 
					('$Compania[0]','$TMPCOD','$Paciente[1]','$Grupo','Medicamentos','$Codigo1','$Nombre',$Cantidad,$VrUnidad,$VrTotal,'$filaMED[1]','$filaMED[2]','$filaMED[3]','$AlmacenPpal',
					$Ambito,'$Fecha 00:00:00','$Codigo1','$ATC','$CodCoM')";
	...
	//echo "<td>$TipoNuevo</td></br>";
		...
		echo$cons="select grupo,cantidad,vrund,vrtotal,generico,presentacion,forma,almacenppal,tipo,codigo $Mas from facturacion.tmpcupsomeds 
		where tmpcod='$TMPCOD' and cedula='$Paciente[1]' and compania='$Compania[0]' and codproducto='$CodCoM'";// $Mas2";		
		//echo $cons;
		$res=ExQuery($cons);
		$fila=ExFetch($res);
		$Codigo=$fila[9]; $Nombre=$NomCoM; $Cantidad=$fila[1]; $VrUnidad=$fila[2]; $VrTotal=$fila[3]; $Generico=$fila[4]; $Presentacion=$fila[5]; $Forma=$fila[6]; $Grupo=$fila[0];
		$Tipo=$fila[8]; $AlmacenPpal=$fila[7];
		$FinalidadProc=$fila[10]; $CausaExterna=$fila[11]; $TipoDx=$fila[13]; $CodDiagnostico1=$fila[12];
		
		if($fila[12]){
			$cons="select diagnostico from salud.cie where codigo='$fila[12]'"; 
			$res=ExQuery($cons); 
			$fila=ExFetch($res); 
			$NomDiagnostico1=$fila[0];
		}
		
		$consCant="select codigo,sum(cantidad),sum(vrtotal) from facturacion.tmpcupsomeds where tmpcod='$TMPCOD' and cedula='$Paciente[1]' and compania='$Compania[0]' and codproducto='$CodCoM'
		group by codigo";
		//echo $consCant;
...
<script language="javascript">
	...
	{		
		frames.FrameOpener.location.href="VerCupsoMeds.php?DatNameSID=<? echo $DatNameSID?>&CodCoM="+document.FORMA.CodCoM.value+"&Nombre="+document.FORMA.Nombre.value+"&Pagador=<? echo $Paga?>&Contrato=<? echo $PagaCont?>&NoContrato=<? echo $PagaNocont?>&TipoNuevo=<? echo $TipoNuevo?>&AlmacenPpal="+document.FORMA.AlmacenPpal.value;
	...
	function Validar()
	{
		if(document.FORMA.CodCoM.value==""){alert("Debe digitar el codigo!!!");return false;}
...
<input type="text" name="TMPCOD" value="<? echo $TMPCOD?>">
...
<input type="hidden" name="CodigoAnt" value="<? echo $CodCoM?>">
...
<table border="1" bordercolor="#e5e5e5" style='font : normal normal small-caps 12px Tahoma;' align="center"> 
	...
	<tr>
        ...
        <td><input type="text" name="CodCoM" onKeyDown="xLetra(this)" onKeyUp="xLetra(this);AsitenteNew('<? echo $TipoNuevo?>')" 
        	onKeyPress="AsitenteNew('<? echo $TipoNuevo?>')" onFocus="AsitenteNew('<? echo $TipoNuevo?>')" style="width:90px" value="<? echo $CodCoM?>"></td>    
