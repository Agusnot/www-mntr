...
<?		$cont=1;
    ...
    <table bordercolor="#e5e5e5" border="1"  cellpadding="2" cellspacing="2"style="font : normal normal small-caps 11px Tahoma;" align="center">
 	<?	$cons="select count(usuario),usuario,nombre from salud.agenda,central.usuarios
		where agenda.compania='$Compania[0]' and fecha>='$Anio-$C1$MesIni-$C2$DiaIni' 
		and fecha<='$Anio-$C1$MesFin-$C2$DiaFin' and usuarios.usuario=agenda.medico 
		group by usuario,usuario,nombre  order by nombre";
		$res=ExQuery($cons);
		//echo $cons;
		while($fila=ExFetch($res))
		{
			$Agenda[$fila[1]]=array($fila[2],$fila[0]);
		}
		$cons="select count(usuario),usuario,nombre from salud.agenda,central.usuarios
		where agenda.compania='$Compania[0]' and fecha>='$Anio-$C1$MesIni-$C2$DiaIni' 
		and fecha<='$Anio-$C1$MesFin-$C2$DiaFin' and usuarios.usuario=agenda.medico  and (agenda.estado='Activa' or agenda.estado='Atendida') 
		group by usuario,usuario,nombre";
		//echo $cons;
		$res=ExQuery($cons);
		$banR=0;
		while($fila=ExFetch($res))
		...
		$cons="select count(usuario),usuario,nombre from salud.agenda,central.usuarios
		where agenda.compania='$Compania[0]' and fecha>='$Anio-$C1$MesIni-$C2$DiaIni' 
		and fecha<='$Anio-$C1$MesFin-$C2$DiaFin' and usuarios.usuario=agenda.medico and agenda.estado='Cancelada'
		group by usuario,usuario,nombre";
		$res=ExQuery($cons);
		while($fila=ExFetch($res))
		{
		...
		$cons="select count(usuario),usuario,nombre from salud.agenda,central.usuarios
		where agenda.compania='$Compania[0]' and fecha>='$Anio-$C1$MesIni-$C2$DiaIni' 
		and fecha<='$Anio-$C1$MesFin-$C2$DiaFin' and usuarios.usuario=agenda.medico and agenda.estado='Pendiente' group by usuario,usuario,nombre";
		$res=ExQuery($cons);
		while($fila=ExFetch($res))
		{
		...
		?>			
        <tr bgcolor="#e5e5e5" style="font-weight:bold" align="center">
            <td>Medico</td><td>Prog</td><td colspan="2">Real</td><td colspan="2">Canc</td><td colspan="2">No Real</td>
        </tr>	
<?		foreach($Agenda as $Agend)
        {	
            if(!$Agend[1]){$Agend[1]="0";}if(!$Agend[2]){$Agend[2]="0";}if(!$Agend[3]){$Agend[3]="0";}if(!$Agend[4]){$Agend[4]="0";}if(!$Agend[5]){$Agend[5]="0";}
