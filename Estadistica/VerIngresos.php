...
<?		$cons="select primape,segape,primnom,segnom,identificacion,eps,servicios.fechaing,servicios.fechaegr,servicios.numservicio,servicios.estado,sexo,fecnac		
		...
		servicios.fechaing<='$Anio-$MesFin-$DiaFin' and servicios.tiposervicio='$Ambito'
		...
		if(ExNumRows($res)>0){
     	...
     	<?	while($fila=ExFetch($res)){
				...
				if($fila[9]=='AC'){$Estado='Activo';}else{$Estado='Inactivo';}
                echo "<td>$cont</td><td>$fila[0] $fila[1] $fila[2] $fila[3]</td>";?>
				<td style="cursor:hand" title="Ver Historia Clinica" 
                onClick="location.href='/HistoriaClinica/ResultBuscarHC.php?DatNameSID=<? echo $DatNameSID?>&Cedula=<? echo $fila[4]?>&Buscar=1'">
			<?	echo "$fila[4]</td><td align='center'>$FechaIng</td><td align='center'>$FechaEgr</td>
				...
				echo "</td><td>$fila5[0]</td><td>$fila[10]</td><td>".ObtenEdad($fila[11]);				
			}	?>	
            <tr><td colspan="11">&nbsp;</td></tr>
            <tr><td colspan="11"><strong>Total Ingresos Periodo: <? echo $cont?></strong></td></tr>
	<?	}
		else{?>
			<tr><td>No Hay Igreso Pacientes Durante Este Periodo</td></tr>
<?		}?>
	</table><?
}?>
</body>
