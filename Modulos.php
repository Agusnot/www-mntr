<?php //REVISIÓN 001
	...
	while ($fila=ExFetch($res))
	{
		...
		if(ExNumRows($res3)>0){
			$BanUsus=1;
			echo $cons4="select usuario from alertas.usuariosxalertas where compania='$Compania[0]' and idalerta=$fila[4] and usuario='$usuario[1]'";
			$res4=ExQuery($cons4);
			if(ExNumRows($res4)>0){
				$BanUsuSi=1; 				
			}
	...
	where Modulo=Perfil and Nivel=0 and Usuario='$usuario[1]' and Compania='$Compania[0]'  and ind_estado = '0' Order By Id";
	$res=ExQuery($cons);
...
body{background-image: url(<?php //REVISIÓN 001
 echo"$root"; //REVISIÓN 001
 ?>/Imgs/Izquierda.jpg); background-repeat:no-repeat;}<?php //REVISIÓN 001
 	}?>
</style>
<?php //REVISIÓN 001
...
<?php //REVISIÓN 001
	...
	if(ExNumRows($res2)==null)
	{
                echo "";
                echo "<ul id='cssmenu'>";
                echo "<li><a target='Derecha' href='".$root."/ModOpciones.php?DatNameSID=$DatNameSID'><img src='".$root."/Imgs/1.png' alt='IR AL INICIO' style='width:20px; border:0px;'></a></li>";
                echo "<li><img src='".$root."/Imgs/2.png' alt='HERRAMIENTAS ADMINISTRATIVAS' style='width:20px; border:0px;'></li>";
				echo "<li><a target='Derecha' href='".$root."/Administracion/CambioClave.php?DatNameSID=$DatNameSID'><img src='".$root."/Imgs/3.png' alt='CAMBIAR LA CLAVE' style='width:20px; border:0px;'></a></li>";
                echo "<li><img src='".$root."/Imgs/4.png' alt='AYUDA' style='width:20px; border:0px;'></li>";
                echo "<li><a href='CerrarSesion.php?CerrarSesion=1' target='_top'><img src='".$root."/Imgs/5.png' alt='CERRAR SESIÓN' style='width:20px; border:0px;'></a></li>";
		...
		if($filaExtra[0]==1){
                    echo "<li><img src='".$root."/Imgs/botonparpadea.gif' alt='SITUACION EXTRAORDINARIA' style='width:20px; border:0px;'></li>";
		...
		if($final[0] == 'ADMINISTRADOR'){
			echo "<li><a target='Derecha' href='".$root."/Errores/general.php?DatNameSID=$DatNameSID'><img src='".$root."/Imgs/errores.png' alt='ERRORES DEL SISTEMA' style='width:20px; border:0px;'></a></li>";
		}
                echo "</ul>";
	}
...
<br><br><br><br><span style="color:#0098d8; padding-left:30px;">SELECCIONE APLICACIÓN</span><br><br>
...
var carpeab = '/carpabiertat_.gif'
var carpece = '/carpcerradat_.gif'
...
<?php //REVISIÓN 001
	...
	while($fila=ExFetch($res))
	{
		...
		and ModuloGr='$fila[0]' and ind_estado = '0' Order By Id";
		$res1=ExQuery($cons1);
  ...
  while($fila1=ExFetch($res1))
		{
			...
			where Modulo=Perfil and AccesoxModulos.Madre='$fila1[0]' and UsuariosxModulos.Madre='$fila[0]' and Usuario='$usuario[1]' and ModuloGr='$fila[0]'
                        and Compania='$Compania[0]' and ind_estado = '0' Order By Id";
			$res2=ExQuery($cons2);
...
<?php //REVISIÓN 001
			...
			while($fila2=ExFetch($res2))
			{
				...
				where Modulo=Perfil and AccesoxModulos.Madre='$fila2[0]' and UsuariosxModulos.Madre='$fila[0]' and Usuario='$usuario[1]' and Compania='$Compania[0]' 
				and ModuloGr='$fila[0]' and ind_estado = '0' Order By Id";
				$res3=ExQuery($cons3);
...
<div style="z-index: -9; background-color: #002147; width: 120%; height: 50px; position: absolute; top: 0px; right: 0px;"></div>
