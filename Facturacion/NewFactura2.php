...
<script language="javascript">	
	...
	function Total()
	{
		document.FORMA.VrTotal.value=parseFloat(document.FORMA.Cantidad.value)*parseFloat(document.FORMA.VrUnd.value);
	}
...
<?
	if($Guardar)
	{
		if(!$Edit) {
			...
			{
				$cons="select grupofact from consumo.grupos where grupo='$Grupo' and almacenppal='$AlmacenPpal' and compania='$Compania[0]'";
				$res=ExQuery($cons);
				$fila=ExFetch($res);
				if($fila[0]){$Grupo=$fila[0];}
				$consMED="select codigo2 from consumo.codproductos where autoid='$Codigo' and almacenppal='FARMACIA'";
				$resMED=ExQuery($consMED);
				while($filaMED=ExFetch($resMED)){
				$Codigo=$filaMED[0];}
...
<table BORDER=1  style='font : normal normal small-caps 12px Tahoma;' border="1" bordercolor="#e5e5e5" cellpadding="2" align="center">  
    ...
    <tr>
    	...
    	<td><input type="text" name="VrUnd" value="<? echo $VrUnd?>" style="width:80; text-align:right"
		onKeyDown="xNumero(this)" onKeyPress="xNumero(this)" onKeyUp="xNumero(this);Total()"></td>
        <td bgcolor="#e5e5e5" style="font-weight:bold" align="center">Vr Total</td>
  	<?	if(!$VrTotal){$VrTotal=$VrUnd*$Cantidad;}?>
    	<td><input type="text" readonly name="VrTotal" value="<? echo $VrTotal?>" style="width:80; text-align:right"
		onKeyDown="xNumero(this)" onKeyPress="xNumero(this)" onKeyUp="xNumero(this);Total()"></td>
