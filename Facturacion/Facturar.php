<?	
	...
	if($Facturar){
		...
		$cons2="select numero from contabilidad.movimiento where comprobante='Venta de servicios' order by numero desc";
		$res2=ExQuery($cons2); $fila2=ExFetch($res2);
		if($fila[0]>$fila2[0]){$AutoId=$fila[0]+1;/*Numero de factura*/}
		if($fila2[0]>$fila[0]){$AutoId=$fila2[0]+1;/*Numero de factura*/}
		if($fila2[0]==$fila[0]){$AutoId=$fila2[0]+1;/*Numero de factura*/}
		$AutoIdIni=$AutoId;	
