<?	
	...
	?>	<script language="javascript">
			location.href="VerRips.php?DatNameSID=<? echo $DatNameSID?>&FechaIni=<? echo $FechaIni?>&FechaFin=<? echo $FechaFin?>&Entidad=<? echo $Entidad?>&Contrato=<? echo $Contrato?>&NoContrato=<? echo $NoContrato?>&FecCortes=<? echo $FecCortes?>";
        </script><?
	}
?>	
...
<script language="javascript">
	function Validar()
	{
		...
		else if(document.FORMA.FechaFin.value==""){
			alert("Debe seleccionar la fecha final!!!"); 
		}
		else if(document.FORMA.FechaIni.value>document.FORMA.FechaFin.value){
			alert("La fecha inicial debe ser menor a la fecha final!!!"); 
		}
		else if(document.FORMA.Entidad.value==""){	
			alert("Debe selecionar una entidad!!!");
		}		
		else{
			document.FORMA.Generar.value=1;
...
<table BORDER="1"  style='font : normal normal small-caps 12px Tahoma;' bordercolor="#e5e5e5" cellpadding="2" align="center">
    ...
    <tr>
  	...
  	<?	$cons="Select numero from contratacionsalud.contratos
        ...
        <td colspan="2"><strong>Fechas Hospitalizacion iguales a las de corte</strong>
        	<input type="checkbox" name="FecCortes" <? if($FecCortes){?> checked<? }?>>
        </td>
    </tr>
    <? /*
    <tr>
    ...
    <tr align="center">
    	<td colspan="8"><input type="button" value="Generar" onClick="Validar()"></td>
    </tr>
</table>
<input type="hidden" name="Generar">
</form>
</body>
<iframe frameborder="0" id="VerFacturado" src="VerFacturado.php?DatNameSID=<? echo $DatNameSID?>&FechaIni=<? echo $FechaIni?>&FechaFin=<? echo $FechaFin?>&Entidad=<? echo $Entidad?>&Contrato=<? echo $Contrato?>&NoContrato=<? echo $NoContrato?>&Ambito=<? echo $Ambito?>&FecCortes=<? echo $FecCortes?>" width="100%" height="85%"></iframe>
</html>
