	...
	for($i=$FacIni;$i<=$FacFin;$i++){	
		...
		if($filaFormat[0]==$Formato){
			...
			if($fila[9]==1){				
				...
				$DatosLiq[$NoFac]=array($fila2[0],$fila2[1],$fila2[2],$fila2[3],$fila2[4],$fila2[5],$fila[6],$fila2[7],$fila2[8],$fila2[9],$filaCopoCuota[0]);				
			}	
			
			$cons2="select grupo,tipo,sum(cantidad),vrunidad,almacenppal,codigo,nombre,generico,presentacion
			from facturacion.detallefactura where compania='$Compania[0]' and nofactura=$fila[14] and vrunidad!=0
			...
			while($fila2=ExFetch($res2)){
				...
				$DetalleFac[$Id][$NoFac]=array($fila2[0],$fila2[1],$fila2[2],$fila2[3],$fila2[4],$fila[14],$fila2[5],$fila2[6],$fila[7],$fila[8]);
				//echo $DetalleFac[$Id][$NoFac][5]." ".$DetalleFac[$Id][$NoFac][6]." ".$DetalleFac[$Id][$NoFac][7]."<br>";
				//$Datos[$Nofac]=(grupo,tipo,cantidad,vrunidad,almacenppal,nofac,codigo,nombre,generico,presentacion)
			}	
		}
	}
...
class PDF extends PDF_Rotate
{
	...
	function Datos($NoFac)
	{
		...
		foreach($GrupsMeds as $GM)
		{
			...
			{	
				if($Meds[$NoFac][0]==$GM[0]&&$Meds[$NoFac][2]=="Medicamentos"&&$Meds[$NoFac][4]==$GM[1])
				{
					$ban=1;
					$this->SetFont('Arial','',8);
					$this->Ln(5);					
					$this->Cell(20,5,substr($Meds[$NoFac][0],0,12),"LR",0,'C');					
					$this->Cell(100,5,strtoupper(substr($Meds[$NoFac][1]." ".$Meds[$NoFac][7]." ".$Meds[$NoFac][6],0,52)),"LR",0,'C');					
					$this->Cell(20,5,substr($Meds[$NoFac][8],0,12),"LR",0,'R');
					$this->Cell(25,5,number_format($Meds[$NoFac][4],2),"LR",0,'R');					
					$this->Cell(29,5,number_format(($Meds[$NoFac][4]*$Meds[$NoFac][8]),2),"LR",0,'R');
					$SubTot=$SuTot+($Meds[$NoFac][4]*$Meds[$NoFac][8]);
		...
		foreach($GrupsCUPs as $GC)
		{
			...
			foreach($DetalleFac as $CUPs){
				...
				if($CUPs[$NoFac][0]==$GC[0]&&$CUPs[$NoFac][2]!="Medicamentos"){
				//$Datos[$Nofac]=(grupo,tipo,cantidad,vrunidad,almacenppal,nofac,codigo,nombre,generico,presentacion)
					...
					$this->Cell(100,5,strtoupper(substr($CUPs[$NoFac][7],0,52)),"LR",0,'C');					
					$this->Cell(20,5,substr($CUPs[$NoFac][2],0,12),"LR",0,'R');
					$this->Cell(25,5,number_format($CUPs[$NoFac][3],2),"LR",0,'R');
					//$this->Cell(29,5,substr("968588493493499349934141234134",0,17),"LRTB",0,'R');
					$this->Cell(29,5,number_format(($CUPs[$NoFac][2]*$CUPs[$NoFac][3]),2),"LR",0,'R');
					$SubTot=$SuTot+($CUPs[$NoFac][2]*$CUPs[$NoFac][3]);
					$GranSubTot=$SubTot+$GranSubTot;	
					$POSY=$this->GetY();
					if($POSY>=250 && $POSY<255){						
						$this->Ln(5);										
		...
		if($DatosFac[$NoFac][5]!=''){		
			...
			$this->SetFont('Arial','B',8);	
			if(!$DatosLiq[10]){
				$this->Cell(165,5,"CUOTA MODERADORA:",1,0,'R');
			}
			else{	
