...
<table BORDER=1  style='font : normal normal small-caps 12px Tahoma;' bordercolor="#e5e5e5" cellpadding="2" align="center">  
    ...
    <tr>
		...
		<?	}
				...
				echo "<strong>Proceso Sin Unidades</strong>";
			}?>
        </td>
   	<?	if(!$Desde){
   ...
   <!--      
     ...
     <td>
     	<select name="Operador">
        	<option></option>
            <option value=">" <? if($Operador==">"){?> selected<? }?>>></option>
            <option value="<" <? if($Operador=="<"){?> selected<? }?>><? echo "<"?></option>
            <option value="=" <? if($Operador=="="){?> selected<? }?>>=</option>
        </select>
     </td>
     <td bgcolor="#e5e5e5" style="font-weight:bold" align="center">a</td>
     <td>
     	<td><input type="Text" name="FecBusq"  readonly onClick="popUpCalendar(this, FORMA.FecBusq, 'yyyy-mm-dd')" value="<? echo $FecBusq?>"></td>
     </td>
     </tr>
    <tr>
...
<iframe frameborder="0" id="PacientesxConsolid" src="PacientesxConsolid.php?DatNameSID=<? echo $DatNameSID?>&Ambito=<? echo $Ambito?>&Pabellon=<? echo $Pabellon?>&FechaIni=<? echo $Desde?>&FechaFin=<? echo $Hasta?>&Ver=<? echo $Ver?>&CedPac=<? echo $CedPac?>&FecBusq=<? echo $FecBusq?>&Operador=<? echo $Operador?>" width="100%" height="85%">
</iframe>
</body>
</html>