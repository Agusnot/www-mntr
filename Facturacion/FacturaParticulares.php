	...
	for($i=$FacIni;$i<=$FacFin;$i++){	
		...
		if($filaFormat[0]==$Formato){
			...
			if($fila[9]==1){				
				...
				$DatosLiq[$NoFac]=array($fila2[0],$fila2[1],$fila2[2],$fila2[3],$fila2[4],$fila2[5],$fila[6],$fila2[7],$fila2[8],$fila2[9],$filaCopoCuota[0],$fila2[10],$fila2[11]);				
			}	
			
			$cons2="select grupo,tipo,sum(cantidad),vrunidad,almacenppal,codigo,nombre,generico,presentacion,sum(vrtotal)
			...
			while($fila2=ExFetch($res2)){
				...
				$DetalleFac[$Id][$NoFac]=array($fila2[0],$fila2[1],$fila2[2],$fila2[3],$fila2[4],$fila[14],$fila2[5],$fila2[6],$fila2[7],$fila2[8],$fila2[9]);
...
class PDF extends PDF_Rotate
{
	...
	{
		//$this->AddPage();			
		global $DatosLiq; global $Compania; 
		...
		if(!$FechaIngreso)
		{
			$cons="select fechaing from salud.servicios where compania='$Compania[0]' and numservicio=".$Liq[$Fac[14]][8];
			$res=ExQuery($cons);
			$fila=ExFetch($res);
			$FechaIngreso=explode(" ",$fila[0]);
			$FechaIngreso=$FechaIngreso[0];
		}	
		//echo "Fecha Ing : $FechaIngreso  --> Fecha Egr: $FechaEgreso";
		...
		if($Cliente[9]==1){						
			...
			{
				$this->Cell(86,5,$FechaPeriodoFin,0,0,'L');///
			}
			$this->SetFont('Arial','B',8);	
		...
		foreach($GrupsCUPs as $GC)
		{
			...
			foreach($DetalleFac as $CUPs){
				...
				//$Datos[$Nofac]=(grupo,tipo,cantidad,vrunidad,almacenppal,nofac,codigo,nombre,generico,presentacion,vrtotal)
					...
					$this->Cell(123,5,strtoupper(substr($CUPs[$NoFac][7],0,80)),"LR",0,'L');					
					$this->SetFont('Arial','',8);
		...
		foreach($GrupsMeds as $GM)
		{
			...
			{				
				...
				if($Meds[$NoFac][0]==$GM[0]&&$Meds[$NoFac][1]=="Medicamentos"&&$Meds[$NoFac][4]==$GM[1])
				{
					...
					if($Ent=='800198972-6'||$Ent=='890399029-5'||$Ent=='891580016-8'||$Ent=='I891280001-0'||$Ent=='800103913-4'||($Ent=='999999999'&&$DatosLiq[$NoFac][1]!='24222')){
						$Meds[$NoFac][2]=0;
					}	
								
					$this->Cell(20,5,number_format((round($Meds[$NoFac][2],0)*round($Meds[$NoFac][3],0)),2),"LR",0,'R');
					$SubTot+=(round($Meds[$NoFac][2],0)*round($Meds[$NoFac][3],0));					
					$SubTot=round($SubTot,0);
