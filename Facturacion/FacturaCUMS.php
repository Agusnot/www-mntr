<?	
	...
	$cons="select codigo1,autoid,cum from consumo.codproductos where compania='$Compania[0]'";
	$res=ExQuery($cons);
	while($fila=ExFetch($res))
	{
		$CUMS[$fila[0]]=array($fila[0],$fila[1],$fila[2]);
	}
	...
	for($i=$FacIni;$i<=$FacFin;$i++){	
		...
		$res=ExQuery($cons); 
		$fila=ExFetch($res);
		$consFormat="select formato,restriccioncobro from facturacion.facturascredito,contratacionsalud.contratos where facturascredito.compania='$Compania[0]' and contratos.compania='$Compania[0]' and 	
		...
		if($filaFormat[0]==$Formato){
			...
			while($fila2=ExFetch($res2)){
				...
				if($fila2[1]="Medicamentos"){
					if($CUMS[$fila2[5]][2]!=""){
							//echo "<br>$fila2[5] = ".$CUMS[$fila2[5]][2];
							$fila2[5] = $CUMS[$fila2[5]][2];
					}
				}
...
;}else{$Entt=$Entidad[0];}
	...
	{		
		...
		global $RestricCobro; global $Rescric;
		...
		foreach($GrupsCUPs as $GC)
		{
			...
			if($RestricCobro){
				if($Rescric[$GC][0])
				{
					if($usuario[1]=='cristhiam'){echo "restringe $GC";}
				}
			}
