<?	
	...
	for($i=$FacIni;$i<=$FacFin;$i++){	
		...
		where facturascredito.compania='$Compania[0]' and terceros.compania='$Compania[0]' and nofactura=$NoFac and 
		entidad=identificacion $Est";
		//echo $cons."<br>";
		$res=ExQuery($cons); 
		$fila=ExFetch($res);
...
;}else{$Entt=$Entidad[0];}
	...
	function Header2($Cliente,$Paciente,$Fac,$Liq)
	{
		...
		if($Cliente[9]==1){		echo "entra";				
			...
			$consFN="select fecnac from central.terceros where compania='$Compania[0]' and identificacion='$Paciente[1]'";
			$resFN=ExQuery($consFN); $filaFN=ExFetch($resFN);
			$this->Ln(4);	
			$this->Cell(10,5,"",0,0,'L');	
			$this->SetFont('Arial','B',8);	
			...
			/*if($Ambito=="Consulta Externa")
			{
				...
				$this->SetFont('Arial','B',8);	
				$this->Cell(33,5,"FECHA NACIMIENTO:",0,0,'L');
				$this->SetFont('Arial','',8);
				$this->Cell(36,5,$filaFN[0],0,0,'L');	
			$this->Ln(4);
			$this->Cell(10,5,"",0,0,'L');	
			$this->SetFont('Arial','B',8);	
...
/*
	...
	//echo "SubTot = > ".round($Meds[$NoFac][2],0)."*".round($Meds[$NoFac][3],0)." = ".round($SubTot,0)." Entonces = >".round($GranSubTot)." SubTotGrup".round($SubTotGrup,0)."<br>";
		...
		if($DatosFac[$NoFac][5]!=''){		
			...
			$this->Cell(30,5,"",0,0,'D');
			$consCrea="select nombre,usuarios.cedula from facturacion.facturascredito,central.usuarios where nofactura=$NoFac and compania='$Compania[0]'
			and usucrea=usuario"; $resCrea=ExQuery($consCrea); $filaCrea=ExFetch($resCrea);
			$this->Cell(40,5,"RESPONSABLE ".strtoupper($filaCrea[0]),"T",0,'C'); 
			$this->Cell(40,5,"",0,0,'C'); $this->Cell(40,5,"FIRMA PACIENTE","T",0,'C');
			$POSYFirma=$this->GetY();
			if (file_exists($_SERVER['DOCUMENT_ROOT']."/Firmas/".$filaCrea[1].".PNG")){
				$this->Image("$ruta/Firmas/".$filaCrea[1].".PNG",35,$POSYFirma-18,40,15);	
			}
