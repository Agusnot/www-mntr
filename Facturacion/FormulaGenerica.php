...
<?	if($Med){$ME=" and usuario='$Med'";}
	...
	if($Medicamentos){
		$MedsSelect=explode("***",$Medicamentos);
		//echo $Medicamentos;
		$RestricMeds="and autoidprod in (";
		$BanRestricMeds=0;
		foreach($MedsSelect as $MedSel)
		{
			if($BanRestricMeds==0){
				$RestricMeds=$RestricMeds."'$MedSel'";
				$BanRestricMeds=1;
			}
			else
			{
	...
	$ME $FI $FF $NF $Ced $RestricMeds order by numorden desc,idescritura desc";
	//echo $consFec;
...
from salud.plantillamedicamentos,consumo.codproductos where codproductos.compania='$Compania[0]' and plantillamedicamentos.compania='$Compania[0]' 
and numservicio=$Numero and autoid=autoidprod and plantillamedicamentos.almacenppal='$AlmacenPpal' and codproductos.almacenppal='$AlmacenPpal'
$ME $FI $FF $NF $Ced $RestricMeds order by codigo2,codigo1";
$res=ExQuery($cons); 
