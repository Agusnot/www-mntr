...
if($Ver){	
	...
	$cons="select nofactura,fechacrea,(primape || segape || primnom || segnom) as noment,total,fecharadic,fechaglosa from facturacion.facturascredito,central.terceros 
	...
	terceros.identificacion=facturascredito.entidad and  facturascredito.estado='AC' $Rad $Ent $Contr $NoContr order by nofactura";
	$res=ExQuery($cons); echo ExError();
	//echo $cons;
	if(ExNumRows($res)>0){?>
...
$consul1= "select numradicacion from facturacion.facturascredito where Compania='$Compania[0]' and numradicacion is not null Order By numradicacion DESC limit 1";	
	...
	?>      </td> 
        	<td colspan="8"><strong>Fecha Radicacion  </strong>
        		<input type="text" readonly name="FechaRad" onClick="popUpCalendar(this, FORMA.FechaRad, 'yyyy-mm-dd')" style="width:90px" value="<? echo $FechaRad?>">
            </td>          
		</tr>   
<?	}?>        
        <tr align="center">
        ...
        <tr align="center"  bgcolor="#e5e5e5" style="font-weight:bold">	
            ...
            <td>Fecha Factura</td><td >No Factura</td><td>Entidad</td><td>Vr Factura</td><?	if($Mostrar=="Radicado"){?><td>Fecha Radicado</td><td>Diferecia Dias</td><? }?>
        </tr>    
   	<?	while($fila=ExFetch($res)){?>
            ...
            <?	if($fila[5]!=''){
				...
				<?	}
	            		...
	            		<td><input type="checkbox" name="SinRad[<? echo $fila[0]?>]" title="Radiacar"></td>
            <? 		}
				}
            ...
            <?	echo "<td align='center'>$fila[2]</td><td  align='right'>".number_format($fila[3],2); 
				if($Mostrar=="Radicado"){
